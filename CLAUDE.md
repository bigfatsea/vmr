...
| `pricing` | Quota-Aware Routing's pricing resolution engine (P2.2 — see `docs/TokenPlan_Quota_P2_DevPlan_opus-5.md`'s §3.3/§6): `Rate`/`Table` (`pricing.go` — per-1M-token four-component prices; a nil component means "unknown", never "free", the distinction the whole package is built around), `resolve.go` (`Resolve`/`RateAt`/`GuaranteedRate`/`AllPathsComplete` — the three-layer account-override → supplement∪standard → none resolution, plus the 4-step canonical-key auto-resolution; `RateAt`'s discount-form handling is recursive — "the rate below this rule" can be another override, not always the table `Base`, matching the design doc's literal "discount × 下层解析出的费率" wording), `embed.go` (`go:embed` of `standard.generated.yaml`/`standard.curated.yaml`, `LoadStandard`), `resolver.go` (`Resolver` — memoized per-provider `RateFor`, the shape `vmr report`'s per-record resolution needs against a large audit log). `tools/gen_standard_pricing` regenerates `standard.generated.yaml` from `docs/data/model_prices_and_context_window.json` (LiteLLM snapshot, MIT licensed); `standard.curated.yaml` is hand-maintained (a handful of domestic first-party vendor rows the generated snapshot doesn't cover), never touched by the generator. Depends only on `core` + stdlib + yaml.v3; both `config` and `report` import it (two independent consumers — `config` for `metric: cost`'s strict load-time resolution, `report` for best-effort $ estimates — see the package's own doc comment) |
