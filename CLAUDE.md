...
| `config` | YAML load, `${ENV}` expand, strict validation (`KnownFields`), hot-reload watch. `quota.go` holds `Provider.Quota`'s YAML shape (`QuotaConfig`/`LimitConfig`) and its validation — P1 accepts exactly one `requests`\|`tokens` `Limit` per provider, rejecting every other knob in the full design (`cost` metric, multi-window, `rolling`, `models:` scope) with an explicit "planned for a later batch" load error rather than a silent no-op |
