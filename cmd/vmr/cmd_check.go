...
func printProviders(w io.Writer, cfg *config.Config) {
	...
	for _, p := range providers {
		...
		if len(p.RoleMap) > 0 {
			rm := make([]string, 0, len(p.RoleMap))
			for _, from := range fmtutil.SortedKeys(p.RoleMap) {
				rm = append(rm, from+"->"+p.RoleMap[from])
			}
			fmt.Fprintln(w, checkLine(2, "role_map", strings.Join(rm, ",")))
		}
		if p.StickyTTL != nil {
			fmt.Fprintln(w, checkLine(2, "sticky_ttl", p.StickyTTL.D().String()))
		}
		if p.Concurrency > 0 {
			qWait := p.ResolvedConcurrencyQueue().String()
