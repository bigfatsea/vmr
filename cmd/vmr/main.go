...
func usage() {
	fmt.Fprintln(os.Stderr, `usage: vmr <start|check> [-c config.yaml]
       ...
       vmr report [-c config.yaml] [-o dir] [-details=false] [-pricing pricing.yaml] [-lang en|zh] [audit.jsonl|glob]...   (default -o: ./reports; auto-loads ./pricing.yaml if -pricing omitted; -lang default: report.yaml's language, or en; no input files => -c's log_dir/vmr-audit-*)
