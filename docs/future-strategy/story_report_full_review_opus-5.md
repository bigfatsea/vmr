...
| 阶段 | 主题 | 核心交付 | 依赖 | 对应 Package | 状态 |
| --- | --- | --- | --- | --- | --- |
| **P11** | 清理与守卫先行 | 废弃代码清空、守卫覆盖源码注释与"自称当前"的文档 | — | D | ✅ 已完成（commit `b771043`），详见 `story_report_p11_action_plan_sonnet-5.md` |
| **P12** | 渲染表示的正确性 | 详单内容与请求它的那次运行一致；人读产物不再静默吞内容 | P11（守卫先立） | B | ✅ 已完成，详见 `story_report_p12_action_plan_sonnet-5.md` |
| **P13** | 证据层体积归位 | 默认路径产物回到索引量级，且这条纪律有常驻守卫 | **P12（硬依赖）** | A（A1–A5） | ✅ 已完成（详见 `story_report_p13_action_plan_sonnet-5.md`） |
| **P14** | 索引与渲染范围一致化 | 索引首屏每一行都可点；检测器覆盖率对读者可见 | P13（已满足） | A6、E | ✅ 已完成（详见 `story_report_p14_p15_action_plan_sonnet-5.md`） |
| **P15** | CLI 入口收敛收尾 | 别名名副其实且结构上不可能分叉 | P11（其余独立） | C | ✅ 已完成（详见 `story_report_p14_p15_action_plan_sonnet-5.md`） |

**2026-08-21 复核订正：P14 与 P15 合并为一个执行批次。** 二者之间没有硬依赖，且都只改
`cmd/vmr` 这一薄层，任务量合计 6 项，与单个 P12/P13 相当，符合 §6.7 允许的"相邻阶段任务合并"。
合并后共用一份 ActionPlan（`story_report_p14_p15_action_plan_sonnet-5.md`），批内顺序仍是先
P14 后 P15（见下方硬依赖第四条）。以下 6.5 节的 P14/P15 分节描述保持独立，只是执行时不再分两次
验收。

**P11 开工前复核的一个方法论结论，适用于后续每个阶段**：DevPlan 对 P11.2 的字面判定（"六个死函数"）
里有 5/6 被证明是测试基础设施而非可删代码——`deadcode` 报告"unreachable from main()"和"没有理由
存在"不是同一件事。这正是 §6.1 已经声明的"不沿用本章对后续阶段的任何预判"在第一个阶段就应验；
P12–P15 的任务描述同样只是起点，开工前仍须基于当时的真实仓库状态重新核实。
...
- **P14 依赖 P13** —— P14 把默认渲染范围从 238 条候选扩到约 370 条。在体积问题修好之前做，
  是给一个已知问题加码。**此依赖已满足**：P13 已完成，真实语料实测确认默认路径体积回落（见
  `story_report_p13_action_plan_sonnet-5.md` 结语），排期约束已在 `KNOWN_ISSUES §1.42` 登记为
  "已满足"。
- **P15 的 C2 依赖 C1** —— 在 `vmr analyze` 补上"只跑宏观半区/只列候选"两个模式之前，
  把别名薄化成 analyze 的转发会改变别名的产出，违反"别名产出不变"这条 P9.3 就定下的约束。
- **（合批新增）P15 排在 P14 之后** —— P14.1（改 `taskOnlyCandidates` 的过滤判据）与 P15.1
  （给 `cmd_analyze.go` 新增两个模式 flag）都要动 `cmd/vmr/cmd_analyze.go` 同一个文件；合批内
  先做 P14 再做 P15，避免同一文件的两次改动互相踩脚。这不是一条新的跨阶段依赖，只是合批后的
  内部排序。
...
**P15 与 P12/P13 之间没有硬依赖**（P13 已完成，原先"不能与 P13 同时进行"的文件重叠顾虑已随之
消失）；它现在的文件重叠对象是同批的 P14（见上），按第一期"不设并行分支"的约束，两者在合批内
仍是先后顺序、不是并行。
...
**状态：✅ 已完成**（commit `b771043`，2026-08-21）。实际交付范围比本节任务表窄：开工前用
`deadcode` + 逐一读调用点复核，P11.2/P11.3 列出的候选里有 5/6 被证明是缓存正确性差分测试的参照
实现或大规模测试 fixture 构造入口，不是死代码，予以保留；`health.Registry.Available` 同理保留。
真正删除的是 1 个废弃子系统（`blobindex.go`）+ 6 个零引用小函数（含复核中新发现的
`providerProxyLines`）。完整的开工前复核、执行记录与外部独立审阅处置见
`story_report_p11_action_plan_sonnet-5.md`；下表按原计划保留供对照，不代表实际执行范围。
...
**状态：✅ 已完成**（2026-08-21）。实际交付范围比本节任务表宽，分两批扩大：开工前通读
`internal/story` 的两个目标文件全文（不只是原登记点名的行号），在 P12.2/P12.3 的"3 处 `<summary>`
+ 2 处 inline 分支"之外先发现 4 处同类未转义注入点（`toolCallLine` 的 `allShort` 分支、Task 标题、
mid-task 指令行、非工具调用 Step 的 report 行）；随后一次独立外部审阅（gemini-3.7-flash）核查
ActionPlan 时指出两处更严重的问题——一是 `EnsureRendered` 把 evidence 补建放在指纹比对之后，
删除 `evidence/` 后重跑（evidence 模式不变）仍然会跳过重建，P12.1 要修的 M9 场景其实没有真正修好；
二是转义范围的边界划错了，`storyindex.go` 索引表格标题列的 `|` 撕裂问题比已修的 `<summary>` 更重
（撕裂 `vmr-stories.md` 整行，不止吞一段内容），且 `render_md.go` 早已被本次改动触及、"未计划修改"
的说法本身不成立。两处均核实属实后当场修复，前者补了直接复现 bug 的回归测试
（`TestEnsureRendered_RebuildsDeletedEvidence`）,后者把 `render_md.go`/`storyindex.go`/
`render_compare.go` 一并转义，未转义注入点总数从 5 处升到 12 处。P12.4 的归属定案选择了"下沉共享"
而非"两个包各留一份"（`internal/story` 已经在生产代码里 import `internal/reqdetail`，不新增依赖
边界）。完整的开工前复核、逐任务实现细节、外部审阅处置记录（§6.8）与两轮真实语料默认路径验证见
`story_report_p12_action_plan_sonnet-5.md`。
...
**状态：✅ 已完成**（2026-08-21）。按计划全部五项任务落地，`-journey` 命中多个的批量路径判为
"用户点名"而非默认套件的隐式批量（DevPlan 未点名的一处执行期判断，见 ActionPlan §0.4）。一次
独立外部审阅（gemini-3.7-flash）核查 ActionPlan 时指出两点：P13.1 让 `ensureJourneyFile`
（`-compare` 路径）"journey-*.md 已存在即直接返回"这条早退分支不再安全——批量套件可以留下一份
无详单的 `.md`，后续 `-compare` 命名同一候选会永远跳过详单物化、链接永久 404；以及 `detailCell`
若对全量语料逐行 `os.Stat` 会在请求索引的多个渲染函数里叠加出两万次以上系统调用。两点均核实属实
后当场修复（前者补了直接复现该 bug 的回归测试，后者改为入口处一次 `os.ReadDir` 建内存集合）。
真实语料（本仓库 `logs/` 下 07-15 单日 394 条记录、全量 34 文件 11353 条记录）默认路径实测：
默认 `vmr analyze` 从 47MB/253 份详单降到 3.0MB/0 份详单；`-render-all` 全量物化详单体积降约
86%；`vmr report -details` 与 `vmr analyze -render-all` 对同一批记录生成的详单逐字节比对全部
相同。完整的开工前复核、逐任务实现细节、外部审阅处置记录见
