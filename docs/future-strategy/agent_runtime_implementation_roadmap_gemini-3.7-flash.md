...
原路线图的 Phase 1 一次性排了 9 个模块，其中 6 个引入全新的 LLM 判定逻辑。核实后确认两条硬约束会让这个范围在第一次落地时就撞墙：

**约束一：`internal/archtest` 的文件行数预算已经逼近上限。** `internal/archtest/file_sizes_test.go` 用硬编码的 `fileLineExemptions` 表给关键文件设定专属行数上限，未登记的文件走默认的 700 行上限；表里设定专属上限时习惯留约 15% 余量。实测当前行数：

| 文件 | 当前行数 | 预算上限 | 剩余余量 |
|---|---:|---:|---:|
| `internal/story/findings.go` | 547 | 580 | 33 行 |
| `internal/story/findings_toolresult.go` | 288 | 320 | 32 行 |
| `internal/story/metrics.go` | 414 | 470 | 56 行 |
| `internal/story/llm.go` | 415 | 700（默认，未登记） | 约 285 行 |
| `internal/story/llm_single.go` | 52 | 700（默认，未登记） | 余量极大 |

触发预算上限时的报错原文是”split it into another file in the same package, don't just raise this number”——即正确做法是新建文件，不是继续往老文件堆代码，更不是去改预算表里的数字。`findings.go`/`findings_toolresult.go`/`metrics.go` 三个文件的余量都只有 30-60 行，容不下 Phase 1 原计划里 6 个新 Finding 检测器中的任何一个完整实现。（`internal/story/` 目录下已有 19 个非测试文件按能力拆分组织——`compare.go`、`llm_divergence.go`、`llm_single.go`、`findings_toolresult.go` 都是独立文件，这是既定的组织惯例，但没有 git 历史证据表明 `findings_toolresult.go` 具体是从 `findings.go` ”拆分”出来的——它是在一次性提交里作为新文件直接加入的。这个细节不影响”新能力应该落新文件”这个结论，只是不能再引用”拆分先例”这个不准确的说法。）

**约束二：`docs/KNOWN_ISSUES_sonnet-5.md` §1.15”分析半区体量增长与单人可维护性”条目里有一条明确的治理原则**，原文是：”新的探索性 Agent 分析指标，优先用外部脚本消费稳定的 `vmr-report.json` / `journey-*.json` 数据契约做验证，证明价值后再评估是否合入主仓库。”该条目在同文档的汇总表里被标注为”不是一个可修的条目，是一条持续性约束”——即这不是”等以后有空再照做”的建议，而是团队已经确立、当前仍然生效的工作方式。Phase 1 原计划里 6 个引入 LLM 判定的模块（任务完成度、工具结果曲解、语义死循环、目标漂移、计划语义核销、Compaction 约束丢失）恰好都是”探索性程度最高、最需要先在真实语料上验证误报率”的一批，如果直接排进”Phase 1 首周/近期”、直接写进 `internal/story` 的正式 Go 结构体和 Finding Code，就是在正面违反这条已确立的治理原则。

**约束三：团队规模。** VMR 是一个 ≤3 人的 AI-native 团队维护的项目。一次性铺开 9 个模块、其中 6 个需要新建 Prompt 契约、跑校准、控制误报率，在人力上不现实，即便忽略上述两条工程纪律也是如此。

基于这三条约束，Phase 1 拆分为：

- **Phase 1a**：纯规则/统计增强，零 LLM 成本，不产生新的”裁决类” Finding，可以直接落 `internal/story`/`internal/chatmsg` 正式代码路径。
- **Phase 1b**：LLM 判定原型，先用外部脚本消费 `vmr-report.json`/`journey-*.json` 数据契约、对照黄金样本集验证误报率，产出校准报告后再决定是否合入正式代码路径。
...
由于 `findings.go`/`findings_toolresult.go`/`metrics.go` 行数预算余量都只有 30-60 行，Phase 1a 的改动应该按下表分配到文件，避免触发 `go test ./internal/archtest/...` 的行数预算失败：
...
落地前跑一次 `wc -l internal/story/corpus.go`，确认 P1a.4+P1a.5 两项加起来不会把这个文件推过默认 700 行上限；如果接近，同样拆成新文件（如 `internal/story/corpus_contextrot.go`）。
...
3. **Phase 1b 的六个模块，落地顺序建议按”越界正当性”排**：E3/E4/E5/E7 先做（正当性强，方案成熟可以直接进入外部脚本验证阶段），E2 的改造版本（`FindingUnverifiedCompletionClaim`）单独排期，需要先确认三态问法在真实语料上确实比原来的四态判决更少产生误导性输出。
