...
4. **P10 · 历史文档与规划遗留清理**——P0–P9 十个阶段累积了 26 份 `docs/future-strategy/` 规划/
   评审文档（含本文；P7–P9 三个阶段各自的 action plan + review 又新增了 6 份），其中一部分内容
   已经被后续文档吸收、甚至源文件已被删除，但引用它们的文字
...
### P8 · JSON 语言策略统一 ✅ 已完成（2026-08-20）

执行细节、真实语料验证结果、与本节设计的落差见
`story_report_p8_action_plan_sonnet-5.md` §8"执行记录"，本节原文保留不动（作为阶段开工前的
设计依据），不在此重复。
...
**已核实的具体缺口**（2026-08-21 复核，P7–P9 落地后对照当前代码库与文档重新逐条核实，不是原样
沿用本文写就时的判断——下表两处相比原版已按核实结果更新）：

| 任务 | 说明 | 验收标准 |
| --- | --- | --- |
| **P10.1** 修复架构文档里指向 `docs/future-strategy/` 之外文件的引用 | `story_report_architecture_opus-5.md` 四处引用的文件已不在 `docs/future-strategy/` 下（**已核实：不是被删除，是被移到了仓库根 `archived/` 目录——该目录被 `.gitignore` 第 52 行显式排除，是本项目既有的"本地存档、不进 git"惯例，`story_report_comprehensive_redesign_gemini-3.7-flash.md`/`story_report_suite_reorganization_glm-4.7.md`/`story_report_peer_review_opus-5.md`/`story_report_ux_review_sonnet-5.md` 均已在那里找到**）：§0.1 引用 `story_report_peer_review_opus-5.md`（"逐条评审...记录在"）；§1.2 引用 `story_report_suite_reorganization_glm-4.7.md` §0.2；§10.3 引用前两者之一并断言 `story_report_ux_review_sonnet-5.md`"仍然有效"。四处都需要改写为不依赖文件路径的措辞（例如"结论已吸收进本文，原始过程记录已归档，不在版本控制范围内"），不是简单删掉引用——删掉会丢失"这个结论曾经过独立评审"这条可追溯性；也不是简单把路径改成 `archived/...`——那个目录对任何新 clone 这个仓库的人都不存在，指向它和指向一个真正不存在的文件对读者是同一种体验。**范围比原判断更宽一格**：同一节（§10.3）里还有一份"若本方案被采纳，需要相应更新的既有条目"清单——写于方案尚未落地时，如今 P0–P9 已全部落地，清单本身整体过时（部分条目如 `KNOWN_ISSUES §1.20`/`§1.21` 已经在清单内部被划掉标注"已闭环"，另一部分如 `§1.1`/`§1.22` 仍以"待更新"的语气存在，但对照 §3 已闭环第 27/28 项，`§1.21` 已经不需要再更新——这份清单该整体替换成一句"已落地"的指针，而不是逐条勘误 | 四处路径引用与 §10.3 整份"若采纳"清单都不再暗示目标文件在版本控制范围内可达、或方案仍待拍板；`internal/archtest` 的文档引用检查覆盖的是 `CLAUDE.md`/`README*`/`docs/` 顶层，不含 `docs/future-strategy/`（已确认，见该检查器 `doc_refs_test.go` 的显式排除注释），所以这条只能靠人工核对，不能指望测试兜底 |
...
| **P10.3** `KNOWN_ISSUES §4` ROI 表补全 | 原判断（"1.18/1.19/1.21/1.22/1.23/1.28/1.29/1.30 一直没有补进来"）已经不是当前文本——P9 收尾时该表自述行已被改写为"此后陆续新增的 `1.18`/`1.22`/`1.23`/`1.29` 已补进本表，其余仍待补"。**但这句新自述本身与表格不符**：ROI 表实际只有 `1.22` 一行，`1.18`/`1.23` 都不在表里，`1.29`（暂不做）本就不需要单独计分。真正的缺口收窄成了两条：给 `1.18`（中危，六个 LLM 判别器未完成黄金样本校准）与 `1.23`（`collect()` 未接入缓存，§4.1 已有姊妹条目 `1.1` 但 `1.23` 本身没有独立行）补 ROI 行，并把自述行改到与表格实际覆盖范围一致 | `1.18`/`1.23` 在 ROI 表里各有一行且评分依据可对源码核实；自述行准确描述表格当前覆盖范围，不再自相矛盾 |

**阶段验收**：`docs/future-strategy/` 里没有指向不存在文件的死引用（人工通读一遍全部 26 份文档的
交叉引用，逐条核实目标文件存在）；`KNOWN_ISSUES §4` 的自述行与表格实际覆盖范围一致，不再自相矛盾。
