...
| 保留项 | 依据 | 实现代价 |
|---|---|---|
| **多条并存 Limit** | 22% 套餐两种 metric 并存；三层窗口是常态 | 极低——桶/闸角色归并只需一次线性扫描（见 §5.2） |
| **metric: requests** | Coding Plan 74% 按次数计 | **最低**——计数器 +1，完全不需要解析响应 |
| **metric: tokens** | 类型 C / D（Token 总量桶），且 Token Plan 占比 62% 中 94% 按 Token/Credits 计——含原本设想靠 `metric: cost` 表达的 Credits 制套餐（换算一次预算/价格即可，见 §7.1） | 中——需要 usage 嗅探 + 降级估算 |
| **模型倍率（Limit 级，P3 订正）** | 类型 B / D 普遍存在，实测倍率跨度 2～9 倍 | 极低——一个 `map[string]float64` |
| **token 分量权重（Limit 级，P3 订正）** | 类型 D 是 Token 桶，四个分量的扣减比例不均一；也承接了 Credits 制套餐的分量折算需求（比例差 5～120 倍）——理由见 §7.1「Credits/金额制套餐现在怎么表达」 | 极低——四个 float，缺省全 1.0 |
| **Scope（按模型限定）** | 存在返回模型级明细的用量接口，提示可能有独立子额度 | 极低——charge 时一次模型名匹配 |
| **rolling 窗口** | 类型 B 的短窗是滚动的 | 低——分桶近似，约 40 行（见 §8） |

### 4.2 砍掉（有依据的简化）

**① 定价收敛为两层：内置标准表 → 账号覆盖，中间不设外部文件层**
...
曾经有过第三层——独立于标准表之外的用户补充文件（`pricing.supplement`），用来补标准表没收录的模型价格且可回贡上游。它已被彻底删除（Pricing 架构极简化重构）：标准表本身持续刷新，缺失的模型直接提 PR 补进 `standard_price_curated.yaml`，不需要用户各自维护一份等价的私有补丁文件；账号覆盖与列表价缺口从此不再共享一个键空间的暧昧地带。解析顺序：`providers[].pricing.rates` 首条匹配（`discount` 形式 = 下层费率 × discount；显式费率形式直接采用）→ 标准表 → 都没有则该 provider+model 无费率（`vmr analyze` 该行无 $ 估算——不再有任何加载期门禁会因此拒绝启动，那道门禁属于已删除的 `metric: cost`，见 §7.1）。`rate` 是 `(provider, model)` 的纯函数，无时间戳参数（见 ④）。

**表内查键的四步**（`internal/pricing.resolveCanonicalKey`）：① `providers[].pricing.aliases` 的显式映射；② `<provider>/<model>`（provider 是 vmr 自取的名字，只在它恰好等于厂商前缀时命中）；③ 裸模型名——先当 canonical key 直查，再查表的**别名**（下文 ⑥）；④ `*/model` 后缀匹配，按**厂商优先级**定胜负（下文 ⑦）。任一步不命中就落到下一步。四步全不命中、且请求名带 `/` 时，还有一步**裸名重试**：用 `ModelBasename` 掐掉 org/路径前缀后，把同一套解析在裸名上重跑（递归，四步全部）——因为表侧 key 已被生成器归一化成两段，而聚合商 API 强制带 org 前缀的上游名（openrouter 的 `meta-llama/...`、together 的 `google/gemma-...`）不降级就永远够不到表里的行。重试只拓宽：原本四步能命中的名字答案不变，带前缀的名字命中后与裸名请求的答案**逐字节一致**（决策主体与规则都没变，只是输入形态被降到了同一命名空间）。仍未命中即“无费率”，不猜。`ModelBasename` 是“裸名”的唯一权威定义，生成器建 key、解析器降级、歧义报告分组三处同源引用，不存在第二份实现。

**⑥ 别名：裸模型名 → canonical key 的引用（不是价格拷贝）**

别名写在 `standard_price_curated.yaml` 的 `aliases:` 段（这是标准表自己的别名机制，解决"裸模型名撞车该判给谁"；与账号级的 `providers[].pricing.aliases`——"我这个部署里这个本地名字对应标准表哪一行"——是两回事，见 §9.1）。它存的是**目标 key**，不是数字——刷新 generated 表时，每条别名的费率跟着一起动，别名本身永不过期。**只解析一跳**：目标本身又是别名，是加载期错误（`Table.ValidateAliases`），因此结构上不可能成环。目标不存在同样是加载期错误——否则一个拼错的别名会静默落回第④步，命中另一家厂商的价。
...
费率行的形状校验：**不仅"查不到条目"不算错，"条目存在但缺少四分量中的任何一项"却是加载期错误**——缺失当 0 会把这一行的 $ 估算算出一个看起来正常、实则系统性偏低的数字，不写清楚地报错就是在放行"没写 = 免费"这种歧义。显式 `0.0`（上游对免费缓存就这么写）算"存在"，与"字段缺失"必须区分——生成脚本**不得把缺失补成 0**。

> **覆盖度必须诚实说明**：上游标准数据对西方主流厂商四分量齐全，对**国产第一方厂商明显偏弱**（全表 `cache_read` 覆盖率仅 23%、`cache_write` 仅 8%，而缓存命中的分量差可达 5～120 倍，是 $ 估算里最容易失真的部分）。所以标准表是**消除入门断崖的基线，不是记账精度的数据源**——`vmr analyze` 对缺分量的行会标注为不完整估算（`CostRateIncomplete`），而不是假装数字是准的。

标准表内部拆两块（合并后对外一张表）：`standard_price_generated.yaml`（脚本从上游生成、可整体覆盖）+ `standard_price_curated.yaml`（项目手工补国产厂商、脚本不得触碰——否则每次刷新手工行被清掉）。

**② 格式不向上游看齐，但键空间对齐**：**单位**保留 per-1M（比科学计数法少一个数量级出错机会）；**分量语义**与上游 `input_cost_per_token`/`cache_read_*`/`cache_creation_*` 等价、可机械转换；**币种**保留 vmr 的 `currency`/`exchange_rate`（否则表达不了非美元套餐——账号的书写币种经顶层 `exchange_rate` 一次性归一为 USD，见 ⑨）；曾有的 `date_*`/`hour_*` 促销时间窗功能面已移除（见 ④）。唯一改动的是**键**：标准表用上游的 canonical key 空间（生成脚本归一化成 `<litellm_provider>/<basename>`），账号覆盖仍用 vmr 自己的 `provider` + `model`——两套键空间因为它们回答两个不同的问题，这正是 `aliases` 字段存在的原因。
...
**④ 折扣与促销归入价格层，不再借道 `model_multipliers`**：折扣若只在 `model_multipliers`（一个额度概念），`vmr analyze` 仍按列表价计算、系统性高报支出；写进价格层则**额度计量与成本报表同时受益**。整个账号打折 → `rates: [{model: "*", discount: 0.6}]`；单模型价格不同 → 写显式四分量费率。有起止日期的限时活动、分时段价差**不做**——价格层只服务 `vmr analyze` 的 $ 精度这个二阶价值，不为一个使用频率未知的场景背上一整套时间可达性分析。`model_multipliers` 因此是**纯额度语义**（"这个模型烧几倍计数单位"），只作用于 `requests`/`tokens`——这两档是仅有的 metric，谈不上"某 metric 下非法"这条校验了。

**⑤ 把 per-provider 定价并入 `config.yaml`，且不再留任何外部文件出口**：独立的 `pricing.yaml`（无论是最早的 `vmr analyze -pricing`，还是后来的 `pricing.supplement`）不在热重载链路里，留在外面就要另建文件监听 + 原子换表；折算这份账号级定价原本还打算供 `metric: cost` 在请求路径上直接消费，就更没有理由让它活在 Snapshot 之外。并入后落点：叶子包 `internal/pricing`（与 `internal/quota` 同层，只依赖 `core`），`internal/config` 在 `validate()` 阶段解析 `providers[].pricing`（本账号覆盖）叠加内置标准表，产出供 `core.PricingSpec` 使用——`metric: cost` 已随决策 6 整体删除（见 §7.1），这条产物现在只服务离线的 `vmr analyze`，从不挂到 Snapshot 的路由结构上。`cmd/vmr/cmd_report.go` 与 `internal/config` 共用 `internal/pricing` 的同一份解析逻辑（**两个消费者、一份实现**）；`internal/report` 依旧只接收已解析对象、绝不 import `config` 或 `pricing`（archtest 强制）。无 config.yaml 时跑 `vmr analyze` 降级为只用标准表列表价、不含账号折扣。
...
- **转售商没有自己的公开定价记录时，按第一方列表价计价是正确的默认**，不是将就。渠道有自定价时（第②步命中 provider 自有行，如 `dashscope` 对 DeepSeek 模型的自定价），用渠道价；需要覆盖时，配 `providers[].pricing.rates` 即可——那正是账号覆盖层存在的理由。
- **它回答的问题是"这个套餐/代理买得值不值"**，而不是"我这个月被扣了多少"。后者以厂商控制台为准，vmr 不试图复现。

**⑨ 汇率折算只发生在两处，都不在运行态，且不存在"记账货币"这个中间概念**

`internal/pricing.Table`/`Resolver` 解析出的一切永远是 USD——`providers[].pricing.currency` 只是一个
**解析期标注**（这个 provider 的 `rates` 行用什么币种书写），加载期经顶层 `exchange_rate` 一次性
折算成 USD 存入内存，从不作为运行态量传递。汇率因此只有两个消费者，都不在运行态：①
加载期把非 USD 书写的 provider `rates` 归一为 USD；② `vmr analyze` 的 `-currency` 对已解析的 USD
结果做一次纯展示折算（`Resolver.WithDisplayFactor`）。未声明的货币先查内置默认汇率表兜底
（`standard_exchange_rate.yaml`），仍查不到才报错——不允许静默按 0/1.0 处理。

这条设计吸取了一个真实教训：早先曾设想一个全局"记账货币"（USD 标准表 → 汇率 → 记账货币 →
`-currency` 显示汇率 → 报表展示币种），因子若挂在 per-provider 策略上，审计日志里点名的、
**曾经跑过但今天 config.yaml 已无条目**的 provider（改过名的、删掉的、被 `api_keys` 拆分过的）
就会以 1.0 折算——同一条 canonical 价格行在同一张表的相邻两行里一行 USD、一行记账货币，
看上去都很正常。现在这条风险被结构性消解：`providers[].pricing.currency` 只是解析期的书写标注，
折算发生一次、结果恒为 USD，不存在"记账货币是哪一种"这个需要按 provider 反查的问题。

汇率换算一律走 **USD 单跳中枢**，不做 M×N 的任意币种链。

**⑥ 折算规则下沉至 Limit 级**：多窗口复合配额里，短窗速率闸通常按原始请求数/等权计量，长周期账单桶需要精确的 `token_weights` 或 `model_multipliers`。若留在账号级，用户被迫让所有窗口共享同一套不适用的系数——是表达力硬伤。因此 `token_weights` 与 `model_multipliers` 完全下沉至单条 Limit 内部，`token_weights` 仅对 `metric: tokens` 合法。
...
- **`raw < 1` 恰好等价于「已用% > 周期已过%」**：`(1-u)/(1-e) < 1 ⟺ 1-u < 1-e ⟺ u > e`。
  ...
  90%，据此报警等于对正确配置持续报警）。`vmr analyze` 的额度耗尽 Finding 正是据此设计成
...
* **桶（Bucket）**——对应**计费周期**的那条 Limit。未用完的额度真的对应到花掉的钱，
  **use-it-or-lose-it 成立**，`raw > 1` 时应当**主动提升**权重。它的每个分量都是 vmr
  自己计量的实测值（`ChargeResponse` 扣账、日历窗口），数字是真的，配得上连续进评分。
* **闸（Gate）**——更短的窗口（5h、周、分钟）。它是**厂商真实速率上限的本地代理**，
  配置上应设得比真实限流紧一点（安全余量）。它保证携带的信息只有一比特——
  **烧断（`used ≥ amount`）或没烧断**——`raw` 的中间值是余量松紧的产物，不是容量刻度。
  用满闸没有任何经济价值，但烧断闸有真实价值：它在厂商 429 之前先本地跳闸。
...
**"周期最长者为桶"这条规则本身是相对某个 judging set 而言的，混合 Scope 时 judging set 不能取全账号的
Limit 全集。** 一个 provider 的 Limit 可以同时有共享池（不写 `models`）、通配（`models: ["*"]`）、限定列表
（`models: [a, b]`）三种 Scope；路由评分从来只按"这条端点的模型实际适用哪些 Limit"（Scope 过滤后的子集，
`AppliesToModel`）取 `BucketIndex`，从不看全集。展示层（`/status`、`vmr status`、`vmr check`）如果直接对全量
Limit 取一次全局 `BucketIndex`，会在下面这种配置下算错：一个限定列表 Limit 的周期比共享池长，会赢下全局
`BucketIndex`，但它只覆盖它点名的模型——对没被点名的模型，它根本不在其 `AppliesToModel` 子集里，那些模型
自己的路由视角里共享池仍是（甚至是唯一的）桶。于是共享池那一行会被错误标成闸，而受害的恰恰是"没有专属行"
的那些模型（它们唯一的可见信息就是这一行）。修法不是让共享行也去凑某一个具体模型（它天然覆盖所有模型，
没有单一模型可选），而是把它的 judging set 收窄成"同样覆盖全体模型的 Limit"——共享池与通配互相竞争，
限定列表被排除在外，因为限定列表本来就只为它点名的模型竞争，不该跨界赢下一个它不完全覆盖的角色
（`internal/quota.Role` 是这条规则唯一的实现，见其文档注释；`vmr check` 对限定单个具名模型的 Limit 能直接
按那一个模型算出精确答案，唯独通配或列出多个模型的 Limit 结构性地给不出单一正确值，见可观测性一节）。
`Role` 判定"谁是 judging set 里的赢家"时按 `limits` 的原始下标比较身份，不按字段值——共享池与一条
metric/period/amount/since 恰好全等的通配 Limit 是合法配置（校验只在同类 Scope 间查重复），字段值相等
分不清两者，按下标才能。
...
```
headroom(L) = raw(L)              L 是桶（clamp 到 [0, HeadroomCap]，HeadroomCap = 5）

score(provider) = raw(桶)         所有闸都活着（used < amount）
                = 0               任一闸烧断（used ≥ amount）
```

闸是**约束**，不是刻度：活着就不参与评分，烧断就把整个账号归零、沉底到短窗口重置。
桶是**目标**，闸是**约束**——约束的松弛量永远不掺进效用函数。`raw` 的中间带
（0 < raw，无论大小）不携带评分可用的信息：把它当刻度用，等于消费一个配置契约不
承诺存在的信号。闸只在**一个点**上要求校准正确（烧断阈值），二值语义恰好只消费
这一个点，对校准误差处处稳健。

> 演进记录：闸的归并先做过 `GateReserve` 渐进带（无数据支撑的"提前预警"宽度），
> 后改为 `min(1, raw)` 硬封顶（原 2.88：闲闸把带闸账号的桶抢跑加分压死在 ≤1.0），
> 最终订正为今天的二值保险丝。三版共享同一条不变量——**闸永远不会把分数抬高**——
> 每删一层没有数据支撑的连续信号，语义就更诚实一分。

**配置契约**（UserGuide 同步）：闸的 `amount` 应设得比厂商真实限流**紧一点**。
烧断发生在厂商 429 之前是机制在工作，不是事故：满速用到安全线 → 本地干净沉底 →
短窗口重置后满血复活——对"榨干一个速率受限账号"这恰是正确的占空比行为。余量须
同时覆盖校准误差与在途请求（评分时已发出、尚未扣账的量）；余量为零时，sticky 会话
在闸烧断后仍 pin 原 provider 的超闸流量才开始吃 429——"紧一点点"是模型成立条件，
不是优化建议。

验证（类型 A，三层窗口）：5h 闸已用 5500/6000（活着）+ 月度桶恰按进度消耗：

```
桶(月)   = 1.0                     （按进度消耗）
闸(5h)   = 活着（5500 < 6000），不参与
score    = 1.0
```

同一闸用到 6000/6000 烧断 → `score = 0`，整个账号沉底，直到 5h 窗口翻转。
归并结果不再等于闸自己落后节奏的比例——那个比例本来就不是闸要表达的量。

**保真度缺口**：厂商速率限制多为滚动窗口，本包只支持 tumbling——一次突发烧断后
账号要暗到下一个整窗口才复活，可能比厂商真实的惩罚期长。这把 rolling 窗口（见
"现状与后续计划"）从精度改进抬成了有经济后果的缺口；实现前靠用户把闸窗口配成与
厂商真实重置窗口一致来缓解。
...
Limit 只支持 `requests`/`tokens` 两种 metric——曾经设想过的第三档 `metric: cost`
已随 Pricing 架构极简化重构（决策 6）整体删除：不是降级或延后，
是路由半区从此**不再解析、不再消费任何价格数据**。`cost` 档当初为什么会被设计进来、
后来又为什么判定它把控制面和计费面焊在了一起，完整论证留在 §12/§13/§14/§15 的决策
记录里，作为历史存档，不代表当前可配置项。
...
| `tokens` | `Σ_c tokens_c × TokenWeights[c]`，权重按单条 Limit（见「折算规则下沉至 Limit 级」）、缺省全 1.0 | 类型 C / D，以及原先需要 `cost` 表达的 Credits 制套餐（换算一次预算/价格，写成 token 数即可，见下） | 中，见 §7.2 |

**Credits/金额制套餐现在怎么表达**：控制面（Limit）要解决的是"别让某个账号/模型把流量吃光"——一个跨模型、跨 Provider 的**量**的均衡问题，直接作用的 substrate 是 token 和请求次数，不是 cost（cost 是 tokens × 价格的**投影**，价格随时间变化，让控制面的触发时机跟着价目表/汇率漂移，是把控制面和计费面焊死在一起）。cost Limit 表达的一切，`tokens` Limit 都能表达：把预算除以价格算一次、写成 token 数，语义从此稳定；"贵模型多占额度"由既有的 `model_multipliers`（按模型倍率）与 `token_weights`（四分量权重）承担，无表达力损失。$ 估计仍然可以从 `vmr analyze` 得到——那是离线的、允许随价目表刷新而漂移的估算，从不是运行态的记账。
...
非等权时（`cache_read`/`cache_write` 折算比例不同）承担了原先要靠 `cost` 才能表达的
"分量折算"需求（见 §7.1「Credits/金额制套餐现在怎么表达」）。

`requests` 与 `tokens`（含 `token_weights`）分两批交付：`requests` 与等权 `tokens`
属第一批（§14.3 P1），`token_weights` 属第二批——理由见 §14.2 的依据 ①：`headroom`
是账号内部的比值，计量单位上的常数倍偏差会自动约掉，所以路由决策不必等到绝对单位
做准。
...
> 正确的分母是**原始四分量之和**。公式在 `internal/quota` 里只有一份
...
这是**近似而非精确复刻**，需在用户文档写明。显式 `since` 建议写 `YYYY-MM-DD` 或带本地偏移的
RFC3339（`…+08:00`）——写成 `Z`/UTC 结尾会把每个周期边界锚到那个 UTC 时刻（`…T00:00:00Z`
在 UTC+8 上落在本地 08:00 而非午夜）。另注：`DisplayZone` 是进程级固定值，若它在两次运行之间
改变导致 `PeriodStart` **前移**，会命中 `resetIfStaleLocked` 的新周期分支、清零计数（后移则按
时钟回退处理，保留计数 + 一次 WARN）。
...
```yaml
# 全局汇率表：仅有两个消费者，且都不在运行态——① 把某个 provider 用非 USD 书写
# 的 pricing.rates 归一为 USD（加载期一次）；② vmr analyze 的 -currency 展示折算。
# 未声明的货币先查内置默认汇率表兜底，查不到才报错（见 §4.2⑨）。
exchange_rate:
  CNY: 7.1
...
providers:
  ...
  # ── 类型 D：Token 桶 + 分量权重 + 模型系数（原 Credits/金额制的表达方式）───
  # 账号内各模型共用一套分量比例 → 用 tokens + token_weights，不必去维护费率表；
  # 原本需要 metric: cost 才能表达的"cache read 比 fresh input 便宜"这类分量折算，
  # 现在也走这条路径（见 §7.1「Credits/金额制套餐现在怎么表达」）。token_weights/model_multipliers 是 Limit
  # 自己的字段（见 §12.1「折算规则的层级」一行的订正）——三条窗口若真的共用同一套
  # 比例，就得三处各写一遍；这里假设 5h 的速率闸窗口够短，厂商对它不做缓存折扣
  # 区分，所以只在两条更长的窗口上写了权重。原"金额预算"账号迁移过来时，把预算
  # 除一次当前价格换算成 token 数即可（一次性换算，语义从此稳定，不再随价目表漂移）。
  - name: plan-d
    quota:
      limits:
  ...
  # ── providers[].pricing：只影响 vmr analyze 的 $ 估算与 vmr check
  # 的展示，从不进入运行态（不进路由、不进配额、不进审计日志）──────────────
  - name: plan-e
    pricing:
      currency: USD                            # 解析期标注：下面 rates 行的书写币种
      aliases: {my-model-x: vendor/model-x}    # 仅在自动解析对不上 canonical key 时才需要
      rates:
        # 该模型的实际分量费率与列表价不同 → 写显式费率（每 1M token）。下面的通配
        # 是 discount 形式（不终结匹配），所以本条写在它前后都可以；但若通配也是
        # 显式形式，本条就必须写在它之前，否则加载期错误。
...
| 字段 | 说明 | 缺省 |
|---|---|---|
| `exchange_rate` | 通用的"1 美元 = X `<货币代码>`"映射表，写成 `{CNY: 7.1}`——归一某个 provider 用非 USD 书写的 `pricing.rates`，也是 `vmr analyze` `-currency` 展示折算的来源。未声明的货币先查内置默认汇率表（`standard_exchange_rate.yaml`）兜底，仍查不到才**加载期报错**——不允许静默按 0/1.0 处理 | 空（未用到非 USD 货币时无需声明） |

**账号级定价字段**（`providers[].pricing`，只写"和标准列表价不一样"的部分；整个 `pricing:` 块可选，缺省不影响路由/配额，只是 `vmr analyze` 的 $ 估算少一些覆盖）：

| 字段 | 说明 | 缺省 |
|---|---|---|
| `currency` | 解析期标注：下面 `rates` 行的书写币种，加载期经顶层 `exchange_rate` 一次性折算成 USD 存入内存。**不是运行态量**——不进 `core.Endpoint`、不进审计日志、不进报表标签 | `USD` |
| `aliases` | `本地 model 名 → 标准表 canonical key` 的映射，只在自动解析失败时才需要 | 空 |
| `rates` | 费率覆盖规则**列表**，静态按模型区分（没有时间窗——见"②格式要不要向上游看齐"一节）。每条含 `model`（支持 `"*"`）+ 二选一的 `discount`（乘"链中它下面解析出的费率"——即后续规则、再落到 Base，递归下钻，不终结匹配）或四项显式费率（直接采用、终结匹配）——四项费率与 `currency` 共用同一个账号级标注，不支持逐行各写各的币种。**只有显式规则终结匹配**：一条显式通配 `"*"` 之后的任何规则、以及两条 `model` 完全相同的显式规则，都是加载期错误（`firstDeadOverride`）——永远轮不到。折扣形式（含通配折扣）不终结、不屏蔽任何规则："通配折扣在前 + 专属显式在后"（语义 = 折扣 × 专属价）与"同一 model 两条折扣叠加"（乘法叠加）都是合法配置。若某 model 既无标准表命中、也无任何显式规则兜底（只匹配到悬空折扣），`Resolve` 返回"无费率"（不产出全空费率冒充 `$0.00`）。 | 空 |

`aliases` 缺省时的**自动解析顺序**（`vmr check` 会把解析结果打出来，可审计）：
① `aliases` 显式项 → ② `<provider 名>/<model>` → ③ 裸 `<model>`（西方厂商多为此形）→
...
| 字段 | 说明 | 缺省 |
|---|---|---|
| `metric` | `requests` \| `tokens`（`cost` 已删除，见 §7.1） | 必填 |
...
| `amount` | 该窗口上限（vmr 可观测口径的 token 数或请求次数，无币种、无价格） | 必填，>0 |
...
| `model_multipliers` | 仅本条 Limit 生效的按模型倍率——**按 Limit 配置**，理由同上 | 未命中模型/无通配 = `1.0` |

**命名**：额度块叫 `quota`、价格块叫 `pricing`，都不叫 `budget`。`vmr analyze` 已有一套以金额为中心的
成本估算，配置里再出现一个 `budget` 却可能指次数或 token，是可预见的混淆源。

**校验**（`config.validate`，沿用现有 fail-fast 风格）：

* `metric` 枚举（`requests`\|`tokens`；`cost` 是值不是键，`KnownFields` 抓不到，显式拒绝并给出迁移
  指引：换算成 `tokens` 预算，`model_multipliers`/`token_weights` 承担按模型/按分量的差异表达）；
  `every` 语法与 `N > 0`（支持 `min`, `h`, `d`, `w`, `mo`）；`amount > 0`；
...
* `rates` 每条规则的 `discount` 与显式费率**二选一**，同时出现是错误；
...
* `pricing.aliases` 的每一条显式映射，其 canonical key 必须在内置标准表里存在 → 否则**加载期错误**：
...
* 顶层遗留的 `pricing:` 块（旧形态：`currency`/`exchange_rate`/`supplement`/`standard`/`rates`/
  `aliases` 都曾挂在这里）**整体拒绝**，给出迁移指引——`exchange_rate` 移到顶层，
  `currency`/`aliases`/`rates` 移到各 `providers[].pricing`；不做静默字段搬迁。

### 9.2 运行态

**折算发生在读取侧还是计费侧，按 metric 分**：`Counters` 只存原始事实、`token_weights` 在读取侧套用（账号内共用同一套四分量权重，读取时用当前配置重新加权、历史数据永远有效）。但 `model_multipliers` 在**计费那一刻**就乘进去——`Registry` 按 provider 聚合、不细分到 model；聚合完成后"这些量来自哪个模型、该乘几倍"已丢失。账号一旦配了它，`Counters.Requests`/`.Fresh` 的语义从"原始数"变成"模型加权后的等价单位"（一处需要在 `/status` 展示上讲清楚的语义变化）。`model_multipliers` 是"计费时预计算"这条规则唯一的例外——已删除的 `metric: cost`（决策 6）曾经是第二个例外（`$` 金额连同 `Counters.Cost` 字段一起在计费那一刻算好写入），随它一并消失。
...
这条对 `model_multipliers` 不成立（见上），是上面那条修正的直接原因，不是这里的例外。
...
> 存原始分量 token 而不是存折算后的数值，这条选择依然成立：改配置不需要数据迁移。
> `model_multipliers` 是唯一的例外——它在写入时就把倍率乘进了 `Counters` 本身（见上），
> 账号一旦配了它，`Fresh`/`Requests` 等字段存的就已经是加权后的等价单位，不再是原始
> 分量，这正是"计费时套用"这个选择带来的直接代价。
...
**定价不挂载到 `core.Endpoint` 上**：`core.Endpoint` 不携带任何定价字段，`core.PricingSpec` 只服务离线的 `vmr analyze`（`internal/pricing.Resolver`），从不挂到 Snapshot 的路由结构上——这与 `QuotaSpec` 挂在 `core.Endpoint` 上（见上文）恰成对照：额度是路由决策要读的量，定价不是。已删除的 `metric: cost`（决策 6）曾经需要一个反向挂点——`internal/pricing.Resolve` 的产物预折叠成单个 `core.Rate` 挂在 `core.Endpoint` 的专属字段上，供计费时刻字段直读——随它一起删除后，KNOWN_ISSUES §1.0"让价目表进实时路由热路径"这条红线也从"靠纪律守住的边界"变成"结构上已不存在的东西"：热路径上连价格数据都没有了。

**实现落点**：`every`/`since` 保持普通 `string` 字段，解析与报错都在 `validate()` 里一次性做完、产出写进 `LimitConfig.Resolved core.Limit`（`yaml:"-"`，对 `KnownFields` 隐身）——不用自定义 `UnmarshalYAML`，符合 KISS，`ProviderPricingConfig`/`PricingOverrideConfig` 同理。`quota.LimitKey` 对 per-model Limit 带 `#model=<name>` 后缀（一条 `models: ["*"]`/具体列表的 Limit 给每个实际命中的模型各开一个独立 bucket，不共享计数器）。rolling 窗口未交付（§15.2 #4），`Registry` 因此是简单的 `map[string]map[string]*bucket`（provider name → `LimitKey` → bucket）、每个 bucket 是惰性重置的单一计数器，从 P1 沿用至今；等真交付 rolling 再把 `bucket` 换成能装下 `Ring` 的类型，不现在为一个还没有真实需求的功能预留骨架。

### 9.3 持久化

* 文件 `<log_dir>/vmr-quota.json`，0600（与审计文件同级同权限；不匹配 `vmr-audit-*` glob，
  不污染 `vmr analyze` 的输入）。不新增配置项。
...
**这个文件是有离线读者的**（`vmr analyze` 的额度对照表），所以惰性重置的两个副作用从
...
* **`vmr check`**：打印各 Provider 的 Limit 配置（纯静态，不读运行态），**并打印生效时区**——
  ...
  把生效值显示出来是最便宜的防呆，与显示生效倍率是同一条思路。同一行还打印 `role=`：
  共享 Limit、以及限定单个具名模型的 Limit，都能纯静态精确算出（后者的 Scope 本身就点明了
  该按哪一个模型判定，不需要运行态）；只有通配或列出多个模型的 Limit 会在不同模型下角色不同，
  一条静态行装不下，这里退化为全量近似，尾部追加一条 note 指向 `/status`/`vmr status` 的
  实时逐模型角色，而不是打印一个只对部分模型成立的值。
...
* **`vmr analyze`**：**已交付，但形态与原计划不同**——不是独立的 `viewmodel_quota.go`，而是挂在
...
| 多窗口归并 | 桶 `raw` 为主，任一闸烧断归零（二值归并） | 桶是目标、闸是约束——约束的松弛量不掺进效用函数（见 §5.2） | 取所有 Limit 的 `min`（把闸的余量当刻度，闲闸压死桶的抢跑加分，原 2.88）；加权平均（会让一个已触顶的闸被其他窗口稀释掉） |
| 桶 vs 闸 | 周期最长者为桶，其余为闸（闸活着不参与、烧断归零） | 用满 5h 窗口没有经济价值，只有计费周期的未用额度对应真实浪费；零配置且在每个观察到的套餐上都成立 | 全部当桶（会在 5h 窗口末尾制造无意义的冲量）；全部当闸、不区分角色（会让 P1/P2 已经在跑的"周期过完前主动多用没花完的预付费额度"这个能力也一起消失——是否需要保留这条能力，取决于账号是否真的存在"过期作废的预付费额度"，不是一个能从代码本身推出答案的问题）|
| 闸的归并公式 | **二值保险丝**：活着（`used < amount`）不参与评分，烧断（`used ≥ amount`）归零沉底，直到窗口重置 | 闸是厂商真实速率上限的本地代理，配置契约只保证一比特（烧断/没烧断）；`raw` 中间值是余量松紧的产物，不是容量刻度。此前的硬封顶 `min(1, raw)`（更早的渐进带 `min(1, raw/GateReserve)`）都把连续信号塞进了一个不携带它的数字——渐进带因"无数据支撑"被删，硬封顶同理（闲闸把带闸账号的桶抢跑加分压死，原 2.88）。闸只在烧断阈值一个点上要求校准正确，二值语义只消费这一个点；不变量「闸永不抬分」三代归并一致 | 保留渐进带（多一个无依据魔数）；保留硬封顶（把余量当刻度，闲闸压死抢跑，且利用率被压在安全线之下）；配置闸时不留安全余量（烧断后 sticky 会话的超闸流量才开始吃 429） |
...
| 定价的落点 | 并入 `config.yaml` 的 `providers[].pricing`，解析逻辑放新叶子包 `internal/pricing`（P1 交付后修订，原方案只规划了 `cmd` 侧） | 已核对 `LoadPricing` 在 `cmd` 侧、`report` 只接收已解析的表，故不触碰 `report ↛ config` 禁令；更关键的是 `cost` 把定价放到了请求路径上，独立 `pricing.yaml` 根本不在热重载链路里。P1 已经验证并投入使用同一个模式（`internal/config` 依赖只依赖 `core` 的叶子包 `internal/quota`，在 `validate()` 阶段把配置解析成运行态值）——`internal/pricing` 是这条模式在定价上的复用，让 `metric: cost` 的费率校验和 P1 的额度校验一样发生在加载期，而不必等 `vmr analyze` 跑一次才发现 | 保留独立文件（需另建文件监听 + 原子换表，凭空多一个子系统）；只让 `cmd/vmr/cmd_report.go` 一侧解析、`metric: cost` 另开一条运行时校验路径（两份实现，容易漂移） |
| 折扣与促销 | 归入价格层：`discount` 或显式费率，静态按模型区分（P0-A 移除了曾经的 `date_*`/`hour_*` 时间窗功能面，见"②格式要不要向上游看齐"一节） | 写进价格层则**额度计量与 `vmr analyze` 的 $ 数字同时变准**；只写进 `model_multipliers` 会让报表按列表价系统性高报 | 用 `model_multipliers` 表达折扣（把价格概念塞进额度字段，且报表仍按列表价） |
...
| `metric: cost` 与两层定价表永久从热路径砍掉，路由侧只留 `requests`/`tokens`，$ 全部交给离线 `vmr analyze` | **否决（后于 2026-09-06 反转，见下）** | 当时的判定：复杂度诊断部分成立，但分批与 opt-in 早已是本设计的既有姿态——`cost` 与两层定价表本来就排在 P2、只在配了 `metric: cost` 的账号上才触发解析、缺失费率是加载期显式报错而非静默降级；国产厂商覆盖率低这条已在 §4.2①、§13 如实写明，不是这轮复核的新发现。永久砍掉的真实代价被低估：`token_weights` 是账号级统一四分量比例，装不下类型 E/F（Credits/金额制）账号"折算率按模型分化"这一实测特征（§4.2⑦已论证 per-provider 全局金额权重会系统性偏差），砍掉 `cost` 等于让这部分账号（Token Plan 62% 里的多数）永久停留在等权 token 记账的 3～8 倍高估里。**后续反转**：Pricing 架构极简化重构（决策 6）判定这条代价被这次否决高估了——`token_weights`/`model_multipliers` 已能表达"贵模型多占额度"，真正装不下的只是"按分量精确折算到具体价格"这个记账精度需求，而这本来就该是离线 `vmr analyze` 的工作，不该让控制面的触发时机跟着价目表漂移。`metric: cost` 已于 2026-09-06 整体删除，本行判定作废，完整论证见本文 §7.1 |
...
| **多 Limit 扁平打分 `Score = min(Score1, ...)`（不区分桶/闸）** | **否决** | 计费周期的未用额度是真实预付费（use-it-or-lose-it 成立，欠用 `raw > 1` 应主动加分争取流量）；而短窗速率闸（如 1min RPM）没有经济上的"该主动烧掉"的理由。采纳桶/闸模型：桶可提升，闸二值归并（活着不参与、烧断归零）——二值闸与扁平 min 只在闸过速未烧断的中间带不同（扁平 min 会用闸的 `raw` 渐进压分，那个值不携带容量信息） |
...
| 时段倍率（`requests`/`tokens` 档） | 不做 | 需新配置面，且这类规则只在个别厂商出现、自身还在频繁调整 | 对这类账号系统性低估；调低 `amount` 缓解，或接官方用量 API 校准 |
| 标准表对国产第一方覆盖不全 | 接受，靠账号覆盖补 | 实测上游数据：部分厂商无缓存字段、部分条目无价格字段、部分厂商零收录；全表 `cache_read` 覆盖率仅 23%、`cache_write` 仅 8%（明细见市场参考文档） | 影响的只是 `vmr analyze` 的 $ 估算精度（`CostRateIncomplete` 标注不完整），不影响路由或配额——账号可写 `providers[].pricing.rates` 补显式费率，标准表本身也持续靠 `standard_price_curated.yaml` 的手工行逐步补齐 |
| 标准列表价会过期 | 接受 | 随二进制内置的是快照 | 表内带生成时间戳，报表免责声明与 `vmr check` 一并显示；`tools/` 下有刷新脚本，用户也可自备标准表 |
| 无 config.yaml 时跑 `vmr analyze` | 接受降级 | 账号覆盖存在 config.yaml 里 | 只拿到标准列表价，$ 数字不含该账号折扣；需在用户文档写明 |
...
| 按下游用户/业务线的实时配额 | 不做 | 需在请求路径引入下游身份维度的配额状态，比 Provider 级重得多 | 事后可见性走 `ClientKeyTag`（已逐请求进审计，`report`/`journey` 已能分组） |
...
是。十四项机制（三种 metric、多窗口归并、桶/闸角色、环形分桶、`(every, since)` 周期数学、`model_multipliers`、`token_weights`、Scope、标准定价表 + 生成脚本、per-provider 价格覆盖 + ID 映射、`Source` 抽象、持久化 + 惰性重置、梯队占位重排、usage 嗅探 + 降级）作为"阶段一"确实过重。
...
这一刀砍掉：多条 Limit、多窗口归并、桶/闸角色判定、rolling 窗口、环形分桶。是最大的一刀。
...
| **P2 计量准确** | `token_weights`、`model_multipliers`（纯额度语义）、`internal/pricing` 叶子包 + `go:embed` 双表 + 生成脚本、`pricing.supplement`、`providers[].pricing`、`metric: cost`（四项费率不齐 = 加载期错误） | 排在 P1 之后是因为这批全是"绝对单位"精度问题、错误方向危险（低估→超支），适合在 P1 计量数据可与厂商控制台对照之后再做。破坏性变更：独立 `pricing.yaml` 与 `vmr analyze -pricing` 取消 |
| **P3 多约束** | 一个 provider 多条 Limit + 桶/闸角色、Scope（`models:`）。**不含 rolling + 环形分桶** | 触发方式与原计划不同——不是"429 数据证明值得"，而是一个具体 provider 的复合窗口需求直接触发的（诚实偏离，两者都是合法立项依据）。立项时复核后：桶/闸显式分层**保留**（闸永不抬分，`TestScoreForLimits_GateNeverBoosts` 钉住）；`GateReserve` 魔数拿掉；rolling 明确砍出本批（`core.Limit`/`config.LimitConfig` 已预留 `rolling` 字段，届时只加窗口实现、不触碰配置形状） |
| **P4 校准与看板** | `vmr analyze` 的额度燃尽看板 + 按 `ClientKeyTag` 归因、官方用量 API 适配器（写这个适配器时才抽 `Source` 接口）、标准表刷新脚本纳入定期流程 | 直接消解 §13 里最难受的三条限制（单位换算、绕过 vmr 的流量、时段倍率） |
...
- **P2 折算**：`token_weights` 全 1.0 时 `base(tokens)` 逐字节等于 `In + Out`；非等权时按分量精确加权（不取整）；生成脚本不得把缺失分量补 0、`standard_price_curated` 手工行重新生成后仍在；`providers[].pricing.rates` 的费率行"四分量全给或全不给"在加载期校验（`vmr check`/`start`/热重载三处共用同一个 `validate()`）——已删除的 `metric: cost` 曾在此基础上再加一层"整个账号所有模型都要解析出费率"的门禁，随该 metric 一起删除，见 §7.1。
- **P3 桶/闸**：最长 tumbling 为桶、全 rolling 的账号没有桶；活闸不抬分（`TestScoreForLimits_GateNeverBoosts`）、烧断闸归零沉底（`TestScoreForLimits_BlownGateZeroesScore` + `router` 的 `TestReorderByQuota_MultiLimit_BlownGateDeprioritizes`）、无渐进压制带（`TestScoreForLimits_NearBlownGateDoesNotThrottle`）；单 Limit 严格退化为 P1/P2 行为；`models:` 过滤只对匹配模型计费、`model_multipliers` 按**上游**模型名生效。
...
**P1、P2、P3 已全部交付并投入使用（rolling 窗口除外）；P4 未启动**。按 §14.1 自评的十四项终态机制计，当前实际生效十一项、永久砍掉两项（`Source` 抽象；`metric: cost`——2026-09-06 随 Pricing 架构极简化重构删除）、剩一项待 P4（环形分桶/rolling）。另有一件终态清单之外的事已交付：**`vmr-quota.json` 从进程私有状态升级成对外可读的格式**——`vmr analyze` 现在把它的实时计数器与从审计日志重算的窗口消耗并排展示（§11），带出两条契约（§9.3 的读取前提、§12.1 的"额度公式唯一实现 + 差分测试"纪律），P3 后都已按多 Limit 重新验证过。

`metric: cost` 的删除同时带走了定价体系的第三层：从"内置标准表 → 用户补充表 → 账号覆盖"三层收敛为"内置标准表 → 账号覆盖"两层，外部 `pricing.yaml` 补充表不再存在（§4.2①）；账号级定价字段改名为 `currency`/`aliases`/`rates`（原 `map`/`overrides`），且账号覆盖的书写币种在加载期即归一为 USD，路由半区从此零价格、零币种（§9.2）。这不是对 P2 的降级——是判定 P2 曾经的核心动因（"贵模型多占额度"）本就该由 `token_weights`/`model_multipliers` 承担，而"精确记账"从来就只是 `vmr analyze` 的离线工作，不该反向影响控制面。真正影响可用性的剩余缺口是**内置标准价目表的四分量完整率不高**（§15.3①，现已降级为影响报表精度而非路由/配额行为）。
...
| # | 机制 | 状态 | 说明 |
|---|---|---|---|
| 1 | 三种 metric（requests / tokens / cost） | ⛔ `cost` 已于 2026-09-06 整体删除 | P2 曾全部落地（`core.QuotaMetric` 含 `MetricCost`）；Pricing 架构极简化重构（决策 6）判定 cost 把控制面与计费面焊在一起，整体删除——`core.QuotaMetric` 现只剩 `requests`/`tokens`，`ChargeResponse` 无 cost 分支，见 §7.1 |
| 2 | 多窗口归并 | ✅ P3 | `config.QuotaConfig.Limits` 不再限制为一条；`quota.ScoreForLimits` 按桶/闸规则归并 |
| 3 | 桶 / 闸角色 | ✅ P3 | `quota.BucketIndex` + `ScoreForLimits`（最长周期为桶，闸二值归并：活着不参与、烧断归零——`GateReserve` 魔数与 `min(1,raw)` 硬封顶均已删），单 Limit 时严格退化为 P1/P2 行为 |
...
| 9 | 标准定价表 + 生成脚本 | ✅ P2，2026-09-06 起为定价体系的唯一底座 | `internal/pricing` 的 `go:embed` 双表 + `tools/gen_standard_pricing`；外部 `pricing.yaml` 补充表（曾经的第三层）已整体删除，见 §4.2① |
| 10 | per-provider 价格覆盖 + ID 映射 | ✅ P2（时间窗子功能面已于 P0-A 移除；2026-09-06 改名） | `providers[].pricing` 的 `aliases`/`rates`（原 `map`/`overrides`，`discount`/显式费率/`"*"` 通配，静态按模型区分，无时间维度）；`currency` 字段现为解析期书写标注，加载期归一 USD，不再是运行态量 |
...
十四项里：**当前实际生效 11 项（#2、3、5、6、7、8、9、10、12、13、14）、P4 待做 1 项
（#4，本批范围内明确排除的环形分桶）、永久砍掉 2 项（#11 `Source` 抽象；#1 `metric: cost`，
2026-09-06 随 Pricing 架构极简化重构删除，比 #11 晚得多，但同样是"永久"而非"待排期"）。**

### 15.3 已知缺口与后续建议

按"是否影响可用性"排序，不按批次；每条都说明为什么现在不做，不是简单地标"待办"。

**① 内置标准表的四分量完整率不高（影响报表精度，不再影响可用性）。**
上游数据本身就不均衡：西方主流厂商覆盖完整，国产第一方厂商明显偏弱——部分缺缓存字段、部分
整条缺价格。`metric: cost` 存在时，这条缺口曾经是加载期门槛（四项费率不齐 = 拒绝启动）；
随该 metric 于 2026-09-06 整体删除，缺口的后果已降级为 `vmr analyze` 的 $
估算不完整（`CostRateIncomplete` 标注），不再触及能否启动。`standard_price_curated.yaml` 已
陆续补入经过官方定价页核对的国产第一方厂商条目，但多数仍因 `cache_write`（部分还缺
`cache_read`）未公开而不满足四项齐全——**逐步补齐更多国产第一方厂商的四分量费率是这项缺口
唯一的实质解法，属于持续的数据维护工作，不是一次性的代码任务，因此不排期，靠社区与项目
持续贡献 `standard_price_curated.yaml` 或各自的 `providers[].pricing.rates` 推进。**

**② 报表的"溯源可见"只做到聚合级，没做到逐行级。** `vmr analyze` §2 只给一个汇总（标准表生成日期 + 账号覆盖条数），单行 $ 数字看不出它走的是标准表还是账号覆盖哪一层。真要做需要在 `pricing.Resolve` 返回值里带上来源标记并一路穿到 `report` 的行结构——不是小改动。低频需求，等 P4 做额度看板（`viewmodel_quota.go`）时报表本就要改一轮，届时一并考虑。

**③ 生成脚本的 canonical key 做了归一化，不是上游 JSON 顶层 key 原文。**归一化保留（org 前缀不是模型身份——同一模型在不同卖主处前缀漂移，裸名归一是去重、刷新稳定、厂商优先级比较的前提）。曾经的后果是**桥只修了一半**：表侧归一、请求侧不归一，带 org 前缀的上游名（openrouter 的 `meta-llama/...`、together 的 `google/gemma-...`、fireworks 的 `accounts/fireworks/models/...`）四步全落空、静默 Unpriced（`metric: cost` 存在时曾因此直接加载期报错，现在只是该行无 $ 估算）。已修复：新增 `pricing.ModelBasename` 作为“裸名”的唯一权威定义（生成器建 key、解析器降级、歧义报告分组三处同源），`resolveCanonicalKey` 四步落空后把请求名掐成裸名递归重跑——带前缀请求与裸名请求答案逐字节一致，原本命中的名字结果不变。配套：`ParseTable` 强制 key 至多一段 `/`（三段 key 会劈出隐形命名空间，标准表里出现即加载期报错）。已知残余：某网关自造 id 掐掉前缀后恰与另一模型裸名同名时，重试会命中那家的价——与第 ④ 步 substring 匹配同型的极小概率误匹配，可用 `providers[].pricing.aliases` 先钉（优先级更高）。

**④ `ParseTable` 不校验标准表内费率的正负与有限性**（账号覆盖那层已有 `positiveFinite`/`nonNegativeFinite` 校验）——因为标准表是脚本产出 + `go:embed` 的受控数据，要出现负数/NaN 都得先绕过生成脚本或手工改坏 `standard_price_curated.yaml`。ROI 不足，登记备查。
