...
- ~~HTML 单文件渲染 + 脱敏~~：已实现（2026-08）。`vmr analyze -journey <id> -html` 额外写一份 `stories/journey-<id>.html`——单文件自包含（内联 CSS + 一小段内联 JS 做时间轴滚动高亮，零外部请求、theme-aware），左侧粘性时间轴 + 右侧 Step 卡片瀑布，数据源就是 Markdown 渲染器走的同一个 `*story.Journey`（不重解析、不引入新判断）。`-redact` 脱敏模式（与 `-html` 组合）把每段对话正文（指令、消息、响应、reasoning、tool args/results、标题）替换为 `‹text: N chars›` / `‹json: N chars›` 长度占位，保留结构、角色、token 数、工具名、时间与 compaction/stitch 标记。仅 `-journey` 单命中支持（与 `-llm-addr` 同款"一次一个 Journey"约束）。产物 0600（未脱敏版仍含完整正文）。渲染器 `internal/story/render_html.go` + `render_html_assets.go`，chrome 文案 `i18n/story_html.go`。默认套件不出 HTML index（用户决定）。
