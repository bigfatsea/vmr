...
- ~~HTML 单文件渲染 + 脱敏~~：已实现（2026-08，2026-08 综合评审 §12.4 重新设计为看板）。`vmr analyze -journey <id> -html` 额外写一份 `stories/journey-<id>.html`——**单页看板**，不是 Markdown 的转写：一屏滚动、分四块（判定条：id/时间/结局 · 结构时间轴：Task→Step 骨架，每步一行带模型/工具 chip/failover 徽章/转换标记/Finding 旗标 · 指标：14 格 stat + 一条内联 SVG sparkline 画每步上下文 token · Findings：规则层 + LLM 层）。逐步的完整对话不内联——每个 Step 一行链到 `../details/*.md`（单 `-journey` 恒物化）。单文件自包含（内联 CSS + 一小段 `IntersectionObserver` 滚动高亮 JS，零外部请求、theme-aware），数据源就是 Markdown 渲染器走的同一个 `*story.Journey` / `Metrics` / `[]Finding`（不重解析、不引入新判断）。`-redact` 脱敏模式（与 `-html` 组合）把对话正文替换为 `‹text: N chars›` 占位、**去掉逐步详单链接**（`details/*.md` 是 0600 未脱敏、不随 HTML 分享）、Findings 只保留代码 + Step 锚、compaction 实体名降级为计数；结构、指标、角色、token 数、工具名、时间与 compaction/stitch 标记保留。仅 `-journey` 单命中支持（与 `-llm-addr` 同款"一次一个 Journey"约束）。产物 0600（未脱敏版仍含完整正文）。渲染器 `internal/story/render_html.go` + `render_html_dashboard.go` + `render_html_assets.go`，chrome 文案 `i18n/story_html.go`。默认套件不出 HTML（用户决定）。
