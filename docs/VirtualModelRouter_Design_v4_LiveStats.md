...
- **速率只有一个口径：`toks`**（读时应用，写在 §9 的决策表里防漂移）——单请求的**输出
  token 生成吞吐**：流式请求扣掉 `ttft_ms` 只算生成段
  `toks = tokens.out / ((dur_ms - ttft_ms) / 1000)`；非流式请求用整个 `dur_ms` 作分母
  `toks = tokens.out / (dur_ms / 1000)`——非流式响应是一次性整体写回（respnorm 的
  buffered 模式），`ttft_ms` 在这里标的是"响应就绪"而非一段独立可分离的预处理阶段，
  没有生成段可扣。
  - 分母不用四分量总和（`tokens.in + out + cache_read + cache_write`）÷ 整请求耗时：
    prefill/网络等待混进分母会稀释"生成速度"这个信号本身，而"首 token 慢不慢"这件事
    `ttft_p50/p90` 已经单独回答了，不需要吞吐口径再答一遍；
