...
`TokenPlan_Quota_Routing_Design_opus-5.md`（下称"设计文档"），本节只记路由框架层面的策略性决定，不复述其算法细节——算法本身仍在演进（当前落地状态见设计文档「现状与后续计划」一节），复述一份会立刻过期的公式副本没有意义。
...
按"精度阶梯"分批交付，而不是横向拆"配置/计量/决策"三层各做一半——每一批都是端到端可上线的垂直切片，且严格遵守"先只观测、后再决策"：先跑几天纯计量、拿 `/admin/status` 对着厂商控制台核对准确，确认可信后再打开真正影响路由的那一步。P1（单桶均衡）、P2（计量准确）均已交付；P3（多约束）、P4（校准与看板）的范围与触发条件见设计文档「分批实施」一节——尤其是 P3 明确"触发条件是上线后的 429 数据，不是排期到了就做"，不是提前承诺的固定日期；当前逐项落地状态见设计文档「现状与后续计划」一节。
