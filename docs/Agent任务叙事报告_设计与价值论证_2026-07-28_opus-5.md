// Ver 2026-07-28 23:55, by Opus 5
...
- **Phase A Markdown**：`{out}/stories/journey-<id>.md`，0600/0700，原始内容 `<details>` 折叠，与现有 detail 页风格一致。
...
| # | 问题 | 决策 |
| --- | --- | --- |
| D1 | Journey 断头（头不在输入集合里） | 默认跳过，`--include-partial` 才处理；ID 加 `t-` 前缀自证；**缓存 key 用 step hash 而非 journey id**，因此 ID 不稳定不再是问题 |
...
| 步 | 主题 | 一句话目标 | 人天 | 交付后能做什么 |
| --- | --- | --- | ---: | --- |
| **1** | **能看** | 单 lineage 的事实层叙事报告，切分但不缝合 | **3.5** | 对 84% 的多轮任务出完整报告，16% 出分段报告 |
| **2** | **能信 + 能比** | 跨 compaction 缝合 + 行为剖面 + 一致性测试 | **5** | 100% 覆盖；两份剖面做差即可对比两套 Agent |
| **3** | **能还债** | `report` 迁移到 `ctxgraph`，修三个已知缺陷 | **3** | 一套 lineage 模型两个渲染器；report 自身变准 |
| **4** | **可选扩展** | 四个互不依赖的模块，按当时需要挑 | **9** | LLM 解读 / HTML+脱敏 / subagent / 对比报告 |
...
**T1.1 `internal/chatmsg` 下沉（0.5 天）**
...
**T1.2 `internal/ctxgraph` 内核（1.5 天）**
...
**验收**：`edit_test.go` 用手工构造的 manifest 覆盖五种编辑各至少两例（含边界）；`Scan` 在全语料上跑通且峰值内存 < 200MB（附录 A.7/F12 预算：manifest 13MB + index 10MB）。

**T1.3 `internal/story` 事实层 + Markdown（1 天）**
...
- `cmd/vmr/cmd_story.go`：`vmr story [--session sNN|--journey jNN|--chat <id>|--since|--until] [--include-partial] [-o dir] <file|glob>...`
...
1. `./vmr story --session s231 logs/vmr*` 产出 **2 篇**（s231 在 turn 20→21 有一次 `contract`，附录 A.3），每篇事件流完整、断裂处有显式标注；
2. 全语料 `--since/--until` 扫描不 panic；总耗时 ≤ 现有 `vmr report` 的 2 倍；
3. **不变量断言**：事件流内 `tool_call` 与 `tool_result` 的 id 配对率必须 100%，不满足即 `t.Fatal`（守住 F9）；
4. golden test：`testdata/` 里一份 3 请求的小语料，Markdown 输出逐字节稳定，重跑幂等；
5. `go test ./internal/archtest/...` 全绿；
6. `diff -r` 证明 `vmr report` 输出未受影响。

#### 明确不做

不缝合（断裂只切不接）· 无剖面层 · 无 LLM · 无 HTML · 不动 `report` · 不处理 subagent · 断头 journey 默认跳过 · 不拆 `splice_or_tail` 混合桶（全部按 `ReplaceTail` 处理，留给第 2 步）。
