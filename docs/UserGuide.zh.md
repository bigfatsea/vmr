...
| `GET /stats` | 实时与历史请求统计 JSON：并发门计数（`limit`/`in_flight`/`waiting`）、进行中请求明细（排队/运行中、调用方、上游端点、流式逐块里程碑、卡死秒数）、逐小时（近 48h）/ 逐天（近 90d）历史聚合，以及按独立的 `client_key_tag` 和上游 `key_label` 维度分组的分位数分布（TTFT 与 TPS 的 p50/p90，受 `api_keys` 鉴权保护）。token 的 `in` 是 **fresh input**（不含 cache_read/cache_write——`in + cache_read + cache_write` 才是上游报告的总 prompt）。读结果缓存约 1 秒，多个看板同时轮询只算一次聚合。`-audit=false` 下照常可用（该模式不缓冲任何请求正文） |
| `GET /stats.html` | 浏览器实时统计看板：单页面暗黑看板，1 秒自动轮询，包含进行中请求监视与流卡死高亮预警徽章、各模型/渠道指标分布，并提供指向状态、日志与帮助页的链接 |
