...
| `GET /status.html` | 浏览器控制台主页（**Overview**）：原先的状态与实时统计两块看板合并为一页平铺暗黑页面——vitals 指标带（并发 / 请求 / tokens / 端点健康）加一行系统状态、配额预算、虚拟模型拓扑与独立的 Fallback 端点表（逐端点配额 headroom 与 24h 流量份额）、进行中请求、带错误类别过滤芯片的近期失败、支持 last-10/last-100 整行窗口切换的性能分位，以及单一 24h/3d/7d 时间窗统辖的流量与用量区（SVG 请求+token 组合图，含错误标记与悬停明细，附按 key 用量表）。整页只有一个刷新时钟（5 分钟，点头部倒计时立即刷，标签页隐藏时暂停——暂停态可见）；头部告警 pill 只承载可操作状态（配置问题、冷却中的端点、濒临耗尽的配额）；所有控制台页面共用同一套 API Key 流程；零外部 CDN 依赖 |
