...
| `GET /stats` | 实时与历史请求统计 JSON：并发门计数、进行中请求明细（排队/运行中、调用方、上游端点、流式逐块里程碑、卡死秒数）、逐小时/逐天历史聚合，以及按独立的 `client_key_tag` 和上游 `key_label` 维度分组的分位数分布（TTFT 与 TPS 的 p50/p90，受 `api_keys` 鉴权保护） |
