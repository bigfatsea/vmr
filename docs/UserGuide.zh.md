...
**`-html`**（只对匹配到单个 journey 的 `-journey` 生效）额外写一份 `{out}/stories/journey-<id>.html`：单文件自包含——内联 CSS、一小段内联脚本、零外部请求、theme-aware——左侧粘性时间轴 + 右侧 Step 卡片瀑布。数据和 Markdown 一样，排版为浏览/分享而非 diff。再加 **`-redact`**（需要 `-html`）会把每段对话正文——指令、消息、模型响应、工具参数与结果——替换为 `‹text: N chars›` 长度占位；角色、token 数、工具名、时间、每个 compaction/stitch 标记都保留。未脱敏的 `.html` 仍含完整对话正文（0600，同 `.md`）；脱敏版才是可以带出本机的那一份。
