...
# <img src="vmr-logo.svg" alt="vmr" width="24" height="28" align="absmiddle" style="vertical-align: middle; margin-right: 4px;" /> vmr — 用户指南
...
`config.example.yaml` 逐项注释了全部字段。修改配置数秒内热生效；坏配置被拒绝、不影响运行实例。解析是严格的：未知或拼错的配置键（如 `max_concurency: 8`）会直接导致加载失败，绝不会被静默忽略、让你误以为设置已生效。
...
- **`image` / `tools`**——确定性的硬要求。请求需要某个能力但找不到任何候选声明支持时，直接快速失败，返回 `vmr_no_candidates` 并点名缺失的能力，而不是白白浪费一次必然被拒绝的尝试。`image` 的判断是结构性的（请求里是不是真的有 `image_url`/`source` 图片块），不是靠猜文本内容——正文里恰好提到"image"这个词的纯文本请求不会被误判；一张 vmr 自己的解码器认不出格式的图片，依然算作有图片（"检测到"和"解得出格式"是两回事）。（`thinking`/`audio`/`video` 暂不检测——这几项的请求侧探测逻辑在各厂协议上还没有确认，现在声明它们也不会有任何效果，`vmr check` 会对每处此类声明给出 warning。）
...
`GET /stats` 提供受 `api_keys` 保护的 JSON 遥测接口：
- **并发与在途状态**：实时并发槽位（`limit`/`in_flight`/`waiting`），以及进行中请求的锁无关流式里程碑打点（`sent_at`, `first_byte_at`, `last_byte_at`, `est_out`）。
...
`vmr analyze` 是本节全部内容的唯一入口：宏观报表半区作为默认套件的一部分运行——也可以用 `-macro-only` 单独变焦进这一个半区、完全不跑 [journey 半区](#agent-任务叙事重建journeys)。两种形态共用同一套输出拓扑：输出根目录的汇总 Markdown 报表、`macro/` 下的机器可读切片、`requests/` 下的逐请求数据，以及消费它们的三张看板骨架页。
...
**从磁盘重绘与产物级缓存**。运行时先把全部数据落盘（`macro/*.json`、`requests/index.json`、`journeys/*`、`manifest.json`），Markdown 再从这些文件渲染——与 `-render-only` 走的是同一条路径。`-render-only` 跳过日志解压与聚合，直接从磁盘 JSON 快照重绘全部常驻人读产物（`vmr-report.md`、`journeys/index.md`、`journeys/benchmarks.md`、每份 `journeys/details/j-<id>.md`——作业清单来自 `journeys/index.json` 而非扫目录、`compares/index.md` + `compare-*.md`、以及 `requests/failed.md`）；懒物化的 `requests/details/`、`requests/evidence/` 永远不在覆盖面内（它们的输入是原始审计字节）。`-render-only -lang <语言>` 现在也能换语言——不传 `-lang` 时继承快照上一次渲染的语言，换成别的语言只是纯重渲染，不再需要全量重跑（R1：JSON 数据产品已经语言中立，换语言重绘不比同语言重绘更贵）。在逐文件解析缓存之上，输入未变的重复运行还能靠产物级缓存整段跳过聚合（L2 判据是输入哈希 + 定价指纹 + 分析参数；L3 是渲染器版本 + 语言）——`-no-cache` 绕过全部缓存，是怀疑缓存产物不可信时常备的逃生通道。
...
`vmr analyze` 的宏观半区还能读懂 Agent 工作负载——全部离线、纯规则、不调用 LLM：
...
`vmr analyze` 默认输出英文（上文这些示例展示的是切到中文之后的样子）。在当前目录放一份 `report.yaml`（写 `language: zh`）即可切换成中文，或者在命令行上加 `-lang en|zh` 只影响这一次运行——`-lang` 优先级高于 `report.yaml`。（`-render-only` 现在也能自由换语言——纯重渲染，不算例外，见上文。）`report.yaml` 是独立的一份小文件，跟 `config.yaml` 完全无关：是可选的，存在就从当前目录自动加载（`-report-config path` 可以指向别的路径）。它可以放一个真实的密钥（`llm_key`，见下文），所以跟 `config.yaml` 一样 `.gitignore`——仓库根目录提交的是模板 `report.example.yaml`，照着复制一份改。这个开关总是影响 Markdown 文档。现在**每一份 JSON 产物都语言中立**（R1）：`macro/*.json`、`manifest.json`、`j-<id>.json`、`compare-*.json` 无论 `-lang` 是什么都保持英文基线——`efficiency[].finding`、`highlights[].text`、`j-<id>.json` 的 `findings[]`、`compare-*.json` 的 `rows[].label` 都配一个 `params` 对象（findings 则是稳定的 `code`），带着原始值，脚本可以拿它们自己拼出任意语言的句子，不需要重跑 `vmr analyze`。**唯一刻意的例外**：LLM 生成的正文（`j-<id>.json` 的 `llm_interpretation.text` 与 `llm_findings[]`，`compare-*.json` 的 `llm_interpretation`/`llm_divergence`）是模型自己的原生成语言，没法重新推导，这些改为带一个 `llm_lang` 字段记录它实际是用哪种语言生成的。写脚本解析都应该依赖 `FindingCode`/`MetricCode`/`EvidenceAnchor` 这类不随语言变化的字段去跨语言对齐同一行，而不是旁边的叙述文本。想同时留一份另一语种的副本？`-render-only` 是原地重绘同一份输出目录，所以先复制一份输出目录（或者让新的 `-o` 指向同一份快照的副本），再对副本跑 `-render-only -lang <另一语种>`——这是一次廉价的纯重渲染，不是第二次完整分析，两份语种并存的代价只是磁盘空间和几秒钟，从来不需要重新计算。
...
每次 `vmr analyze` 调用（任何模式，变焦或默认套件）都会在输出根目录幂等刷新三张看板骨架页：`macro-dashboard.html`、`request-browser.html`、`journey-viewer.html`。它们不携带任何业务数据：所有渲染都发生在浏览器端——每个页面 fetch 本次运行写出的相对路径 JSON 切片（`manifest.json`、`macro/*.json`、`requests/`、`journeys/`），再从中渲染 DOM 与内联 SVG——写好这些 JSON 切片，就等于交付了整个看板。每页启动时先 fetch `manifest.json` 并比对其中的 `format` 版本与页面内置的期望版本：不一致时显示告警 banner 但渲染继续（加性变更下老页面大概率照常渲染），manifest 不存在则提示"这不是一套完整的 analyze 产物目录"。页面通过 `#data=` URL hash 参数打开某个具体数据文件（例如 `journey-viewer.html#data=journeys/details/j-<id>.json`）；不带参数时，它会探测对应的索引（`journeys/index.json`）列出候选。`journey-compare`、`benchmarks`、`tool-waste` 现在只有 Markdown 产物（`compares/*.md`、`journeys/benchmarks.md`、报表 §7）——被删掉的这三张 HTML 页早已跟不上对应 Markdown，删掉不损失任何能力。页面需要 HTTP 才能 fetch 到数据——`file://` 直接打开会显示明确的提示，不会静默白屏，但也确实看不了。三种拿到 HTTP 的方式：`vmr analyze -open` 为这一次运行的产物起一个一次性本地查看服务（只绑本机回环地址、随机端口、数据已经在自己磁盘上所以不需要鉴权、前台运行直到 Ctrl-C、自动在默认浏览器打开 `macro-dashboard.html`）——"就想看一眼"场景下的零配置默认选项；任意静态文件服务器指向输出目录也可以；或者通过 vmr 自带的可选 `/reports/` 托管（`config.yaml` 里 `analytics.serve: true`；默认关闭，托管 `analytics.serve_dir`，默认 `./reports`——与 `analyze -o` 同一个默认值，所以先 analyze 再 start 路由器就直接能看）——这一种是为 `request-browser` 那种长期查询场景准备的常驻服务，不是为看一眼准备的，也是三者里唯一需要 `api_keys` 和逐请求 Bearer key 的。在该托管下，HTML 骨架页本身免鉴权（零业务数据），每个数据请求（`.json`/`.jsonl`/`.md`）必须带 Bearer key，收到 401/403 会弹出 key 输入框；目录列表被禁用；完全没配 `api_keys` 时数据请求一律拒绝（403）而非开放——承载完整对话正文的报表绝不会在无鉴权状态下对外服务。骨架页刷新失败只在 stderr 告警——从不阻断或污染分析本身，而且下一次 `vmr analyze` 会用正在运行的二进制重写这些页面。
...
面向 Agent 流量的双向安全层，默认关闭：完全不写 `guard:` 键（或不接线）时，请求路径是零开销、字节保真的透传。完整设计、威胁模型与里程碑/验证记录见 Agent Guard 设计规范（已归档）。`config.example.yaml` 的 guard 块逐项注释了全部旋钮，本节只讲运维视角。
...
| `GET /status.html` | 浏览器控制台主页（**Overview**）：vitals 指标带（并发 / 请求 / tokens / 端点健康）加一行系统状态、活动的 **进行中请求（Live Requests）** 表（首列 Seq，其后为 State / Elapsed / Caller / Model / Provider : Model / Attempts / TTFT（附带 >5s 卡顿告警）/ Tok in / Tok out，已结束请求整行置灰）、基于全局最新 300 条请求的性能分位（支持 last 10 / 30 / 100 / 300 窗口切换，按 Requests 数量倒序排列）、12h / 24h / 3d / 7d 时间窗统辖的流量与用量区（SVG 单柱四层堆叠 fresh in / cache write / cache read / tok out，含请求折线、错误标记与悬停明细，附按 Provider/Model 和按 Caller 用量表），以及页面底部默认折叠的 **近期失败（Recent Failures）** 表。整页主体按单一 5 分钟倒计时时钟刷新（点头部倒计时立即刷，标签页隐藏时暂停——暂停态可见）；Live Requests、Recent Failures 与并发 vitals 由前端自适应轮询单独驱动（有在跑或排队请求时 ~2 秒一拍，空闲退避至 15 秒，标签页隐藏停拍）。头部告警 pill 只承载可操作状态（配置问题、冷却中的端点、濒临耗尽的配额）；所有控制台页面共用同一套 API Key 流程；零外部 CDN 依赖 |
| `GET /models.html` | 浏览器控制台 **Models** 页：配额预算（额度/已用/进度/headroom/24h 份额/重置时间）与按协议分表的虚拟模型 → 端点拓扑（每个协议一张独立表格，展示优先级层级、provider:model、健康状态、账户级 headroom、24h 流量份额、上下文窗口/能力），含统一展示的 Fallback 端点。两张表都是相对静态的配置/拓扑事实，整页只跟随与 Overview 相同的 5 分钟倒计时刷新，不做自适应快轮询 |
| `GET /stats` | 实时与历史请求统计 JSON：并发门计数（`limit`/`in_flight`/`waiting`）、进行中请求明细（排队/运行中、调用方、上游端点、流式逐块里程碑、卡死秒数）、逐小时（默认近 48h；`?range=24h\|3d\|7d` 放宽尾窗，上限 7d 是刻意的）/ 逐天历史聚合、按物理模型维度的累计指标（`by_provider_model[]`）、内存中跨端点全局不分组的最新 300 条完成请求（`recent_requests[]`，供控制台 Performance 视图按全局最新 10/30/100/300 条请求动态切片与倒序呈现）、`overall` 全局窗口块（基于全局环计算分位数与吞吐），以及最近 24 小时内（最多 100 条）失败/取消请求的 `recent_errors` 环（带路由半区盖章的 `error_class`）——按独立的 `client_key_tag` 和上游 `key_label` 维度分组（受 `api_keys` 鉴权保护）。token 的 `in` 是 **fresh input**（不含 cache_read/cache_write——`in + cache_read + cache_write` 才是上游报告的总 prompt）。并发与 in-flight 每次读实时算；历史聚合段读缓存数秒（按 range 分键），多个看板同时轮询只算一次聚合。内存 rollup 是近 7 天滑动窗口（7 个整日历天 + 当天）——更早的行留在只追加的 rollup 文件里归档，但不加载、不返回，所以 `by_*` 累计是滚动近 7 天口径，`/stats` 成本不随部署年限增长（更久的历史交给 `vmr analyze`）。`-audit=false` 下照常可用（该模式不缓冲任何请求正文） |
...
| `GET /log.html` | 浏览器控制台 **Log** 页：共享控制台骨架上的满宽终端——工具条（level 芯片、子串过滤与命中高亮、⌘P 暂停、Copy view）位于自动换行日志区上方——自行打开 `/log` 并使用共用的已存 Key，自动滚底在上翻时交给 "↓ N new" 芯片，断线时由独立横幅提供重连按钮；底部状态条显示 `N shown / M lines`。进行中请求与近期失败表在 Overview 页，不在这里 |
