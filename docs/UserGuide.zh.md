...
| `GET /status.html` | 浏览器控制台主页（**Overview**）：原先的状态与实时统计两块看板合并为一页平铺暗黑页面——vitals 指标带（并发 / 请求 / tokens / 全局 TTFT / 端点健康）加一行系统状态、配额预算、虚拟模型拓扑与独立的 Fallback 端点表（逐端点配额 headroom 与 24h 流量份额）、进行中请求、带错误类别过滤芯片的近期失败、支持 last-10/last-100 整行窗口切换的性能分位，以及单一 24h/3d/7d 时间窗统辖的流量与用量区（SVG 请求+token 组合图，含错误标记与悬停明细，附按 key 用量表）。整页只有一个刷新时钟（5 分钟，点头部倒计时立即刷，标签页隐藏时暂停——暂停态可见）；头部告警 pill 只承载可操作状态（配置问题、冷却中的端点、濒临耗尽的配额）；所有控制台页面共用同一套 API Key 流程；零外部 CDN 依赖 |
| `GET /stats` | 实时与历史请求统计 JSON：并发门计数（`limit`/`in_flight`/`waiting`）、进行中请求明细（排队/运行中、调用方、上游端点、流式逐块里程碑、卡死秒数）、逐小时（默认近 48h；`?range=24h\|3d\|7d` 放宽尾窗，上限 7d 是刻意的）/ 逐天（近 90d）历史聚合、每个 provider 一行带 `last_10`/`last_100` 窗口块（窗口内实际样本数、token 四项和、TTFT 与 `toks` 的 p50/p90——`toks` 是唯一吞吐口径：token 四项总和 ÷ 整请求耗时，流式与非流式同分母）、`overall` 合并窗口块（全局分位数由服务端对所有 ring 样本并集计算），以及最近 50 条失败/取消请求的 `recent_errors` 环（带路由半区盖章的 `error_class`）——按独立的 `client_key_tag` 和上游 `key_label` 维度分组（受 `api_keys` 鉴权保护）。token 的 `in` 是 **fresh input**（不含 cache_read/cache_write——`in + cache_read + cache_write` 才是上游报告的总 prompt）。读结果缓存约 1 秒（按 range 分键），多个看板同时轮询只算一次聚合。`-audit=false` 下照常可用（该模式不缓冲任何请求正文） |
...
| `GET /log.html` | 浏览器实时日志页：共享控制台骨架上的满宽暗色终端——自行打开 `/log` 并使用共用的已存 Key，level 芯片（对纯文本流的展示层启发式分类）、子串过滤与命中高亮、Wrap 切换、Copy view、⌘P 暂停、自动滚底在上翻时交给 "↓ N new" 芯片、断线时出现独立横幅与重连按钮；底部状态条显示 `N shown / M lines`。本地视图操作（清空、过滤）不影响服务端缓冲与实时流 |
