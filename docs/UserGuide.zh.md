...
**`-html`**（只对匹配到单个 journey 的 `-journey` 生效）额外写一份 `{out}/stories/journey-<id>.html`：一张**单页事故报告**——单文件自包含、内联 CSS + 一小段 `IntersectionObserver` 滚动高亮脚本、零外部请求、theme-aware（暗色是飞行记录仪读数，亮色是方格纸上的打印报告）。不是转写：一个 **主因判定** 面板（`CRITICAL`/`WARNING`/`NOMINAL` 严重度印章 + 定级的那一条 Finding 的大白话表述 + `N 步 · Mm Ss · X token` 损耗行，定价能解析时追加 `≈ $Y`，见[成本估算与定价](#成本估算与定价)），一个 **死亡转折点** 条（本次运行最早一次不可逆损伤——丢约束的压缩、未适配的重试循环、或硬历史收缩——都没有则显示"始终在轨"），然后是 Task→Step 结构排成紧凑的"一步一行"时间轴（模型、工具 chip、failover/Finding 徽章、转换标记），一个指标网格加一条内联 SVG sparkline 画每步上下文 token，以及 Findings。逐步的对话不内联——每行链到该 Step 的 `details/*.md`。再加 **`-redact`**（需要 `-html`）就是可以带出本机的那一份：每段对话正文替换为 `‹text: N chars›` 占位，逐步详单链接去掉（`details/*.md` 是 0600、不随分享），Findings 只留代码 + Step 锚，compaction 实体名降级为计数——结构、指标、角色、token 数、工具名、时间、判定条与死亡转折点条均保留。未脱敏的 `.html` 仍含完整对话正文（0600，同 `.md`）。
