...
| `GET /log.html` | 浏览器控制台 **Live & Log** 页：与主控制台等宽（1280px 对齐）的终端与实时监控视图——自行打开 `/log` 并使用共用的已存 Key，终端区域默认不自动折行，带 level 芯片（对纯文本流的展示层启发式分类）、子串过滤与命中高亮、Copy view、⌘P 暂停与自动滚底芯片。终端下方承载活动的 **进行中请求（Live Requests）** 表与默认折叠的 **近期失败（Recent Failures）** 表（由 ~2s/15s 自适应 `/stats` 轮询实时驱动）；底部状态条显示 `N shown / M lines` |
