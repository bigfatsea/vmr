...
- **概览大屏 Overview (`GET /status.html`)**：核心运行大屏。顶部状态条实时展示并发度、总请求数、Token 吞吐（含 Prompt 缓存命中率）与端点健康度；**正在执行请求（Live Requests）** 实时列出活动与排队中的请求及流式耗时；按提供商与模型的**性能分位表**展示 TTFT p50/p90 与 Token 产出速率（`Tok OUT/s` p50/p10，支持 last-10 与 last-100 整行切换）；纯 SVG **流量与用量图表**支持 24h / 3d / 7d 范围无缝切换，并附带模型和调用方分组消耗；底部提供默认折叠的**近期失败明细（Recent Failures）**（24 小时内至多 100 条错误）。并发与 Live Requests 采用自适应快轮询（活跃时 ~2s，空闲时 15s，切到后台暂停）；整页随 5 分钟倒计时时钟自动刷新。
- **模型与配额 Models (`GET /models.html`)**：拓扑与配额配置大屏。展示每条账号的 **配额预算（Quota Budgets）**（配置限额、已用进度、Headroom 比值、24h 流量占比与重置倒计时），以及**虚拟模型与端点拓扑（Virtual Models & Endpoint Topology）**（端点优先级 P0/FB 区分、健康度、Headroom 与上下文窗口/能力芯片）。跟随 5 分钟倒计时刷新，不进行高频轮询。
...
| `GET /status.html` | 浏览器控制台主页（**Overview**）：vitals 指标带（并发 / 请求 / tokens / 端点健康）加一行系统状态、活动的 **进行中请求（Live Requests）** 表、支持 last-10/last-100 整行窗口切换的性能分位、单一 24h/3d/7d 时间窗统辖的流量与用量区（SVG 请求+token 组合图，含错误标记与悬停明细，附按 Provider/Model 和按 Caller 用量表），以及页面底部默认折叠的 **近期失败（Recent Failures）** 表。整页主体按单一 5 分钟倒计时时钟刷新（点头部倒计时立即刷，标签页隐藏时暂停——暂停态可见）；Live Requests、Recent Failures 与并发 vitals 由前端自适应轮询单独驱动（有在跑或排队请求时 ~2 秒一拍，空闲退避至 15 秒，标签页隐藏停拍）。头部告警 pill 只承载可操作状态（配置问题、冷却中的端点、濒临耗尽的配额）；所有控制台页面共用同一套 API Key 流程；零外部 CDN 依赖 |
| `GET /models.html` | 浏览器控制台 **Models** 页：配额预算（额度/已用/进度/headroom/24h 份额/重置时间）与虚拟模型 → 端点拓扑（优先级层级、provider:model、健康状态、账户级 headroom、24h 流量份额、上下文窗口/能力），含统一展示的 Fallback 端点。两张表都是相对静态的配置/拓扑事实，整页只跟随与 Overview 相同的 5 分钟倒计时刷新，不做自适应快轮询 |
