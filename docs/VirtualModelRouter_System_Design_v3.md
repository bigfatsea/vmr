...
internal/report            共享基础：会话/任务分组 + 逐请求详单渲染（不含聚合，见 report2）
                            ├─ session.go/export.go  Agent 会话/任务分组（AnalyzeSessions）+ 工具使用聚合（ToolShapes）
                            ├─ detail.go/render.go   WriteDetails：逐请求 Markdown+JSON 详单（report2 复用）
internal/report2           审计日志聚合统计（vmr report 实际调用这里，透明读取明文/.zst），复用 internal/report 的会话分组与详单渲染
...
读取上一节的 JSONL，输出聚合统计。实现拆成两个包：`internal/report2`（聚合、派生指标、Markdown 渲染、请求详单、定价——`vmr report` 直接调用的那一层）复用 `internal/report`（`AnalyzeSessions` 会话/任务分组、`WriteDetails` 逐请求详单渲染、`ExtractUsage`、`ToolShapes`——共享的确定性/启发式核心，不重复实现）。**与审计格式强耦合：改动记录结构必须同步改这两个包及其测试**（复用 `audit.Record` 类型，编译期即绑定）。
...
vmr report [-o dir] [-details=false] [-pricing pricing.yaml] <file|glob>...
    ...
    # + {out}/details/ 逐请求详单（输入可混合明文 .jsonl 与历史压缩产生的 .jsonl.zst；-details=false 关闭详单；-pricing 打开 §2 成本估算）
```
...
* **两遍读取**：第一遍 `report.AnalyzeSessions` 做会话/任务分组（含 compaction 链路、工作负载标签、工具签名——791 行的启发式核心）；第二遍 `report2.Build` 自己流式重新扫一遍同一批文件，按 `path:line` 用 `sess.Lookup` 把每条 record 接到它的 `ReqInfo`（分组坐标、usage、工具调用等），同时重算 `ReqInfo` 里没导出的原始量（bytes、tool 声明字节数、serving endpoint、错误类）。
...
* **成本估算（H 族，可选）**：`-pricing pricing.yaml` 指定一份本地维护的定价 sidecar（`endpoint → 每 1M token 单价`：`in_fresh_per_1m`/`cache_read_per_1m`/`cache_write_per_1m`/`out_per_1m`；`currency` 缺省 **CNY**）——不传该参数时全程不出现任何 `$`/成本数字，优雅降级。每条 record 按它实际服务的 endpoint 查价，命中则把 `fresh×in_fresh + cache_write×cache_write + out×out` 累加进 `Overall`/`ByModel`/`EndpointsAll`/`ByClient` 各自的 `cost_estimate` 指针字段（`nil` = 未配置定价或该 endpoint 未报价，前端据此判断是否渲染）。定价 sidecar 的 `updated_at` 日期会渲染成一句免责声明（"成本估算基于 X 的价格配置，不代表报告所涵盖历史请求实际发生时的价格"），避免旧报告的 `$` 列被误当成实际账单。
* **JSON 输出**（`meta.format` = **10**，`internal/report2.Format`）：`meta`（格式号、生成时间、输入清单、记录数、坏行数、时段、慢请求阈值、百分位方法说明）；`overall`/`by_model[]`/`by_date[]` 每条含 A（请求/结果分布）+ B（token 经济，含上面的派生字段）+ C（延迟，真 p50/p95/max，`stream_ms` 独立 p50/p95）+ D（bytes/messages/role_chars）+ 可选 H（`cost_estimate`）；`endpoints[]`/`endpoints_all[]`（G 族：attempts/ok/failed/availability/error_rate/error_classes + B/C + 可选 H）；`hours[]`/`hours_of_day[]`；`by_client[]`；`workloads[]`（含 `tool_call_rate`）；`sessions[]`（含 `context_growth` = 末轮/首轮 `tokens_in`、`compaction_chain`）；`tools[]`（`schema_bytes_shipped`/`distinct_called`/`declare_utilization`/`schema_waste_bytes` 派生字段）；`efficiency[]`（§7 自动发现表）。请求级明细**不在**这份 JSON 里（保持"聚合专用"），单独落 `vmr-requests.jsonl`。
...
* **Markdown 输出**：按九个编号章节组织，每章回答一个运维问题（`§0` 摘要 + 自动亮点、`§1` 成本与 Token 经济、`§2` 成本估算——仅配置了 `-pricing` 时渲染，按模型/端点/客户端三张表、`§3` 可靠性、`§4` 延迟与吞吐、`§5` 负载分布、`§6` 会话与任务、`§7` 效率与浪费、`§8` 请求详单入口），外加一段附录（数据源、百分位方法、n 基准、低置信度脚注规则、`⭐` 含义、慢请求阈值）。每张表控制在 7 列以内。百分位单元格带样本量 `(n=…)`，`n<20` 加 `⚠️low-n` 标记；`⭐` 标记任何衍生/预估指标列（相对上游原始返回值而言）。每小时活跃度（请求量柱状图 + p95 耗时折线图）与每小时错误数用 Mermaid `xychart-beta` 渲染。§6 的会话表把定时单发会话（heartbeat/dream_diary，恰好 1 请求）合并成一行；compaction 链路 ≥3 节点时额外渲染一个 Mermaid `flowchart` 展示链路关系。§7 的工具浪费表（Top-5，按浪费字节降序）下方，每个工具形态再展开一个 `<details>` 折叠块列出"调用过的工具"（numbered、按调用次数降序）与"声明但从未调用"（numbered、字母序）——数据来自 `report.ToolShapes()`，60+ 工具的大 schema 不会撑爆文档。`vmr-report.md` 页脚加 `详单见 [vmr-requests.md]` 链接（+ 每个 `client_key_tag` 一个 `[-<tag>](./vmr-requests-<tag>.md)`）。
* **逐请求详单**（`report.WriteDetails`，`internal/report/detail.go` + `render.go`，默认开启，`-details=false` 关闭；`vmr report` 唯一的详单产出路径）：每条审计记录导出一个 Markdown 文件**及同名 JSON 文件**（原始 record，供 jq/脚本查询）到 `{out}/details/`（**全部报表产物文件 0600、目录 0700**）。文件名 `{YYYYMMDD-HHMMSS.mmm}_{虚拟模型}_{真实模型}_{outcome[-错误类]}.md`（`.json` 同名），零填充时间戳开头，按名字排序即按时间排序；同毫秒冲突加数字后缀，重跑幂等覆盖。`WriteDetails` 只写这一批 per-request 文件，不产出任何索引——请求详单是 `report2` 自己的产物（见下）。
...
* **Agent 会话分析**（`internal/report/session.go` + `export.go`）：离线、纯规则、不调 LLM，按「会话 → 任务 → 轮次」分组并提取逐请求特征——核心算法（会话指纹、max-LCP 选父、任务边界判定、compaction 双向链接、`isRealUser` 剥离 OpenClaw 路由头）见上一版设计文档。产物：`report.ToolShapes()`（声明工具 vs 当轮实际调用 + never_called 清单，供 §7 消费）；**`vmr-requests.jsonl`**（`report2.RequestRow`：会话/任务/轮次坐标、`client_key`、endpoint、finish、token 四分体 + `cache_eff`、fallback/truncated/错误类、detail 文件、标题）。
* **请求详单索引 `vmr-requests.md`（`report2.WriteRequestsIndex`）**：按 **Chat User（`client_key_tag`）→ Session → Task → Turn** 组织。每个调用方一段 `# Chat User: <key> · N 会话 N 任务 N 轮`；每个会话 `## sNN · <时间> · N 任务 N 轮`（非 interactive 类追加 `· <class>`）；每个任务 `### tNN · <时间> · N 轮`，紧跟一行首条消息的引用块，然后是轮次表（`轮 / 时间 / msgs / finish / dur / ttft / fresh/cached/out / cache-eff⭐ / 文件`）。**全文时间戳统一转 UTC+8**（`time.FixedZone`，不依赖系统 tzdata，不管源记录自带什么 offset）。**单发定时脚手架**（heartbeat/dream_diary，恰好 1 请求）拆成独立的顶层 `# 定时任务 · <class> 单发会话 × N` 区块（一张扁平表，覆盖每次触发），不挂在任何 Chat User 下；轮次数 >1 的定时会话（真正的多步 cron 任务）仍作为普通会话卡片留在其调用方名下。文末 `# 全部请求（时间序）` 覆盖每条记录，不分组。会话分组以 `(session_id, client_key)` 为键，而不是单纯 session_id——不同调用方各自的"未分组/被拒"请求（`session_id` 均为空）不会因为同用空 ID 而被合并显示到同一个调用方名下。
* **按调用方（`client_key_tag`）分组导出**（`config.APIKeys` + `audit.KeyTag` + `RequestRow.ClientKey`）：`report2.WriteRequestsJSONL`/`WriteRequestsIndex` 各自检测出现过的 `ClientKey` 去重集合，每个非空 tag 都在原有产物旁多写一份同目录 sibling —— `vmr-requests-<tag>.jsonl` 与 `vmr-requests-<tag>.md`——同目录，`details/…` 相对链接不用调整。`vmr-report.md`/`.json` 与 `details/*.md`/`*.json` 本身不受影响，永远不分组不重复。不配置 `api_keys` 时这套逻辑整体不产生任何文件。
...
* **报表增强**：成本核算已落地（§9.4 的 `-pricing pricing.yaml` + §2 成本估算，价目 key 用 `protocol:provider:model` 冒号格式，与 `attempts[].endpoint`/`EndpointRow.Endpoint` 一致）。图表/HTML Dashboard（以 report JSON 为数据源二次开发）仍未设计。
