...
本文档是设计方案的第三版，在第二版基础上做了一次全文重写：合并了条件路由、Sticky Model、性能验证三块此前独立存在的设计文档，压缩了迭代过程中的修订记录性质的内容，只保留对当前系统仍然成立的设计与决策依据。第二版内容已核对全部并入本文档，不再独立存档；早期版本的过程记录如需追溯，见 git 历史。
...
internal/report             `vmr report`：审计日志聚合统计（透明读取明文/.zst）
  ...
  ├─ rows.go                  Report2 及各 Row 类型（= vmr-report.json 的公开 schema）
  ├─ aggregate.go/metrics.go  桶聚合 + 派生指标（fresh/cache_efficiency/真 p50p95/成本估算）
  ├─ sticky.go                Sticky 有效性统计（按会话缓冲，聚合结束后解算）
  ├─ render_doc.go            章节运行顺序 + 共享的 mdTable；render_cells.go 纯格式化与 mermaid
  ├─ section_*.go             一个章节一个文件（新增章节 = 新增文件，不是把某个文件改大）
...
| `GET /admin/status` | `instance`（进程身份 + 配置新鲜度）+ `reload`（最近一次热重载结果）+ `sticky.entries` + 端点健康 + 并发指标 JSON（健康部分含 `probing` 字段——某个端点当前是否正被一次恢复探测占着单飞名额，被动/主动两种模式共用同一个字段）；仅接受 loopback 来源 |
...
读取上一节的 JSONL，输出聚合统计，全部在 `internal/report` 一个包里：`aggregate.go`/`rows.go`/`metrics.go`/`render_doc.go`+`section_*.go`/`requests.go`/`pricing.go`（聚合、数据形状、派生指标、Markdown 渲染、请求详单、定价——`vmr report` 直接调用的入口；渲染拆成 `render_doc.go` + 每章一个 `section_*.go`，数据形状单列在 `rows.go`）与 `session.go`/`export.go`/`detail.go`/`render.go`/`usage.go`（`AnalyzeSessions` 会话/任务分组、`WriteDetails` 逐请求详单渲染、`ExtractUsage`、`ToolShapes`）同包共存，前者是后者的调用方，不是两套并行实现。**与审计格式强耦合：改动记录结构必须同步改这个包及其测试**（复用 `audit.Record` 类型，编译期即绑定）。
...
vmr report [-o dir] [-details=false] [-pricing pricing.yaml] <file|glob>...
    ...
    # + {out}/details/ 逐请求详单（输入可混合明文 .jsonl 与历史压缩产生的 .jsonl.zst；-details=false 关闭详单；加载了定价配置——-pricing 或自动加载 ./pricing.yaml——才会打开 §2 成本估算）
```

* **输入**：一个或多个审计 JSONL 路径/通配符；坏行跳过并计数（`meta.parse_errors`）；超过 128MB 的单行同样跳过并计入坏行。全内存聚合，几十 MB 日志无压力。运行过程**按文件输出进度**到 stdout（`[i/N] <path>  done: M records, K parse errors (Ts)`）。
* **两遍读取**：第一遍 `AnalyzeSessions` 做会话/任务分组（含 compaction 链路、工作负载标签、工具签名——791 行的启发式核心）；每个输入文件的读取+`collect()`彼此独立，跑在一个有界 worker 池上（`analysisWorkerCount` 个 goroutine），文件读完后按原始 `paths` 顺序合并再统一按时间戳稳定排序——排序结果与文件读取的并发顺序无关，所以并行化不改变输出。第二遍 `Build` 自己流式重新扫一遍同一批文件，按 `path:line` 用 `sess.Lookup` 把每条 record 接到它的 `ReqInfo`（分组坐标、usage、工具调用等），同时重算 `ReqInfo` 里没导出（或语义不同）的原始量（tool 声明字节数、serving endpoint、错误类——`bytesIn`/`bytesOut` 不在此列，`collect()` 根本不算这两个值，因为算出来后从没人读过）。`Build` 这一遍同时把每条记录的 detail 渲染也带上了（见「逐请求详单」），`vmr report` 因此只需要对审计源文件读两遍，不是三遍。
* **架构：每维度一个独立桶，各自从原始值算自己的百分位**。百分位不可加——跨桶拿已经算好的 p95 再汇总，只能退化成 `max(row.p95)` 这种错误近似。所以每条 record 在 `Build()` 内一次遍历中同步 push 到**所有相关桶**，每个桶自己收原始 `dur_ms` / `ttft_ms`，各自算 p50/p95；桶与桶之间不互相派生。**`stream_ms`（= `dur_ms − ttft_ms`）是独立收集的第三条原始切片**，不是拿 `dur` 的 p95 减 `ttft` 的 p95——两者不相等（P95(dur)−P95(ttft) ≠ P95(dur−ttft)）。当前桶集合：`Overall`（单桶）、`ByModel`（`model × protocol`）、`ByDate`、`Hours`（`date × local hour`）、`HoursOfDay`（`local hour`，全部日期合并，独立收原始值）、`Endpoints`（`date × endpoint`）、`EndpointsAll`（`endpoint`，全部日期合并）、`ByClient`（`client_key_tag`）、`Workloads`（工作负载类）、`Sessions`（会话）、`Tools`（声明工具集形态，来自 `report.ToolShapes()`）。
* **派生指标**（每个 finish 阶段算完就地写回，原始切片随即释放）：`tokens_in_fresh = tokens_in − tokens_in_cached − tokens_in_cache_write`；`cache_efficiency = cached / (cached + fresh)`（分母为 0 记 0——没有 fresh 输入的负载没有可优化的空间）；`cache_hit_rate = cached / tokens_in`；`slow_requests`（`dur_ms` 超过 30s 阈值的请求数）；`tok_out_per_sec`。比值类指标的分母（`tokens_known`/`requests_with_dur` 等）低于该桶总请求数 90% 时，Markdown 侧标注 `¹` 低置信度脚注。
* **成本估算（H 族，可选）**：`-pricing pricing.yaml` 显式指定一份本地维护的定价 sidecar，不传时自动尝试加载当前目录下的 `./pricing.yaml`（不存在则安静跳过）——全程不出现 `$`/成本数字，优雅降级。定价按 **provider+model**（不含协议）配置：`in_fresh_per_1m`/`cache_read_per_1m`/`cache_write_per_1m`/`out_per_1m`，每个字段既可以是裸数字（用文件顶层 `currency`，缺省 **CNY**）也可以是带货币前缀的字符串（`"USD0.14"`，大小写不敏感），通过顶层 `exchange_rate`（成对等值货币列表，如 `{USD: 1, CNY: 7}`，按图 BFS 支持链式换算）折算成 `currency`；换算用到一个 `exchange_rate` 里没定义、又不是 `currency` 本身的货币，是加载期报错，不是静默按 0 处理。同一 provider+model 可以配多条规则，各自带可选的 `date_range`/`hour_range`（后者支持跨零点），按文件顺序取第一条时间窗口覆盖该请求时间戳的规则——更窄/促销性质的规则要写在兜底规则前面。每条 record 用 `endpoint`（`protocol:provider:model`）拆出 provider/model 查价，命中则把 `fresh×in_fresh + cache_write×cache_write + out×out` 累加进 `Overall`/`ByModel`/`EndpointsAll`/`ByClient` 各自的 `cost_estimate` 指针字段（`nil` = 未配置定价或该 provider+model 未报价，前端据此判断是否渲染）。定价 sidecar 的 `updated_at` 日期会渲染成一句免责声明（"成本估算基于 X 的价格配置，不代表报告所涵盖历史请求实际发生时的价格"），避免旧报告的 `$` 列被误当成实际账单；§2 末尾还会把当次实际用到的 `pricing.yaml` 原文折叠嵌入报告本身，冻结价格快照。
* **JSON 输出**（`meta.format` = **10**，`internal/report.Format`）：`meta`（格式号、生成时间、输入清单、记录数、坏行数、时段、慢请求阈值、百分位方法说明）；`overall`/`by_model[]`/`by_date[]` 每条含 A（请求/结果分布）+ B（token 经济，含上面的派生字段）+ C（延迟，真 p50/p95/max，`stream_ms` 独立 p50/p95）+ D（bytes/messages/role_chars/role_tokens）+ 可选 H（`cost_estimate`）；`endpoints[]`/`endpoints_all[]`（G 族：attempts/ok/failed/availability/error_rate/error_classes + 请求级 requests/requests_ok/success_rate + B/C（含 in/out token 分位数）+ 可选 H）；`hours[]`/`hours_of_day[]`；`by_client[]`；`workloads[]`（含 `tool_call_rate`）；`sessions[]`（含 `context_growth` = 末轮/首轮 `tokens_in`、`compaction_chain`）；`tools[]`（`schema_bytes_shipped`/`distinct_called`/`declare_utilization`/`schema_waste_bytes` 派生字段）；`efficiency[]`（§7 自动发现表）。请求级明细**不在**这份 JSON 里（保持"聚合专用"），单独落 `vmr-requests.jsonl`。
* **双指标原则**：tokens 与 bytes 并行统计，usage 提取覆盖 OpenAI/Anthropic 的 JSON 与 SSE 流四种形态（见 `internal/report/usage.go`）；`tokens_in` 缓存细分（`tokens_in_cached`/`tokens_in_cache_write`，两家上游定义差异的归一化处理）。
* **Markdown 输出**：按九个编号章节组织，每章回答一个运维问题（`§0` 摘要 + 自动亮点、`§1` 成本与 Token 经济——含按角色的消息字符/预估 token 占比、`§2` 成本估算——一旦加载了定价配置就渲染，按模型/端点/客户端三张表 + 折叠嵌入的定价快照、`§3` 可靠性——端点健康与错误类别两张表都按协议（openai/anthropic）再拆分、`§4` 延迟与吞吐——按模型和按端点两张表，都按 tok/s 降序、`§5` 负载分布——按虚拟模型/工作负载类/端点/客户端，后两张表还带每请求 in/out token 分位数，外加每小时和每日的请求量/输入 token 图表、`§6` 会话与任务——只列 interactive 会话，按 Chat User 分组、`§6.5` Sticky 有效性、`§6.6` 端点性价比、`§7` 效率与浪费、`§8` 请求详单入口），外加一段附录（数据源、百分位方法、n 基准、低置信度脚注规则、`⭐` 含义、慢请求阈值）。百分位单元格带样本量 `(n=…)`，`n<20` 加 `⚠️low-n` 标记；`⭐` 标记任何衍生/预估指标列（相对上游原始返回值而言）。每小时/每日活跃度（请求量柱状图 + 输入 token 柱状图）与每小时错误数用 Mermaid `xychart-beta` 渲染。§6 只展示 `Class == "interactive"` 的会话，按 `client_key`（Chat User）分组——单发定时会话（heartbeat/dream_diary，恰好 1 请求）完全不出现在 §6，改在 `vmr-requests.md` 自己的分组里（见下）；compaction 链路 ≥3 节点时额外渲染一个 Mermaid `flowchart` 展示链路关系。§7 的工具浪费表（Top-5，按浪费字节降序）下方，每个工具形态再展开一个 `<details>` 折叠块列出"调用过的工具"（numbered、按调用次数降序）与"声明但从未调用"（numbered、字母序）——数据来自 `report.ToolShapes()`，60+ 工具的大 schema 不会撑爆文档。**`§6.5` Sticky 有效性**验证 Sticky Model（§6.5 调度）到底有没有兑现：同一会话内，落回**上一条请求所用端点**的请求 vs 换了端点的请求，比较两组缓存效率。按**结果**（端点连续性）而不是按机制度量——不需要新增审计字段，也更诚实：sticky 指针命中却落到一个冷端点，这里照样算切换。会话首条无前驱，计数但不入组；两组任一带 usage 的样本 < 20 条时只出表、不下结论。**不解释切换原因**：sticky_ttl 到期、端点冷却、条件路由淘汰、该模型没开 sticky，事后不可区分。按虚拟模型再拆一张表——sticky 是按虚拟模型配置的，那才是可操作的粒度。

**`§6.6` 端点性价比**回答"单位产出的代价"而不只是总花费：成本/1M out token、成本/成功请求、失败尝试数与**失败尝试累计墙钟时间**。全部由 §2/§3 已有字段在渲染期派生，唯一新采集的是 `EndpointRow.WastedMS`。**只记时间、不折算成钱**：失败尝试拿不到 usage（vmr 只从客户端真正收到的响应里提取），厂商通常也不对失败请求计费，标金额会是编造。

`vmr-report.md` 页脚加 `详单见 [vmr-requests.md]` 链接（+ 每个真的写出了 `vmr-requests-<tag>.md` 详单的 `client_key_tag` 各一个 `[-<tag>](./vmr-requests-<tag>.md)` —— 只有定时任务单发流量的 tag 不会出现在这里，因为它没有自己的 Chat User 详单）。
* **逐请求详单**（`internal/report/detail.go` + `render.go`，默认开启，`-details=false` 关闭）：每条审计记录导出一个 Markdown 文件**及同名 JSON 文件**（原始 record，供 jq/脚本查询）到 `{out}/details/`（**全部报表产物文件 0600、目录 0700**）。文件名 `{YYYYMMDD-HHMMSS.mmm}_{虚拟模型}_{真实模型}_{outcome[-错误类]}.md`（`.json` 同名），零填充时间戳开头，按名字排序即按时间排序；同毫秒冲突加数字后缀，重跑幂等覆盖。渲染+写盘（`renderDetail` + 两次 `os.WriteFile`）跑在一个有界 worker 池（`DetailWriter`，`detailWorkerCount` 个 goroutine）上——`vmr report` 本身把 `DetailWriter.Submit` 当 `Build` 聚合循环的 `onRecord` 回调传进去，详单渲染跟聚合共享同一趟对审计源文件的读取，不再单独扫一遍；`WriteDetails` 作为独立、自带文件扫描的备选路径保留下来（测试、或只想要详单不想跑完整聚合的场景），内部复用同一个 `DetailWriter`。`Build` 自身的成功/失败与 `onRecord` 的结果无关——detail 目录写坏了也不会连累已经落盘的 `vmr-report.json`/`.md`，调用方在 `Build` 返回、报表已写完之后才检查 `DetailWriter.Close()` 的错误。请求详单索引是 `WriteRequestsIndex` 的产物（见下），不是这一步的输出。
  * 单条详单头部：`虚拟模型 / 上游端点 / 结果 / 耗时 / 首字延迟 / 尝试次数 / stream / Tokens In/CacheHit/Out / 客户端`；下方 `trace / chat user / tools` 元信息行取值加粗（`<strong>`）
  * 头部表格下紧跟一段 `VMR 路由前判断`（`renderFactsLine`，读 `rec.Facts`，原样展示，不重新计算）：**「请求所需能力」只列出实际探测到的能力**（`image`、`tools`，各自渲染成一个反引号包裹的小标签，都没探测到时显示"无"），加「预估Token数量」（`fmtTokensPlain`，K/M 缩写，不带 `fmtutil.FmtTokens` 那个防止和真实 usage 混淆的 `EST` 后缀——详单页已有文字标签，不需要那层保护）。`rec.Facts` 为空（请求在拿到 `facts` 之前就被拒绝）时这一段整体不出现。
  * 文档按请求物理路径分三段：① Client→VMR、② VMR→上游每次 attempt（headers/body 全量对照，🟢/🔴/🔶 标变化；`think_strip` 剥离前内容与原始 SSE）、③ VMR→Client 响应（SSE 流重组）。
  * Messages 区每条消息默认折叠；**「角色 Token 估算占比」**行用 token 估算而非字符计数——`internal/core.EstimateTextTokens`（ascii 4 字节/token、宽字节 2 字节/token 的分类估算，与 `RequestFacts.EstimatedTokens` 背后同一套公式，导出为 `internal/core` 函数供两处共享）逐段计算，按角色汇总。**注意**：§1 聚合层"请求消息字符及占比"表用的是字符计数（`roleChars`）——两处口径故意不同：详单页关心的是 token/成本，§1 聚合关心的是消息构成占比。
* **Agent 会话分析**（`session.go` + `export.go`）：离线、纯规则、不调 LLM，按「会话 → 任务 → 轮次」分组并提取逐请求特征——核心算法（会话指纹、max-LCP 选父、任务边界判定、compaction 双向链接、`isRealUser` 剥离 OpenClaw 路由头）见上一版设计文档。产物：`ToolShapes()`（声明工具 vs 当轮实际调用 + never_called 清单，供 §7 消费）；**`vmr-requests.jsonl`**（`RequestRow`：会话/任务/轮次坐标、`client_key`、endpoint、finish、token 四分体 + `cache_eff`、fallback/truncated/错误类、detail 文件、标题）。
* **请求详单索引 `vmr-requests.md` + 分组 sibling（`WriteRequestsIndex`，`requests.go`）**：`vmr-requests.md` 本身是一份纯索引，不含任何 Session/Task/Turn 展开——每个分组一条 `## Chat User: <key> · N 会话 N 任务 N 轮`（或 `## 定时任务 · <class> 单发会话 × N`），带一行摘要引用块（请求数/成功率/fresh-cached-out/缓存效率）和指向该分组自己详单文件的链接，文末保留一张不分组的 `# 全部请求（时间序）` 全量表。真正的 **Chat User（`client_key_tag`）→ Session → Task → Turn** 展开（每个会话 `## sNN · <时间> · N 任务 N 轮`，非 interactive 类追加 `· <class>`；每个任务 `### tNN · <时间> · N 轮`，紧跟首条消息引用块和轮次表：`轮 / 时间 / msgs / finish / dur / ttft / fresh/cached/out / cache-eff⭐ / 文件`，「文件」列同时链到 `.md` 与 `.json`）只存在于分组自己的文件里：`vmr-requests-<tag>.md`（每个真实 `client_key_tag`）、`vmr-requests-unresolved.md`（没有 tag 的会话）、`vmr-requests-cron-<class>.md`（每个定时任务类别；`heartbeat` 这一个的文件名固定拼作 `vmr-requests-cron-hartbeat.md`，其余类别照 `-cron-<class>` 模式）。**全文时间戳统一转 UTC+8**（`time.FixedZone`，不依赖系统 tzdata，不管源记录自带什么 offset）。**单发定时脚手架**（heartbeat/dream_diary，恰好 1 请求）不管是哪个 client 发起的，永远归到对应的 `vmr-requests-cron-<class>.md`（一张扁平表，覆盖每次触发），不出现在任何 Chat User 的分组里；轮次数 >1 的定时会话（真正的多步 cron 任务）仍作为普通会话卡片留在其调用方名下。会话分组以 `(session_id, client_key)` 为键，而不是单纯 session_id——不同调用方各自的"未分组/被拒"请求（`session_id` 均为空）不会因为同用空 ID 而被合并显示到同一个调用方名下。
* **按调用方（`client_key_tag`）分组导出**（`config.APIKeys` + `audit.KeyTag` + `RequestRow.ClientKey`）：`WriteRequestsIndex` 检测出现过的 `ClientKey` 去重集合，每个非空 tag 都多写一份 `vmr-requests-<tag>.md`（见上）——同目录，`details/…` 相对链接不用调整。`vmr-requests.jsonl`（`WriteRequestsJSONL`）**不**按调用方拆分，始终是覆盖全部记录的单一文件。`vmr-report.md`/`.json` 与 `details/*.md`/`*.json` 本身也不受影响，永远不分组不重复。不配置 `api_keys` 时这套逻辑整体不产生任何额外文件。
...
**vmr.sh（唯一脚本入口，双模式 + 原生命令透传）**：脚本不认识的子命令原样转发给二进制（`check|diagnose|report|dirs|replay|version|…`），**刻意不做白名单**——二进制新增子命令无需改脚本，拼错由二进制打自己的 usage。转发只调整路径语义两处：① `cd` 回调用者原目录（`report` 的 glob、`-o`、`-detail` 都该按调用者所站目录解析）；② 未给 `-c` 时补上 checkout 的 config 绝对路径——这是 ① 的后果（二进制的 `-c` 缺省是相对路径）。补 `-c` 的子命令硬编码六个（`start/check/status/dirs/diagnose/replay`），因为 `report` 没有 `-c`、盲注会直接失败；漏进列表的新子命令退化为"自己敲 `-c`"，不会拿到错配置。
...
| `Build` 里 `AnalyzeSessions` 失败即整个 `vmr report` 返回错误，`vmr-report.json`/`.md` 都不写出（只把错误信息写清楚，不做分文件容错） | 降级为 stderr 警告，聚合统计部分仍正常写出（老版 `cmdReport` 的行为）／按文件粒度容错，单个文件读失败只丢那个文件的数据 | 老版 `Build()`（确定性聚合）与 `AnalyzeSessions`（启发式分组）是分离的两步，后者失败不影响前者；现在的 `Build` 把两者合并成一趟——`AnalyzeSessions` 的分组坐标是第二遍扫描 join `ReqInfo` 的前提，每条记录的 token/usage 现在也只从 `ReqInfo.Usage` 取（不再像老版那样独立提取一次），拆不开了。已核实 `AnalyzeSessions` 唯一的失败面是**文件级 I/O**（`OpenLogFile` 打开失败 / `ForEachLine` 扫描中途出错——单行 JSON 解析失败两遍代码都只是跳过计数，不会导致整体失败），而 `Build` 自己的第二遍扫描读的是同一批文件、同一套原语，两者失败根因基本重合，现实中触发场景几乎只有一种：`vmr start` 常驻进程的 housekeeping 轮转扫描（`internal/audit/housekeep.go`）与 `vmr report` 并发读同一份日志时的竞态窗口。按文件粒度容错能更精细地扛住这类竞态，但要动 `AnalyzeSessions`/`Build` 两处文件循环 + 想清楚"哪些文件被跳过"如何在 `vmr-report.md`/`.json` 里可见（不能悄悄产出不完整报告）——评估后判定为这条窄竞态换不来这个复杂度，**选择只把错误信息写清楚**（点明大概率是 housekeeping 竞态、建议重跑），保持"失败就该显眼报错，而不是悄悄产出缺数据的报告"这条更高优先级的原则 |
...
* **报表增强**：成本核算已落地（§9.4 的 `-pricing pricing.yaml`/自动加载 `./pricing.yaml` + §2 成本估算，价目 key 用 provider+model，不含协议——同一账号/模型不管走哪个协议面成本都一样）。图表/HTML Dashboard（以 report JSON 为数据源二次开发）仍未设计。
