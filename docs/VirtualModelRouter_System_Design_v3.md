...
internal/report             `vmr report`：审计日志聚合统计（透明读取明文/.zst）
                            ...
                            ├─ detail.go/render.go      WriteDetails：逐请求 Markdown+JSON 详单
                            ├─ aggregate.go/metrics.go  桶聚合 + 派生指标（fresh/cache_efficiency/真 p50p95/成本估算）
                            ├─ aggregate_render.go      §0-§8 Markdown 渲染 + mermaid 图表
                            ├─ requests.go              vmr-requests.md：Chat User → Session → Task → Turn
                            └─ pricing.go                定价 sidecar（可选，-pricing）
...
* **两遍读取**：第一遍 `AnalyzeSessions` 做会话/任务分组（含 compaction 链路、工作负载标签、工具签名——791 行的启发式核心）；第二遍 `Build` 自己流式重新扫一遍同一批文件，按 `path:line` 用 `sess.Lookup` 把每条 record 接到它的 `ReqInfo`（分组坐标、usage、工具调用等），同时重算 `ReqInfo` 里没导出的原始量（bytes、tool 声明字节数、serving endpoint、错误类）。
...
* **JSON 输出**（`meta.format` = **10**，`internal/report.Format`）：`meta`（格式号、生成时间、输入清单、记录数、坏行数、时段、慢请求阈值、百分位方法说明）；`overall`/`by_model[]`/`by_date[]` 每条含 A（请求/结果分布）+ B（token 经济，含上面的派生字段）+ C（延迟，真 p50/p95/max，`stream_ms` 独立 p50/p95）+ D（bytes/messages/role_chars）+ 可选 H（`cost_estimate`）；`endpoints[]`/`endpoints_all[]`（G 族：attempts/ok/failed/availability/error_rate/error_classes + B/C + 可选 H）；`hours[]`/`hours_of_day[]`；`by_client[]`；`workloads[]`（含 `tool_call_rate`）；`sessions[]`（含 `context_growth` = 末轮/首轮 `tokens_in`、`compaction_chain`）；`tools[]`（`schema_bytes_shipped`/`distinct_called`/`declare_utilization`/`schema_waste_bytes` 派生字段）；`efficiency[]`（§7 自动发现表）。请求级明细**不在**这份 JSON 里（保持"聚合专用"），单独落 `vmr-requests.jsonl`。
...
* **逐请求详单**（`report.WriteDetails`，`internal/report/detail.go` + `render.go`，默认开启，`-details=false` 关闭；`vmr report` 唯一的详单产出路径）：每条审计记录导出一个 Markdown 文件**及同名 JSON 文件**（原始 record，供 jq/脚本查询）到 `{out}/details/`（**全部报表产物文件 0600、目录 0700**）。文件名 `{YYYYMMDD-HHMMSS.mmm}_{虚拟模型}_{真实模型}_{outcome[-错误类]}.md`（`.json` 同名），零填充时间戳开头，按名字排序即按时间排序；同毫秒冲突加数字后缀，重跑幂等覆盖。`WriteDetails` 只写这一批 per-request 文件，不产出任何索引——请求详单是 `WriteRequestsIndex` 自己的产物（见下）。
...
* **请求详单索引 `vmr-requests.md`（`WriteRequestsIndex`，`requests.go`）**：按 **Chat User（`client_key_tag`）→ Session → Task → Turn** 组织。每个调用方一段 `# Chat User: <key> · N 会话 N 任务 N 轮`；每个会话 `## sNN · <时间> · N 任务 N 轮`（非 interactive 类追加 `· <class>`）；每个任务 `### tNN · <时间> · N 轮`，紧跟一行首条消息的引用块，然后是轮次表（`轮 / 时间 / msgs / finish / dur / ttft / fresh/cached/out / cache-eff⭐ / 文件`）。**全文时间戳统一转 UTC+8**（`time.FixedZone`，不依赖系统 tzdata，不管源记录自带什么 offset）。**单发定时脚手架**（heartbeat/dream_diary，恰好 1 请求）拆成独立的顶层 `# 定时任务 · <class> 单发会话 × N` 区块（一张扁平表，覆盖每次触发），不挂在任何 Chat User 下；轮次数 >1 的定时会话（真正的多步 cron 任务）仍作为普通会话卡片留在其调用方名下。文末 `# 全部请求（时间序）` 覆盖每条记录，不分组。会话分组以 `(session_id, client_key)` 为键，而不是单纯 session_id——不同调用方各自的"未分组/被拒"请求（`session_id` 均为空）不会因为同用空 ID 而被合并显示到同一个调用方名下。
* **按调用方（`client_key_tag`）分组导出**（`config.APIKeys` + `audit.KeyTag` + `RequestRow.ClientKey`）：`WriteRequestsJSONL`/`WriteRequestsIndex` 各自检测出现过的 `ClientKey` 去重集合，每个非空 tag 都在原有产物旁多写一份同目录 sibling —— `vmr-requests-<tag>.jsonl` 与 `vmr-requests-<tag>.md`——同目录，`details/…` 相对链接不用调整。`vmr-report.md`/`.json` 与 `details/*.md`/`*.json` 本身不受影响，永远不分组不重复。不配置 `api_keys` 时这套逻辑整体不产生任何文件。
