...
### 1.4 [中] `internal/report/detail.go` 逼近行数预算（1047 / 1150）
...
### 1.6 [中] `vmr report` §2 成本表的 $ 列与 Token 列口径不一致
...
### 1.7 [低] `archtest` 函数长度豁免的键无法区分同文件重名方法
...
### 1.9 [低] 审计落盘的 `write` syscall 在全局锁内
