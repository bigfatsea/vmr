...
> 已移入仓库根目录 `/archived/`（未入库，`.gitignore` 忽略），仅作历史存档，**不再更新**；本文档才是当前状态的准绳。
>
...
- **`router.WriteJSON` 忽略 `Encode` 的返回错误**：响应体写一半断掉时服务端毫无痕迹。此时 `WriteHeader`
  已发出、无法改状态码，加一行日志需要给这个函数引入 logger 或改签名，成本大于收益，暂不动。
...
- **`core.go` 一个文件装七个不同领域概念**（路由类型、错误分类、额度类型、定价类型、token
