...
- **§1 分布**（2026-08-21 P14/P15 执行后重算，含外部独立审阅处置后的最终结果）：**高危 0 项**、中危 3 项、低危 15 项（另有 1 项已评估决定不做，登记备查，不计入分布）；0 + 3 + 15 = 18，加 `1.27` 共 19 条。`1.21`/`1.28`/`1.31`（P7）、`1.19`（P8，JSON 语言策略统一）、`1.30`/`1.33`/`1.34`（P9）、`1.39`（P11，死代码清理，判定同批大幅订正）、`1.41`/`1.37`（P12，渲染表示正确性）、`1.35`/`1.36`（P13，证据层体积纪律归位）、`1.38`/`1.42`/`1.43`（P14/P15，索引渲染范围一致化 + CLI 入口收敛）已修复并移入 §3；原 `1.24`（浮点 1 ULP）经重新分类后移入 §2.5——它是浮点算术的性质，不是待办。
...
- **现状**：分析半区（`report`/`story`/`ctxgraph`/`taskseg`/`chatmsg`/`reqdetail`/`i18n`）的代码体量已超过在线路由半区。
  ...
  **额度 P3（多 Limit/桶闸/Scope，2026-08-22）之后的路由半区四包**：`router` 2119 + `config` 1730 +
  `quota` 909 + `server` 735 ≈ **5,500**，`report`/`story` 也各自小幅增长（§2.5 额度对照表改成一
  Limit 一行）——量级变化不影响本条"分析半区体量更大"的结论，不逐条更新比例，仅记录这次改动
  的落点没有打破既有的相对体量关系。
...
> `1.38`/`1.42`/`1.43` 已由 P14/P15 修复（见 §3 第 39–41 项），均已移出本表——`1.37` 的修法范围
> 在 P12 执行期间、`1.43` 的受限项清单与披露路径在 P14 执行期间，均经外部独立审阅
> （gemini-3.7-flash）指出边界有误后当场扩大并一并修复，未留下需要单独登记的剩余项。
...
- **中 ROI（3 条，看触发；`1.28` 已由 P7 修复移入 §3，`1.39` 已由 P11 清理移入 §3，
