...
- **稳定性与安全性**：无数据丢失、凭证泄漏或服务阻断级别的缺陷；单机生产环境可稳定运行。唯一需要挂号的例外是 §1.16 那条竞态——它确属未定义行为，但后果被限制在**异常路径**（客户端已断开或上游已静默）的审计元数据上，未观测到影响转发正确性。
- **自动化基线**：`go test ./...` 与 `go test -race ./...` 全绿；`internal/archtest` 强制导入单向边界、文件行数预算、函数长度预算与文档引用完整性。
- **§1 分布**：中危 7 项、低危 10 项，无高危项。
...
### 1.16 [中] `copyFlush` 提前返回后，reader goroutine 仍会再读一次 body

- **现状**：`copyFlush` 在两条路径上先于 reader goroutine 返回——idle 超时，以及向客户端写入失败（客户端 Ctrl-C / 超时）。正常路径下每次 `body.Read` 的结果都经 channel 交给主循环，天然建立 happens-before；但 reader 投递完最后一个 chunk 后（`err == nil`）会继续循环、**再发起一次 `body.Read`**，而这次 Read 对流内部状态的写入不再经过 channel。而 `forwardSuccess` 紧接着就读同一个 `respnorm` 流对象：`Applied()` / `RawPreStrip()` / `ObservedModel()` 三个 getter 读的字段**没有任何同步保护**（`qmu` 只保护配额四字段）。
- **证据**：把 body 换成每次 `Read` 都写外部变量的 reader、其余形态与真实调用一致，`-race` 直接报 `DATA RACE`（写方栈帧为 `transport.go` 的 `copyFlush.func1`）。用真实 `respnorm` 流则难以稳定命中——`applied` 的 append 带去重、`observedModel` 只在首次见到 model 字段时写，需要精确控制「最后一次 Read 恰好写到这三个字段之一」。**窗口由 `copyFlush` 的结构提供，是否命中取决于最后一次 Read 的内容**；`-race` 全绿是因为现有测试没覆盖这个交错，不是窗口不存在。
- **影响**：常见后果是审计记录的 `norm` 字段少一项或多一项（该次转发本就已失败）；较少见的是 `rawPreStrip` 的 slice header 撕裂读；理论上 string/slice header 撕裂可越界 panic。属未定义行为，不能靠「没见过」排除。触发场景（客户端断开）在 Agent 工作流下是常态，与 §1.4 同一批场景——那条讲语义标注，这条讲内存安全。
- **可能方案**：① 给 reader goroutine 加退出信号，`copyFlush` 在提前返回的两条路径上先关 body 解阻塞、再等它退出（需把 body 关闭时机从 `forwardSuccess` 的 `defer` 前移，并确认不重复关闭）；② 退而求其次，给那三个字段补锁，把未定义行为降级成与 `qmu` 现有处理一致的良性末段缺失。
- **已落地的部分**：本条目的登记，以及两处失效注释的修正——`respnorm` 的 `qmu` 注释此前指向本文件一个并不存在的条目，且把「良性少计」这个界说成了适用于全部字段（实际只对它自己加锁的那四个成立）；`copyFlush` 的 doc comment 此前完全没提「返回不代表 reader 已停止触碰 body」。两处均为纯注释，零行为变更。
- **为什么待定**：剩下的**代码**部分。方案 ① 是**唯一碰转发热路径**的改动，关闭时机处理不当会引入「提前关闭丢末段数据」或「重复关闭」这类新的正确性 bug；主要工作量也不在改代码，而在写一个能稳定复现该交错的测试。方案 ② 风险低但只解决一半。
...
- **`respnorm` 的 usage sniffing 不外移为 `router` 侧装饰器**：装饰器要在转发热路径上每 chunk 多付一次接口调用与边界检查；当前实现搭 `ingest` 已有的 per-chunk 循环，零额外开销。理由写在 `internal/respnorm` 的包注释末尾。注意这条只讲**位置**——sniffing 累加器与审计字段之间的同步问题是另一回事，见 §1 的 `copyFlush` 提前返回条目。
...
| 1.16 | `copyFlush` 提前返回后 reader 仍会再读一次 body | 中 | 中 | 中高 | **中** | §1 里唯一一条**内存安全**问题，且触发场景（客户端断开）在 Agent 场景下常态化。但方案 ① 是唯一碰转发热路径的改动，主要工作量在写一个能稳定复现该交错的测试。**可先做零风险的一半**（本条目登记 + 修正 `qmu` 注释里那句失效引用，已完成），热路径改动单独排批；短期不动热路径时方案 ② 是可接受的中间态 |
| 1.4 | 客户端断开与成功在审计日志中不可区分 | 中 | 中 | 中高 | **中** | 与 1.16 并列价值最高——它是**数据正确性**，不是整洁度：断开请求被计入成功，其 token/耗时分布系统性偏移。但要动 `audit.Attempt`、两侧同步、并决定报表呈现口径。加 `omitempty` 布尔不会破坏旧日志读取，「格式版本升级」的顾虑偏重。**Agent 场景取消频繁时优先级跳升** |
...
- **中 ROI（4 条，看触发）**：`1.16` 是唯一一条内存安全问题，但要动转发热路径，零风险的一半已先行落地；
  `1.4` 价值同样高但要动审计契约；`1.12` 成本比看起来低但价值也低；`1.13` 按产品路线排。
- **低 ROI（11 条，等触发条件）**：其中 `1.1`/`1.2`/`1.3`/`1.10`/`1.17` 的共同点是**收益未经测量**——它们不是"不值得做"，
  是"还不知道值不值得"，而先做优化再测量正是这个项目一贯拒绝的顺序。`1.5`/`1.6`/`1.8` 则是守卫或真实反馈会
  自己举手的事。

**关于这张表本身的一个观察**：17 条里只有 2 条是高 ROI，且都是小改动；没有一条是"高价值但一直没做"。
这说明这份清单已经被治理得比较干净——**剩下的绝大多数是真正该等的，不是积压的**。
如果哪天这张表里出现了「价值高 + 成本低 + 却还在等」的条目，那才是需要解释的异常。

**一个补充判据**：`1.16` 是这张表里第一条**从源码注释里捞出来、而不是从代码里读出来**的条目——
