...
### 1.53 [低] LLM `goal_drift` 可能把漂移锚点定位到 Step 1
...
- **为什么待定**：仅 `-llm-addr` 时触发，无任何 GTM/规则层工件依赖 LLM 解读层；属 §1.18 未完成黄金样本校准的一个具体表现，随那批一并处理。2026-08-30 增长打磨批次评估后明确剔除出第一梯队。

### 1.54 [低，暂不做] `/help.html` 页内发测试请求
