...
> **覆盖范围的一处如实声明（2026-08-23，V4 设计文档全面 review 新登记 `1.45` 后更新）**：本表覆盖 §1 全部待评估条目（共 16 条，无遗漏）。`1.22` 已给出量化"不做"结论（本表一行）；`1.18` 正常列出；`1.29`（暂不做）与 `1.40`（单个消费方）已给出明确重估触发条件；原 `1.23` 已并入 `1.1`；`1.21`/`1.28`/`1.31`（P7）、`1.19`（P8）、`1.30`/`1.33`/`1.34`（P9）、`1.39`（P11）、`1.41`/`1.37`（P12）、`1.35`/`1.36`（P13）、`1.38`/`1.42`/`1.43`（P14/P15）已修复并移入 §3；原 `1.24`（浮点 1 ULP）、原 `1.5`（文件行数预算线）、原 `1.15`（可维护性与架构复杂度原则）与原 `1.27`（缓存孤儿分片不设 GC）重新归类为 §2 的工程惯例与架构哲学，均已移出本表。
