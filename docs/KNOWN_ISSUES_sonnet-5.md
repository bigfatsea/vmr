...
- **§1 分布**（2026-08-21 P11 执行后重算）：**高危 3 项**（`1.35`/`1.36` 同属"证据层体积纪律"，`1.41` 是"渲染表示正确性"且是前两项的硬前置）、中危 7 项（含 `[中低]` 1 项）、低危 15 项（另有 1 项已评估决定不做，登记备查，不计入分布）；3 + 7 + 15 = 25，加 `1.27` 共 26 条。`1.21`/`1.28`/`1.31`（P7）、`1.19`（P8，JSON 语言策略统一）、`1.30`/`1.33`/`1.34`（P9）、`1.39`（P11，死代码清理，判定同批大幅订正）已修复并移入 §3；`1.32` 的闭环判定已在 2026-08-21 全面复查中**撤回**（见 §3 第 31 项的订正与新登记的 `1.35`）；原 `1.24`（浮点 1 ULP）经重新分类后移入 §2.5——它是浮点算术的性质，不是待办。
...
> `1.35`/`1.36`/`1.37`/`1.38`/`1.41`/`1.42`/`1.43` 各有一行（见下）；
...
- **高 ROI（3 条，2026-08-21 全面复查 + 三份独立审阅核实后更新）**：`1.41`/`1.35`/`1.36`——
  ...
  **更普适的那条教训是：验收对象要是"那条纪律现在还成立吗"，而不是"这次改动做了什么"。**
  ...
  的任务，也是唯一一条至今没有退化的纪律。
- **中 ROI（7 条，看触发；`1.28` 已由 P7 修复移入 §3，`1.39` 已由 P11 清理移入 §3）**：
  `1.37`（人读产物转义，已有真实反例，
