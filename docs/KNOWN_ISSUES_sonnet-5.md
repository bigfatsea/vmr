...
- **§1 分布**（2026-08-23 全面复核后重算，含 V4 设计文档全面 review 新登记的 `1.45`，以及同日 status 端点/报告/看板 review 新登记 `1.47`；原 `1.46` 经评审确认"版本必须匹配、直接报错"后移入 §2）：**高危 0 项**、中危 2 项、低危 15 项，合计 **17 条**。`1.21`/`1.28`/`1.31`（P7）、`1.19`（P8，JSON 语言策略统一）、`1.30`/`1.33`/`1.34`（P9）、`1.39`（P11，死代码清理，判定同批大幅订正）、`1.41`/`1.37`（P12，渲染表示正确性）、`1.35`/`1.36`（P13，证据层体积纪律归位）、`1.38`/`1.42`/`1.43`（P14/P15，索引渲染范围一致化 + CLI 入口收敛）已修复并移入 §3；原 `1.24`（浮点 1 ULP）、原 `1.5`（文件行数预算线）、原 `1.15`（可维护性与架构复杂度原则）与原 `1.27`（解析缓存孤儿分片不设 GC）经重新分类后移入 §2——它们分别是浮点算术性质、工程提醒机制、架构治理哲学与持久化取舍，均非待办缺陷；原 `1.23` 已并入 `1.1` 消除重复；新登记 `1.45`（额度状态文件孤儿 `limitKey` 不清理）——已在 Quota.md 记录多时、本轮 review 核实为真实待办后同步补登进本清单。
...
- **现状**：`story_report_architecture_opus-5.md` 的"建议"一节，建议 2 的原话是"`chatmsg` 的配对逻辑加归一化回退"。实际实现
...
- **触发条件**：出现第二个需要按 id 配对工具结果的消费方（`report` 侧的工具形态章节、或
  `story_report_architecture_opus-5.md`"面向未来：如果这套工具将来要变成一个 Web 服务"一节描述
  的只读服务）。到那时应把归一化下沉到 `chatmsg`，而不是再写第三份。
- **登记来源**：2026-08-21 全面复查，`story_report_full_review_opus-5.md` 的 R1 条目。

### 1.45 [低] 额度状态文件里被替换的 `limitKey` 从不清理

- **现状**：`vmr-quota.json` 由 `quota.Registry` 整份读、整份写，`Registry.Load` 不做任何 key
  过滤——改一次某个 Limit 的 `every`/`metric`/`models`，旧 `limitKey` 对应的桶会永久留在文件里，
  且没有时间戳可以判断它有多旧。详细现状、处置方案与为什么现在不适合继续拖延，见
  `docs/VirtualModelRouter_Design_v4_Quota.md`"现状与后续计划"一节第 ⑥ 项。
- **为什么现在登记**：该文档原先把这条留给"P3 一起做"，但 P3（多 Limit + Scope）已于 2026-08-22
  交付，触发条件已经成立——一个 provider 现在可以同时挂好几条 Limit，改一条窗口的配置就会立刻
  产生孤儿桶，风险窗口比单 Limit 时代明显放大，不再适合继续待定在设计文档里而不登记进这份清单。
- **可能方案**：清理只能由路由半区做（它是唯一写者），在 `Registry.Load` 之后、首次 `Flush`
  之前，用当前配置解析出的合法 key 集合过滤一次，约 10 行改动。
- **登记来源**：2026-08-23 全面复查（本轮 V4 设计文档 review），核实为已在 Quota.md 记录、但未
  同步登记进本清单的真实待办。

### 1.47 [低] `auditBlock` 在 server 侧重建审计日志文件名，与 `internal/audit` 的命名知识重复

- **现状**：`internal/server/admin.go` 的 `auditBlock` 用 `"vmr-audit-"+time.Now().Format("2006-01-02")+".jsonl"`
  重建"今日活跃文件"路径来 `os.Stat` 大小，与 `internal/audit.Logger.Path()` 的命名逻辑重复（两份
  都用本地时区、目前一致）。audit 改名/改格式时，这里会静默失效（报告 0 字节而不是报错）。
- **处置**：留待后续 `internal/audit` 重构时合并（例如导出 `TodayPath()`/大小访问器），不单独立项。
  这是 3 行知识重复、风险为"改名后静默归零"的低危项，随下次 audit 改动顺手收敛。
- **登记来源**：2026-08-23 status 端点/报告/看板三文档全量 review，经评审确认留待重构。
...
> **覆盖范围的一处如实声明（2026-08-23，V4 设计文档全面 review 新登记 `1.45`、status 端点/报告/看板 review 新登记 `1.47` 后更新）**：本表覆盖 §1 全部待评估条目（共 17 条，无遗漏）。`1.22` 已给出量化"不做"结论（本表一行）；`1.18` 正常列出；`1.29`（暂不做）与 `1.40`（单个消费方）已给出明确重估触发条件；原 `1.23` 已并入 `1.1`；原 `1.46`（版本错配）经评审确认"CLI 与 Server 版本必须匹配、直接报错"后移入 §2.2，不再列入本表；`1.21`/`1.28`/`1.31`（P7）、`1.19`（P8）、`1.30`/`1.33`/`1.34`（P9）、`1.39`（P11）、`1.41`/`1.37`（P12）、`1.35`/`1.36`（P13）、`1.38`/`1.42`/`1.43`（P14/P15）已修复并移入 §3；原 `1.24`（浮点 1 ULP）、原 `1.5`（文件行数预算线）、原 `1.15`（可维护性与架构复杂度原则）与原 `1.27`（缓存孤儿分片不设 GC）重新归类为 §2 的工程惯例与架构哲学，均已移出本表。
...
| 1.45 | 额度状态文件孤儿 `limitKey` 不清理 | 低 | 低 | 中 | **中** | 改动约 10 行，落点在 `Registry.Load`→首次 `Flush` 之间，路由半区自身无风险；价值在于消除离线读者（`vmr report` 额度对照表）面对的噪声源。P3（多 Limit）已交付，触发条件已经成立，不必等 P4 |
| 1.47 | `auditBlock` 重建审计文件名与 audit 包知识重复 | 低 | 低 | 低 | **低** | 3 行知识重复，风险为"audit 改名后此处静默归零"；随下次 `internal/audit` 重构合并（导出 `TodayPath()` 或大小访问器），不单独立项 |
...
- **中 ROI（4 条，看触发；`1.28` 已由 P7 修复移入 §3，`1.39` 已由 P11 清理移入 §3，
  `1.37` 已由 P12 修复移入 §3，`1.38`/`1.42`/`1.43` 已由 P14/P15 修复移入 §3）**：
  ...
  性误报信号，等黄金样本标注窗口再推进；`1.45` 是 P3 交付后新出现的触发条件已经成立的条目，成本
  低（约 10 行）且价值明确（消除离线读者的噪声源），只是尚未排进具体版本。原 `1.16`（并发竞态）、
...
- **明确不做（1 条）**：`1.22`——它自己提出的前置问题（Responses API 占比）已经用真实语料回答了，
  ...
  重开条件，后者只会被反复重新讨论。
- **低 ROI（9 条，等触发条件）**：其中 `1.2`/`1.3`/`1.7`/`1.10`/`1.17` 的共同点是**收益未经测量**——它们不是"不值得做"，
