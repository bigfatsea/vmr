...
- **`internal/story/mdlite.go` 只覆盖 `-compare -html` 的 LLM 解读段实际会用到的 Markdown 子集**（ATX 标题、段落、无序列表、GFM 竖线表格、`**粗体**`、`` `行内代码` ``——全部先转义）：`-compare` 的 LLM 提示词明确要求"结论句 + 候选根因表 + 三个三级小节"，围绕这个形状裁剪。有序列表（`1. `）与围栏代码块（```` ``` ````）落进段落分支，会被空格 join 成单行 `<p>`——排版难看但已转义、无注入、不丢字符。不引 CommonMark 解析器；真观察到 LLM 频繁输出编号列表再补一个十行的 `1. ` → `<ol>` 分支即可（同款镜像现有 `<ul>` 路径）。
...
47. **2026-08 DX / 内存 / E1 / E2 落地**：
    ...
    - E3（per-virtual-model 预算硬闸）本轮 **hold**（用户决定）。
...
48. **2026-08 评审第一梯队落地**：
    ...
    - **E10（`vmr share <id>` 一键分享命令）本轮不做**——判断题，仅在确会用它对外分享 Journey 时才值得；未确认前不立项。
