...
**Scope so far** (see `docs/TokenPlan_Quota_Routing_Design_opus-5.md`'s "现状与后续计划" section for the full current-status breakdown): exactly one `limits:` entry per provider, `metric: requests`, `metric: tokens`, or `metric: cost` (see [Pricing and cost-metric quota](#pricing-and-cost-metric-quota) below), tumbling (non-rolling) windows only. Multiple windows on one account, `rolling: true`, and a per-Limit `models:` scope remain a **load-time error** naming the field and saying it's planned for a later batch (P3). None of them are silently ignored; if you write one and vmr accepts the config anyway, that's a bug, not a feature.
