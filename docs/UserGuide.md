...
**`-html`** (single-match `-journey` only) writes an extra `{out}/stories/journey-<id>.html`: one self-contained file — inline CSS, one small inline script, zero external requests, theme-aware — with a sticky left timeline and a right-hand waterfall of Step cards. Same data as the Markdown, laid out for scrolling and sharing rather than diffing. Add **`-redact`** (needs `-html`) to swap every conversation body — instructions, messages, model responses, tool arguments and results — for a `‹text: N chars›` length placeholder; roles, token counts, tool names, timestamps, and every compaction/stitch marker stay. The un-redacted `.html` still carries full conversation text (0600, like the `.md`); the redacted one is the variant meant to leave the machine.
