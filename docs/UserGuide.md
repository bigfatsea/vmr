...
**The `include_usage` gap.** On `openai-completions`, a *streaming* response carries no usage block at all unless the client sent `stream_options: {include_usage: true}` in its request — and vmr never injects request fields (byte-faithful passthrough). So a `metric: tokens` or `metric: cost` account backed by an `openai-completions` endpoint is metered almost entirely by byte estimate for every streaming caller that omits the flag; `estimated_pct` sits near 100. `vmr check` warns about this the moment such a config loads, and `vmr status` / `vmr report` flag a near-total estimated share with the same explanation. Fixes: have the client send the option, or route that account through `anthropic-messages` / `openai-responses` (both always report usage), or accept the overestimate (the bias is conservative, so an account is deprioritized early, never overrun silently). Non-streaming callers are unaffected.
