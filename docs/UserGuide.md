...
**`-html`** (single-match `-journey` only) writes an extra `{out}/stories/journey-<id>.html`: a single-page **dashboard** — one self-contained file, inline CSS + a small `IntersectionObserver` script, zero external requests, theme-aware. Not a transcript: a verdict line (id, time span, outcome), the Task→Step structure as a compact one-row-per-Step timeline (model, tool chips, failover/finding badges, transition markers), a metrics grid with an inline SVG sparkline of per-step context tokens, and the Findings. Per-step conversation isn't inlined — each row links out to that Step's `details/*.md`. Add **`-redact`** (needs `-html`) for the variant meant to leave the machine: every conversation body becomes a `‹text: N chars›` length placeholder, the per-step detail links are dropped (`details/*.md` is 0600 and not shared), Findings keep only their code + Step anchor, and compaction entity names collapse to counts — structure, metrics, roles, token counts, tool names and timestamps stay. The un-redacted `.html` still carries full conversation text (0600, like the `.md`).
