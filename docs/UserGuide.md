...
| `vmr report [-o dir] [-pricing pricing.yaml] [-lang en\|zh] [-report-config report.yaml] <glob>` | audit logs (plain or `.zst`) → usage statistics + session/tool analysis + per-request features (`vmr-requests.json`) + detail files (`-details=false` to skip); adds the §2 cost-estimate section once a pricing sidecar is loaded — `-pricing` names one explicitly, or a `./pricing.yaml` in the current directory is auto-loaded when `-pricing` is omitted. Output language defaults to English; `-lang` or a `report.yaml`'s `language:` (see "Output language" above) switches to Chinese |
| `vmr story [-journey <id> \| -render-all \| -compare <id1,id2> \| -corpus] [-lang en\|zh] [-report-config report.yaml] <glob>` | reconstruct one agent task's full execution history into a readable Markdown narrative (see "Agent task narratives" below); no args lists candidate tasks with their ids, `-render-all` renders every one in a single batched pass, `-compare id1,id2` diffs two already-built tasks' behavior profiles (rule-derived facts, including a divergence-point detection), `-corpus` computes statistics across every candidate. `-llm-addr host:port -llm-model name [-llm-key KEY] [-llm-dry-run]` adds an optional LLM interpretation section on `-journey` or `-compare` (not `-render-all`/`-corpus`). `-lang`/`report.yaml` control output language exactly as for `vmr report` |
