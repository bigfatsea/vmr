...
| `GET /log.html` | browser console **Live & Log** page: a 1280px-aligned terminal and live-activity view on the shared console skeleton — opens `/log` itself with the shared stored key, non-wrapping raw terminal output, level chips (presentation-only heuristic classification of the plain-text stream), substring filter with hit highlighting, copy view, ⌘P pause, and an auto-scroll chip. Positioned below the terminal are the active **Live Requests** table and the collapsible **Recent Failures** table (errors within the last 24h, up to 100), kept up-to-date by an adaptive `/stats` poller (~2s active, 15s idle); the footer status line shows `N shown / M lines` |
