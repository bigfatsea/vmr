...
**Output language.** `vmr report`/`vmr story` render in English by default. Drop a `report.yaml` (`language: zh`) in the current directory to switch to Chinese, or pass `-lang en|zh` on either command to override it for one run — `-lang` always wins over `report.yaml`. `report.yaml` is its own small file, entirely separate from `config.yaml`: it holds no secrets, is optional, and is auto-loaded from the current directory the same way `pricing.yaml` is (`-report-config path` to point at a different one). This only changes what the Markdown documents say — `vmr-report.json`/`journey-*.json`/`compare-*.json` are unaffected: every narrative field in them (e.g. `efficiency[].finding`, `compare-*.json`'s `rows[].label`) stays English regardless of `-lang`, so a script parsing those files never has to account for which language a report was generated in.
