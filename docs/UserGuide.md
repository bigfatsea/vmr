...
A bidirectional security layer for agent traffic, off by default: omitting the whole `guard:` key (or not wiring it) keeps the request path a zero-overhead, byte-faithful passthrough. Full design, threat model, and the milestone/verification record: `docs/design/agent-guard-technical-spec-final-2.0.md`. The `config.example.yaml` guard block documents every knob; the notes below are the operational picture.
