...
| `GET /status.html` | browser console home (**Overview**): the former status and stats dashboards merged into one flat dark page — a vitals strip (concurrency / requests / tokens / global TTFT / endpoint health) with a one-line system status, quota budgets, virtual-model topology with a separate fallback-endpoints table (per-endpoint quota headroom and a 24h traffic-share column), live in-flight requests, recent failures with error-class filter chips, performance percentiles with last-10/last-100 whole-row window switching, and a traffic & usage section (SVG requests+tokens combo chart with error markers and hover breakdown, plus per-key usage tables) under a single 24h/3d/7d window. One refresh clock for the whole page (5 minutes, click the header countdown to refresh now, paused — visibly — while the tab is hidden); a header alert pill lists actionable states only (config issues, cooling-down endpoints, quota gates near exhaustion); the shared API-key flow covers every console page; zero external CDN dependencies |
| `GET /stats` | real-time and historical telemetry JSON payload: concurrency (`limit`/`in_flight`/`waiting`), in-flight request details (queued vs. running, caller, upstream endpoint, per-chunk streaming milestones, stall age), hourly (last 48h by default; `?range=24h\|3d\|7d` widens the tail, 7d cap deliberate) / daily (last 90d) rollups, per-provider rows with `last_10`/`last_100` window blocks (actual sample count, window token sums, TTFT and `toks` p50/p90 — `toks` is the one throughput caliber: four-component token sum over the whole request duration, stream and non-stream alike), an `overall` merged window block (global percentiles are computed server-side over all rings' sample union), and a `recent_errors` ring of the last 50 failed/canceled requests with their stamped `error_class` — grouped by independent `client_key_tag` and upstream `key_label` dimensions (auth-gated via `api_keys`). Token `in` is **fresh input** (cache-read/cache-write excluded — `in + cache_read + cache_write` is the upstream's total prompt count). Read-cached ~1s, so several dashboards polling at once cost one aggregation. Works with `-audit=false` (no request bodies are buffered in that mode) |
...
| `GET /log.html` | browser live-log page: a full-bleed dark terminal on the shared console skeleton — opens `/log` itself with the shared stored key, level chips (presentation-only heuristic classification of the plain-text stream), substring filter with hit highlighting, wrap toggle, copy view, ⌘P pause, auto-scroll that hands off to a "↓ N new" chip when you scroll up, and a dedicated disconnect banner with a reconnect button; the footer status line shows `N shown / M lines`. Local view actions (clear, filters) never touch the server-side buffer or the live stream |
