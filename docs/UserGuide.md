...
**Scope so far** (P1 + P2.1; see `docs/TokenPlan_Quota_P1_DevPlan_opus-5.md`/`docs/TokenPlan_Quota_P2_DevPlan_opus-5.md` for why): exactly one `limits:` entry per provider, `metric: requests`, `metric: tokens`, or `metric: cost` (P2.2 — see [Pricing and cost-metric quota](#pricing-and-cost-metric-quota) below), tumbling (non-rolling) windows only. Multiple windows on one account, `rolling: true`, and a per-Limit `models:` scope remain a **load-time error** naming the field and saying it's planned for a later batch (P3). None of them are silently ignored; if you write one and vmr accepts the config anyway, that's a bug, not a feature.
...
providers:
  - name: anthropic # naming the provider after its vendor helps auto-resolution — see below
    ...
    pricing:
      ...
      overrides:
        - {model: "*", discount: 0.25, date_from: "2026-06-08", date_to: "2026-08-08"} # a temporary promo, listed first (first-match-wins)
        - {model: my-model-x, in_fresh: 1.58, cache_read: 0.32, cache_write: 1.58, out: 9.54} # this account's actual negotiated rate for one model
...
**`providers[].pricing.overrides`** is a first-match-wins rule list: each entry is either `discount` (a fraction of whatever the layer below it resolves to — the standard table, or another override further down the list) or an explicit four-component rate (`in_fresh`/`cache_read`/`cache_write`/`out`, all four required together — a partial explicit rate is rejected, since "the rest are free" and "the rest are unspecified" are different claims and vmr won't guess which one you meant). Both forms accept `date_from`/`date_to` (`yyyy-MM-dd`) and/or `hour_from`/`hour_to` (`HH:MM`, a from>to pair wraps past midnight) for time-scoped promos and off-peak pricing.
...
**Migrating from the old `pricing.yaml` sidecar** (pre-P2.2, now removed — `-pricing` is no longer a recognized `vmr report` flag): each `rates:` entry becomes a `providers[].pricing.overrides` entry on the matching provider, with an explicit four-component rate (its `in_fresh_per_1m`/`cache_read_per_1m`/`cache_write_per_1m`/`out_per_1m` map directly to `in_fresh`/`cache_read`/`cache_write`/`out`) and its `date_range`/`hour_range` pairs becoming `date_from`/`date_to`/`hour_from`/`hour_to`. The old file's top-level `currency`/`exchange_rate`/`updated_at` become the new global `pricing:` block's `currency`/`exchange_rate` (`updated_at` has no equivalent — the standard table carries its own generation date instead). Many rows can simply be **deleted** once you check whether the standard table already covers that model at an acceptable price — the whole point of the built-in table is to make most of that file unnecessary.

`vmr report`'s $ estimates use the same two layers, resolved independently at report-generation time from whatever `config.yaml` is reachable via its `-c` flag (default `./config.yaml`) — no config.yaml in reach degrades gracefully to standard-list-price-only, same as before. Its `metric: cost` account resolution and `vmr report`'s own resolution deliberately do NOT require the same completeness — a `vmr report` run degrades a missing price to "no $ column for that row," not a hard error, since report's whole design philosophy is that a pricing gap must never cost you the rest of the report.

Full design, including everything staged for later batches (multi-window accounts, rolling windows, official usage-API calibration): `docs/TokenPlan_Quota_Routing_Design_opus-5.md`.
...
**No input files needed for the common case.** Both `vmr report` and `vmr story` accept zero positional arguments: omit the glob entirely and they resolve it themselves from `-c config.yaml`'s own `log_dir` (`<log_dir>/vmr-audit-*`, matching plain `.jsonl` and compressed `.jsonl.zst` alike) — loading the config only to read that one field. So reporting on the instance you're already running is just `./vmr report` (or `./vmr story`), full stop; the explicit `$(./vmr check -c config.yaml log)/vmr-audit-*.jsonl*` form above is for pointing at a *different* directory (another instance's logs, an archived set of files) or a custom glob.

`vmr report` aggregates tokens *and* bytes (bytes as the fallback when a provider omits usage).

#### Report sections

The Markdown groups into nine numbered sections, each answering one operator question:
...
- **§3 Reliability** — endpoint availability/error-rate and error-class breakdown, each split into one table per ingress protocol actually in use (openai first, anthropic second, any other alphabetically) since every protocol surface routes independently, plus an hourly error-count chart.
...
`vmr report`/`vmr story` render in English by default. Drop a `report.yaml` (`language: zh`) in the current directory to switch to Chinese, or pass `-lang en|zh` on either command to override it for one run — `-lang` always wins over `report.yaml`. `report.yaml` is its own small file, entirely separate from `config.yaml`: it's optional, and is auto-loaded from the current directory if present (`-report-config path` to point at a different one). It can hold a real secret (`llm_key`, see below), so like `config.yaml` it's `.gitignore`'d — `report.example.yaml` in the repo root is the committed template to copy from. This only changes what the Markdown documents say — `vmr-report.json`/`journey-*.json`/`compare-*.json` are unaffected: every narrative field in them (e.g. `efficiency[].finding`, `compare-*.json`'s `rows[].label`) stays English regardless of `-lang`, so a script parsing those files never has to account for which language a report was generated in.

`report.yaml` isn't just language: `-o`/`-details`/`-include-partial`/`-llm-addr`/`-llm-model`/`-llm-key`/`-llm-cache-dir` all have a matching default field there too, and an explicit same-named flag still wins — see `report.example.yaml` in the repo root for the full annotated list. `llm_key` can be plaintext (this file is `.gitignore`'d) or a `${VMR_LLM_KEY}`-style reference to an existing environment variable, whichever is more convenient; `llm_cache_dir` has no implicit default path — leave it (and `-llm-cache-dir`) unset anywhere and LLM interpretation results are never cached.
...
| `GET /admin/status` | process identity (pid/listen/version/config path/uptime), config-staleness verdict, reload outcome, per-endpoint health, Sticky Model registry size, concurrency metrics, and — for any provider with a `quota:` block — a `quota` section with live consumption (see [Quota-Aware Routing](#quota-aware-routing) above), including whether a background recovery probe currently has an endpoint's single-flight slot (loopback only) — `vmr status` below is the CLI front end for this same payload |
...
| `vmr report [-c config.yaml] [-o dir] [-lang en\|zh] [-report-config report.yaml] [glob...]` | audit logs (plain or `.zst`) → usage statistics + session/tool analysis + per-request features (`vmr-requests.json`) + an error/truncation index (`vmr-requests-failed.jsonl`/`.md`) + detail files (`-details=false` to skip); adds the §2 cost-estimate section whenever pricing data resolves anything — the built-in standard table always applies, `-c`'s config.yaml layers account-specific overrides on top if reachable (see [Cost estimate and pricing](#cost-estimate-and-pricing) above). `glob` is optional — omit it entirely to report on `-c config.yaml`'s own `log_dir`. Output language defaults to English; `-lang` or a `report.yaml`'s `language:` (see [Output language](#output-language) above) switches to Chinese |
| `vmr story [-journey <id\|id-prefix\|glob>[,...] \| -render-all \| -compare <id1,id2> \| -corpus] [-lang en\|zh] [-report-config report.yaml] [glob...]` | reconstruct one agent task's full execution history into a readable Markdown narrative (see [Agent task narratives](#agent-task-narratives-vmr-story) above); no args lists candidate tasks with their ids, `-journey` accepts a comma-separated list of ids/id-prefixes/shell-style globs (`*`/`?`/`[...]`) and renders every match — one match renders directly, more than one batches like `-render-all` (which renders every candidate in a single batched pass), `-compare id1,id2` diffs two already-built tasks' behavior profiles (rule-derived facts, including a divergence-point detection), `-corpus` computes statistics across every candidate. `-llm-addr host:port -llm-model name [-llm-key KEY] [-llm-dry-run]` adds an optional LLM interpretation section on a single-match `-journey` or on `-compare` (not `-render-all`/`-corpus`, nor a multi-match `-journey`). `-lang`/`report.yaml` control output language exactly as for `vmr report`; `glob` is optional the same way (see "No input files needed" above) |
...
| `vmr replay -provider NAME <audit.jsonl>` | rebuild and resend one request from an audit record through the exact same request-building path vmr itself uses — `-dry-run` to print without sending, `-record path` to save the replay as its own audit line, `-model`/`-protocol` to override what the record itself says, `-stream true\|false` to force streaming on/off, `-max-time` to cap the upstream wait. Pick the record with `-detail file` (a `vmr report` `details/*.json` file — no line-counting needed), `-ts <timestamp>` (matches either `vmr-requests.json`'s or the raw audit log's `ts` field), or `-line N` (default: the last one in the file) — the three are mutually exclusive |
