...
```bash
./vmr story "$(./vmr dirs -c config.yaml log)/vmr-audit-*.jsonl*"           # list candidate tasks
./vmr story -journey j-agent-20260716T152238-20260716T153122-42f908fa \
    "$(./vmr dirs -c config.yaml log)/vmr-audit-*.jsonl*"                   # render one, by id (a prefix is enough)
./vmr story -render-all "$(./vmr dirs -c config.yaml log)/vmr-audit-*.jsonl*"   # render every candidate in one pass
```
...
**Comparing two tasks**: `-compare-a <id> -compare-b <id>` diffs two tasks' behavior profiles directly (no separate `-journey` render needed first) and writes `compare-<idA>-vs-<idB>.md` + `.json` next to the individual task files. Each row shows both values and the relative change, with a rule-based ⚠️ flag on differences large enough to be worth a second look (a fixed threshold, not a judgment call) — useful for "did switching agent frameworks/prompts actually change how this task got done."
...
| `vmr story [-journey <id> \| -render-all \| -compare-a <id> -compare-b <id>] <glob>` | reconstruct one agent task's full execution history into a readable Markdown narrative (see "Agent task narratives" below); no args lists candidate tasks with their ids, `-render-all` renders every one in a single batched pass, `-compare-a`/`-compare-b` diff two already-built tasks' behavior profiles |
