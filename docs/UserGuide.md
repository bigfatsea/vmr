...
| `vmr report [-c config.yaml] [-o dir] [-lang en\|zh] [-currency CODE] [-report-config report.yaml] [glob...]` | audit logs (plain or `.zst`) → usage statistics + session/tool analysis + per-request features (`vmr-requests.json`) + an error/truncation index (`vmr-requests-failed.jsonl`/`.md`) + detail files (`-details=false` to skip); adds the §2 cost-estimate section whenever pricing data resolves anything — the built-in standard table always applies, `-c`'s config.yaml layers account-specific overrides on top if reachable (see [Cost estimate and pricing](#cost-estimate-and-pricing) above). `glob` is optional — omit it entirely to report on `-c config.yaml`'s own `log_dir`. Output language defaults to English; `-lang` or a `report.yaml`'s `language:` (see [Output language](#output-language) above) switches to Chinese. `-currency` picks the $ column's display currency, independent of whatever currency it was computed in (see [Cost estimate and pricing](#cost-estimate-and-pricing) above) |
