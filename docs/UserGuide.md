...
- **§1 Token economics** — cache-hit/fresh/cache-write/reasoning breakdown, per-model cache efficiency, per-role message-character/estimated-token share.
- **§2 Cost estimate** — only rendered once a pricing sidecar is loaded (see [Cost estimate and pricing.yaml](#cost-estimate-and-pricingyaml) below); one table each by model/endpoint/client, plus the exact pricing config used, embedded verbatim in a collapsed block so a report's $ figures stay traceable even after `pricing.yaml` later changes.
- **§3 Reliability** — endpoint availability/error-rate and error-class breakdown, each split into an openai/anthropic table since the two protocol surfaces route independently, plus an hourly error-count chart.
...
- **§5 Workload split** — by virtual model, by workload class (interactive vs. scheduled scaffolding), by endpoint, and by client (the latter two also carrying per-request input/output token percentiles), plus hourly and daily request-volume/input-token Mermaid charts.
...
`vmr report`/`vmr story` render in English by default. Drop a `report.yaml` (`language: zh`) in the current directory to switch to Chinese, or pass `-lang en|zh` on either command to override it for one run — `-lang` always wins over `report.yaml`. `report.yaml` is its own small file, entirely separate from `config.yaml`: it's optional, and is auto-loaded from the current directory the same way `pricing.yaml` is (`-report-config path` to point at a different one). It can hold a real secret (`llm_key`, see below), so like `config.yaml` it's `.gitignore`'d — `report.example.yaml` in the repo root is the committed template to copy from. This only changes what the Markdown documents say — `vmr-report.json`/`journey-*.json`/`compare-*.json` are unaffected: every narrative field in them (e.g. `efficiency[].finding`, `compare-*.json`'s `rows[].label`) stays English regardless of `-lang`, so a script parsing those files never has to account for which language a report was generated in.
...
| `GET /admin/status` | process identity (pid/listen/version/config path/uptime), config-staleness verdict, reload outcome, per-endpoint health, Sticky Model registry size, and concurrency metrics, including whether a background recovery probe currently has an endpoint's single-flight slot (loopback only) — `vmr status` below is the CLI front end for this same payload |
...
| `vmr report [-o dir] [-pricing pricing.yaml] [-lang en\|zh] [-report-config report.yaml] [glob...]` | audit logs (plain or `.zst`) → usage statistics + session/tool analysis + per-request features (`vmr-requests.json`) + an error/truncation index (`vmr-requests-failed.jsonl`/`.md`) + detail files (`-details=false` to skip); adds the §2 cost-estimate section once a pricing sidecar is loaded — `-pricing` names one explicitly, or a `./pricing.yaml` in the current directory is auto-loaded when `-pricing` is omitted. `glob` is optional — omit it entirely to report on `-c config.yaml`'s own `log_dir`. Output language defaults to English; `-lang` or a `report.yaml`'s `language:` (see [Output language](#output-language) above) switches to Chinese |
