...
| `vmr story [-journey <id\|id-prefix\|glob>[,...] \| -render-all \| -compare <id1,id2> \| -corpus] [-lang en\|zh] [-report-config report.yaml] [glob...]` | reconstruct one agent task's full execution history into a readable Markdown narrative (see [Agent task narratives](#agent-task-narratives-vmr-story) above); no args lists candidate tasks with their ids, `-journey` accepts a comma-separated list of ids/id-prefixes/shell-style globs (`*`/`?`/`[...]`) and renders every match — one match renders directly, more than one batches like `-render-all` (which renders every candidate in a single batched pass), `-compare id1,id2` diffs two already-built tasks' behavior profiles (rule-derived facts, including a divergence-point detection), `-corpus` computes statistics across every candidate. `-llm-addr host:port -llm-model name [-llm-key KEY] [-llm-dry-run]` adds an optional LLM interpretation section on a single-match `-journey` or on `-compare` (not `-render-all`/`-corpus`, nor a multi-match `-journey`). `-lang`/`report.yaml` control output language exactly as for `vmr report`; `glob` is optional the same way (see "No input files needed" above). Candidates in `vmr-stories.md` are grouped by category (`task`/`cron`/`heartbeat`/`subagent`, from title content markers) — `heartbeat`/`subagent` collapse into a `<details>` block by default so a real task list isn't drowned in scheduled polls; `vmr-stories.json` lists every candidate regardless. `vmr story -llm-addr`'s own analysis traffic is excluded from the candidate list by default (see `vmr analyze`'s row below for the shared exclusion mechanism) |
| `vmr analyze [-c config.yaml] [-o dir] [-details] [-include-partial] [-include-self-traffic] [-lang en\|zh] [-currency CODE] [-report-config report.yaml] [glob...]` | `vmr report` then `vmr story -render-all` in one call, sharing the same `-o` — the full navigable suite (`vmr-report.md` + `vmr-requests.md` + `stories/vmr-stories.md` + every rendered journey, cross-linked) from a single invocation, so the two commands' outputs never land in different places by accident. Flags are the shared subset of both commands' own flags; `vmr report`/`vmr story` are unchanged and still available standalone. `-include-self-traffic` disables the default exclusion of `vmr story -llm-addr`'s own analysis requests from both halves' totals — the identification rule is computed once (from `report.yaml`'s `llm_key`, the same tail-transform `api_keys` auth uses, plus an optional explicit `self_traffic_client_tags` list) and shared by both `vmr report` and `vmr story` |
