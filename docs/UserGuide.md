...
| `GET /stats` | real-time and historical telemetry JSON payload: concurrency (`limit`/`in_flight`/`waiting`), in-flight request details (queued vs. running, caller, upstream endpoint, per-chunk streaming milestones, stall age), hourly (last 48h) / daily (last 90d) rollups, and quantile distributions (p50/p90 for TTFT and TPS) grouped by independent `client_key_tag` and upstream `key_label` dimensions (auth-gated via `api_keys`). Token `in` is **fresh input** (cache-read/cache-write excluded — `in + cache_read + cache_write` is the upstream's total prompt count). Read-cached ~1s, so several dashboards polling at once cost one aggregation. Works with `-audit=false` (no request bodies are buffered in that mode) |
| `GET /stats.html` | browser live-stats dashboard: single-page dark UI with 1-second auto-polling, in-flight request monitoring with visual stall warning indicators, provider/model performance breakdown, and links to status, log, and help pages |
