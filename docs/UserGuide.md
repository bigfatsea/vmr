...
Whenever the report has tool data, `vmr analyze` also writes **`{out}/tool-waste.html`** — a standalone one-screen card built from the same §7 numbers: the share of shipped tool-schema bytes that went to tools never called this window, the total shipped / dead weight / estimated wasted tokens, and a per-shape table. Self-contained, carries only tool names and counts (no conversation content), 0600. Shareable as-is.
