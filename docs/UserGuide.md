...
- [Agent Guard](#agent-guard)
  - [Outbound: keeping credentials away from untrusted relays](#outbound-keeping-credentials-away-from-untrusted-relays)
  - [Inbound: offline forensics, online sanitization only](#inbound-offline-forensics-online-sanitization-only)
  - [Probing a relay before trusting it](#probing-a-relay-before-trusting-it)
...
`config.example.yaml` documents every field inline. Config edits hot-reload within seconds; a broken config is rejected and the running instance keeps its current one. Parsing is strict: an unknown or misspelled key (`max_concurency: 8`) is a load error, never a silently ignored no-op you believe is in effect.
...
`vmr analyze`'s macro half also understands agent workloads — offline and purely rule-based, no LLM involved:
...
`vmr analyze` renders in English by default. Drop a `report.yaml` (`language: zh`) in the current directory to switch to Chinese, or pass `-lang en|zh` to override it for one run — `-lang` always wins over `report.yaml` — including on `-render-only`, which now switches language freely too (pure re-render, never a full rerun — see above). `report.yaml` is its own small file, entirely separate from `config.yaml`: it's optional, and is auto-loaded from the current directory if present (`-report-config path` to point at a different one). It can hold a real secret (`llm_key`, see below), so like `config.yaml` it's `.gitignore`'d — `report.example.yaml` in the repo root is the committed template to copy from. This always changes the Markdown documents. Every JSON product is language-invariant now (R1): `macro/*.json`, `manifest.json`, `j-<id>.json`, and `compare-*.json` all stay the English baseline regardless of `-lang` — `efficiency[].finding`, `highlights[].text`, `j-<id>.json`'s `findings[]`, and `compare-*.json`'s `rows[].label` alike carry a `params` object (or, for findings, the metric's own stable `code`) with the raw values a script can use to build its own sentence in any language without re-running `vmr analyze`. The one deliberate exception: LLM-generated prose (`j-<id>.json`'s `llm_interpretation.text` and `llm_findings[]`, `compare-*.json`'s `llm_interpretation`/`llm_divergence`) is the model's own original-language output and is never re-derivable — those carry an `llm_lang` field recording which language they were actually generated in instead. `FindingCode`/`MetricCode`/`EvidenceAnchor` are the stable, language-independent fields a script should key off of for matching a row across languages, never the narrative text next to them. Want a copy in the other language too? `-render-only` redraws the same output tree in place, so copy the output directory first (or point a fresh `-o` at the same snapshot) and run `-render-only -lang <other>` against the copy — a cheap pure re-render, not a second full analysis, so keeping both languages around costs disk space and a few seconds, never a recomputation.
...
Every `vmr analyze` invocation (any mode, zoom or default suite) also refreshes three dashboard skeleton pages into the output root: `macro-dashboard.html`, `request-browser.html`, and `journey-viewer.html`. They carry zero business data: all rendering happens browser-side, each page fetching the relative-path JSON slices this run wrote (`manifest.json`, `macro/*.json`, `requests/`, `journeys/`) and rendering DOM + inline SVG from them — writing the JSON slices *is* delivering the dashboard. Each page starts by fetching `manifest.json` and comparing its `format` version against the one the page was built for: on mismatch it shows a warning banner and keeps rendering (additive schema changes usually render fine anyway), while a missing manifest gets a "this isn't a complete analyze output directory" hint instead. A page opens on one specific data file via a `#data=` URL hash parameter (e.g. `journey-viewer.html#data=journeys/details/j-<id>.json`); without one, it probes the corresponding index (`journeys/index.json`) and lists candidates instead. `journey-compare`, `benchmarks`, and `tool-waste` stay Markdown-only products now (`compares/*.md`, `journeys/benchmarks.md`, report §7) — the three deleted HTML pages had drifted out of sync with them without adding any capability the Markdown lacked. The pages need HTTP to fetch their data — `file://` cannot serve them. Three ways to get that HTTP: `vmr analyze -open` starts a one-shot local viewer for exactly this run's output (loopback-only, random port, no auth needed since the data is already sitting on your own disk, foreground until Ctrl-C, opens the default browser at `macro-dashboard.html` automatically) — the zero-config default for "just let me look at this"; any static file server pointed at the output directory works too; or vmr's own opt-in `/reports/` hosting (`analytics.serve: true` in `config.yaml`; default off, serving `analytics.serve_dir`, which defaults to `./reports` — the same default as `analyze -o`, so run analyze then start the router and it just works) — sized for `request-browser`'s long-running query workload rather than a one-off look, and the one of the three that requires `api_keys` and a Bearer key on every data request. Under that hosting the HTML pages themselves stay auth-exempt (zero business data), every data request (`.json`/`.jsonl`/`.md`) must carry the Bearer key, directory listing is disabled, and if `api_keys` isn't configured at all the data requests are refused (403) rather than served open — a report carrying full conversation bodies never serves itself without a key. A skeleton refresh failure only warns on stderr — it never blocks or corrupts the analysis itself, and the next `vmr analyze` run rewrites the pages from the running binary anyway.
...
A bidirectional security layer for agent traffic, off by default: omitting the whole `guard:` key (or not wiring it) keeps the request path a zero-overhead, byte-faithful passthrough. Full design, threat model, and the milestone/verification record: the Agent Guard design spec (archived out of the repository). The `config.example.yaml` guard block documents every knob; the notes below are the operational picture.

**What Agent Guard is not**: it is not a substitute for a reliable API provider, a reliable Coding Agent, or a reliable client-side sandbox — those are your first line of defense, and Agent Guard does not replace them. It solves two narrow, low-cost problems VMR is uniquely positioned to help with: an extra check on credentials leaving your machine (some agents aren't careful about what ends up in a request body), and after-the-fact visibility when something leaks anyway despite everything else being in place. It does not try to judge whether a tool call is safe to run — that decision belongs to your client's own approval gate and sandbox, which have far more context (the workspace, your intent, the ability to ask you) than a gateway looking at a JSON blob ever will.

**Outbound** (`guard.outbound.mode`) scans request bodies for high-confidence credential formats before they leave for an untrusted relay. Three modes, in increasing order of intervention: `off`, `audit_only` (record hits in the audit trail — the only mode calibrated purely from offline evidence, and the right starting point), and `block` (reject the request with HTTP 400). `trusted_providers` exempts a virtual model entirely when every candidate endpoint (fallbacks included) belongs to a trusted provider.

**Inbound** (`guard.inbound.sanitize_invisible_runes`) only strips invisible/steganographic Unicode characters from responses before relaying them (emoji-critical characters are preserved) — that's the whole inbound feature. It never inspects tool calls, never blocks a response, and never changes the HTTP status code: sanitization either changes a few bytes or it doesn't run at all, Fail-Open on any error. An earlier version of Agent Guard also gated tool calls online (a two-level literal/regex check against a high-risk-command library, dropping and circuit-breaking flagged ones) — that was removed after a first-principles review: the judgment belongs at the client, where there's more context to make it with, not at a gateway that only sees a JSON blob. The detection logic itself (which tool calls look dangerous, which paths look sensitive, whether a credential got echoed back) lives on in the offline side below — you still get to see it, just not have it block anything in real time.

### Outbound: keeping credentials away from untrusted relays

`audit_only` is where to start: `vmr analyze`'s guard section then reports which rules hit, how often, and — through provider exposure attribution — which upstreams actually saw credential-shaped content. Move up the mode ladder only after reviewing that evidence for your own traffic; `block` rejects requests, and a false positive is a broken workflow. Tier 2 rules (looser shapes like `sk-`-prefixed strings) never trigger blocking — only Tier 1's five-anchor-verified formats do. (An earlier `mode: replace` — pseudonymizing hits and restoring them in the response — was removed: `block` is strictly safer, and refusing a request that carries a credential is the right outcome for an accidental paste.)

### Inbound: offline forensics, online sanitization only

Real-time, Agent Guard only strips invisible characters — nothing else. The deeper forensics (dangerous tool calls, sensitive file paths, credential echoes, Unicode steganography by category) all run offline instead, in `vmr analyze`'s guard section, over your audit log: it reports what a relay actually sent, going back as far as your retention window allows, with no risk of a false positive ever interrupting a real response. Treat a finding there as a reason to stop using a relay or rotate a credential, not as something that was blocked in the moment — nothing in the inbound path blocks anything anymore.

### Probing a relay before trusting it

`vmr diagnose -guard` actively probes an upstream with five canary payloads (tool-call tampering, silent context truncation, reasoning stripping, usage inflation, invisible-rune injection) and reports per-probe verdicts. The 100%-detection / 0-false-positive hard metric was established against mock relays; real relays with unusual protocol shapes can produce false verdicts — treat surprises as a reason to investigate, not an automated conviction. Zero user-traffic intrusion: probes are synthetic requests you choose to send.
...
| `GET /status.html` | browser console home (**Overview**): a vitals strip (concurrency / requests / tokens / endpoint health) with a one-line system status, the active **Live Requests** table (Seq first, then State / Elapsed / Caller / Model / Provider : Model / Attempts / TTFT with >5s stall detection / Tok in / Tok out; ended rows gray out), performance percentiles computed over the global last 300 requests with last-10/30/100/300 window filtering and sorted by request count descending, a traffic & usage section (single four-layer stacked SVG token bars for fresh in / cache write / cache read / tok out, requests line and hover breakdown, plus per-provider/model and per-caller usage tables) under a 12h/24h/3d/7d window, and a collapsible **Recent Failures** table (errors within the last 24h, up to 100) at the bottom. The whole page refreshes on a single 5-minute countdown clock (click to refresh now, pauses visibly while hidden); Live Requests, Recent Failures and concurrency vitals poll adaptively on their own fast cadence (~2s while requests are active or queued, 15s idle, paused while hidden). A header alert pill lists actionable states only (config issues, cooling-down endpoints, quota gates near exhaustion); the shared API-key flow covers every console page; zero external CDN dependencies |
| `GET /models.html` | browser console **Models** page: quota budgets (amount/used/progress/headroom/24h share/reset time per account) and virtual-model → endpoint topology grouped into per-protocol tables (priority tier, provider:model, health, account headroom, 24h traffic share, context window/capabilities) with unified fallback endpoints. Relatively static configuration/topology facts, so the whole page just rides the same 5-minute countdown clock as Overview — no adaptive polling here |
| `GET /stats` | real-time and historical telemetry JSON payload: concurrency (`limit`/`in_flight`/`waiting`), in-flight request details (queued vs. running, caller, upstream endpoint, per-chunk streaming milestones, stall age), hourly (last 48h by default; `?range=24h\|3d\|7d` widens the tail, 7d cap deliberate) / daily rollups, per-provider cumulative metrics (`by_provider_model[]`), a global cross-endpoint ring of the last 300 completed requests (`recent_requests[]`, sliced into last 10 / 30 / 100 / 300 dynamic windows and sorted descending on the console), an `overall` window block (global percentiles and throughput computed over the global ring), and a `recent_errors` ring of the last 100 failed/canceled requests within 24h with their stamped `error_class` — grouped by independent `client_key_tag` and upstream `key_label` dimensions (auth-gated via `api_keys`). Token `in` is **fresh input** (cache-read/cache-write excluded — `in + cache_read + cache_write` is the upstream's total prompt count). Concurrency and in-flight are computed fresh on every read; the historical aggregate is read-cached for a few seconds (keyed per range), so several dashboards polling at once cost one aggregation. The in-memory rollup is a ~7-day sliding window (7 whole calendar days plus today) — older rows stay archived in the append-only rollup file but are not loaded or served, so `by_*` totals are a rolling ~7-day view and `/stats` cost does not grow with deployment age (deeper history is `vmr analyze`'s job). Works with `-audit=false` (no request bodies are buffered in that mode) |
