...
| `GET /log.html` | browser live-log page: a full-bleed dark terminal on the shared console skeleton — opens `/log` itself with the shared stored key, level chips (presentation-only heuristic classification of the plain-text stream), substring filter with hit highlighting, copy view, ⌘P pause, auto-scroll that hands off to a "↓ N new" chip when you scroll up, and a dedicated disconnect banner with a reconnect button; the footer status line shows `N shown / M lines`. Local view actions (clear, filters) never touch the server-side buffer or the live stream |
