...
| `GET /log.html` | browser console **Live & Log** page: a 1280px-aligned live-activity and terminal view on the shared console skeleton. Above the terminal sit the active **Live Requests** table and the collapsible **Recent Failures** table (errors within the last 24h, up to 100), kept up-to-date by an adaptive `/stats` poller (~2s active, 15s idle); below them the toolbar (level chips, substring filter with hit highlighting, ⌘P pause, copy view) and the word-wrapped terminal itself — opens `/log` with the shared stored key, auto-scroll hands off to a "↓ N new" chip, and a disconnect banner carries the reconnect button; the footer status line shows `N shown / M lines` |
