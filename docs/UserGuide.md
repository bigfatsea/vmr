...
`vmr report` aggregates tokens *and* bytes (bytes as the fallback when a provider omits usage), and organizes the Markdown around nine numbered sections, each answering one operator question: `§0` summary (headline numbers + up to 3 auto-generated highlights — low cache efficiency, wasted tool schemas, a misbehaving endpoint), `§1` token economics (cache-hit/fresh/cache-write/reasoning breakdown, per-model cache efficiency), `§2` cost estimate (only rendered when `-pricing pricing.yaml` is given — CNY by default, one table each by model/endpoint/client), `§3` reliability (outcome distribution, per-endpoint availability/error-rate, an hourly error-count chart), `§4` latency & throughput (ttft/duration/stream percentiles, each with its sample size and a `⚠️low-n` flag under n=20), `§5` workload split (by virtual model, by workload class — interactive vs. scheduled scaffolding — and by client, plus hourly request-volume/p95-duration charts), `§6` sessions & tasks, `§7` efficiency & waste (auto findings, plus the full used/never-called tool breakdown per declared-tool shape), `§8` a pointer to the request index. Every table stays to a handful of columns; percentiles are true per-bucket values — each bucket keeps its own raw samples and computes p50/p95 directly during the single pass, never derived by re-merging other buckets' already-finalized percentiles (there'd be nothing left to recompute from). An `⭐` marks any column that's a derived/estimated metric rather than a raw upstream value. Hourly activity and the hourly error count render as Mermaid `xychart-beta` charts. Run progress goes to stdout as a file-level line per processed file (`[i/N] <path>  done: M records, K parse errors (Ts)`). The JSON (`vmr-report.json`) is the data source for any dashboarding you want to build on top — anything the Markdown only summarizes or truncates (e.g. Top-5 tool shapes) is present there in full.
...
| `vmr report [-o dir] [-pricing pricing.yaml] <glob>` | audit logs (plain or `.zst`) → usage statistics + session/tool analysis + per-request features (`vmr-requests.jsonl`) + detail files (`-details=false` to skip); `-pricing` adds the §2 cost-estimate section |
