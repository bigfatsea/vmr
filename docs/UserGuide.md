...
| `GET /status` | process identity & execution context (pid/listen/version/cwd/executable/uptime), config freshness (mtime/stale/reload/issues), concurrency throttles, system resources (memory/goroutines/disk free space), live traffic telemetry (requests/tokens/sticky), per-endpoint health, and live quota metrics (auth-gated via `api_keys`) — `vmr status` below is the CLI front end for this payload |
| `GET /status.html` | browser visual dashboard: client-side rendered (CSR) single-page dark UI for `/status` data with live auto-refresh (5s/15s/30s/60s), visual health/cooldown countdown badges, interactive API key prompt modal, and zero external CDN dependencies |
...
| `GET /log.html` | browser live-log page: dark terminal-style viewer that opens `/log` itself — tries keyless first, falls back to the same API-key prompt as `/status.html` (the two pages share the stored key), pauses auto-scroll when you scroll up, offers manual retry after a disconnect, and has a Clear button that wipes the local view (the server-side buffer and live stream are untouched) |
