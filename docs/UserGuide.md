...
**Per-request memory budget**: three buffer caps, each independently reasonable, multiply: `max_request_body_mb` (default 8MB, the inbound request), the response-normalization buffer (32MB, guards against a runaway upstream when vmr has to buffer a response instead of streaming it), and the audit response copy (64MB, bounds how much of a response the audit trail retains). Worst case is roughly the sum — about 104MB — resident per in-flight request, before `bytes.Buffer` growth overhead. `max_concurrency` defaults to unlimited, so that number is only bounded by how many requests actually arrive at once. On a single-user local instance this is background noise; on a shared instance, set `max_concurrency` to something concrete rather than leaving the product of these four numbers open-ended.
