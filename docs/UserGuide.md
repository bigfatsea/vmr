...
| `GET /stats` | real-time and historical telemetry JSON payload: concurrency limits, in-flight request details (queued vs. running, caller, upstream endpoint, per-chunk streaming milestones, stall age), hourly/daily rollups, and quantile distributions (p50/p90 for TTFT and TPS) grouped by independent `client_key_tag` and upstream `key_label` dimensions (auth-gated via `api_keys`) |
