...
- **Phase 2 波次划分**（按文件交集重排）：波次 A = 2A（新建 internal/dashboard，不碰 archtest——预算已由主控预登记）∥ 2C（server/config/archtest 边界）；波次 B = 2B（删旧自包含 HTML 渲染器 + 接线 WriteSkeletons + fmtutil 侧 fixture 消费 + 删 -html/-redact），必须在 2A/2C 合并后串行（与两者均有交集）。波次 B 已合并（2B worker 中途按 PID 监控完成，主控验收含 gofmt 修复与 KNOWN_ISSUES 登记）。另派 2D 文档同步组（UserGuide 双语 + Analytics 设计文档，与代码组正交，PID 72439）。2A 的 `testdata/fmt_cases.json` 是 2B 的 Go 侧消费契约，字段结构不得擅改。
