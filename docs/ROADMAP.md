...
- **可能方案**：在 `journey-compare.html` 引入紧凑的双栏并排卡片——共享前缀一段折叠展示，分叉点起两侧逐步并排（入参、耗时、tokens、意图摘要）。
- **触发条件**：需要先评估与现有 Markdown 产物（`compare-*.md` 的纯文本描述）的关系——是否只做 HTML 看板侧的加强，还是也要改 Markdown 渲染层；立项前先写设计草案。
