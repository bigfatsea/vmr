<!-- Ver 2026-08-23 15:38, by Gemini -->
...
5. **实时轮询与交互控制**：支持可调节的自动刷新频率（关闭 / 5s / 15s / 30s / 60s）、倒计时指示、手动即时刷新与状态高亮变色。
...
```
+--------------------------------------------------------------------------------------------------+
|  VMR STATUS DASHBOARD    [ v4.0.0-b5749f7 ] [ pid:38491 ] [ darwin/arm64 ]     [ 15s ⟳ ] [ 🔑 Key ] |
+--------------------------------------------------------------------------------------------------+
...
* **刷新控制器 (Refresh Control)**：
  - 自动刷新下拉菜单（`Off`, `5s`, `15s` (默认), `30s`, `60s`）；
  - 刷新倒计时环形/数字指示；
  - “立即刷新” 按钮（带加载中 Spinner 动画）；
* **鉴权状态 (Auth Indicator)**：
  - 绿锁图标：`Public`（免密模式）；
...
| **5. 自动轮询与实时倒计时** | 选择 5s 自动刷新，制造一个冷却中的端点 | 冷却时间秒级倒计时平滑递减，5s 到期后静默刷新并更新状态。 |
...
1. **核心落地成果**：
   ...
   - **交互式运维控制**：支持可配置自动轮询（5s/15s/30s/60s/Off）、倒计时与即时刷新，实时展现并发排队、系统资源、宏观吞吐、端点健康状态机与额度水位；
