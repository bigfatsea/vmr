...
* 协议是 Adapter 的属性（`Protocol() string`），也是 `models.<name>.endpoints` 的 map key。一条 endpoint-group 引用的 provider 必须在所在 key 的协议下声明了 base_url——引用一个没声明该协议 base_url 的 provider，是加载期错误（"provider 有没有这个协议的 base_url"），不是配置写不出来；但单条 endpoint-group 内部混用两种协议依旧没有语法能表达它，每条 entry 天生只属于一个协议（见「配置参考」）。
...
Upstream   ├─ 2xx → 响应归一化 → 转发 → 上报健康成功 → 更新粘性指针 → 审计落盘
           ├─ 4xx/5xx → ClassifyError → ErrClient 直接返回；其余试下一个（请求侧误配类零冷却）
           └─ 网络错误 → 短冷却，试下一个
```
...
* **`role_map`：按 provider 做 role 改写**：部分 OpenAI 兼容 provider 会拒收它上游不认识的 role（典型：DashScope/千问拒收 OpenAI 为 o1/o3 系列引入的 `developer` role）——role 拒收发生在 provider 网关入口的枚举校验层，是该 provider API 实现的属性而非某个模型的属性，故 `role_map: {developer: system}` 挂在 `providers[]` 条目上，账号下所有端点自动继承。`jsonscan.RewriteRoles`（`internal/jsonscan/rewrite.go`）在 `RewriteModel` 之后、发出请求之前，用同一套字节级扫描/拼接手法（`TopLevelValues`/`SkipJSONValue` 与 `RewriteModel` 共享）定位顶层 `messages` 数组里每个消息对象的 `"role"` 键，命中 `role_map` 就地替换值，其余字节（键序、空白、消息正文、未知字段）原样保留；未命中任何映射时零拷贝返回原 slice。加载期校验拒绝空白 role 名/空白目标值/自映射（`system: system`）/首尾带空白的名字——匹配是精确字符串比较，带空白的键永远命中不了（用户要修的 400 照常发生）、带空白的值则改写出网关拒收的 role，两种失败在运行时都无任何提示，故在加载期直接拒绝；空 map 规整为 nil。`core.Endpoint.RoleMap` 随 `BuildSnapshot` 从 `config.Provider.RoleMap` 原样传下去。审计日志无需为此单独打标：`Attempt.Request.Body` 记录的就是改写后、真正发给上游的字节，与改写前的客户端原始请求对照即可看出差异（同 `RewriteModel` 的既有做法，未走 `Attempt.Norm`——那个字段专属响应侧归一化）。
...
cmd/vmr                    CLI（stdlib flag），一命令一文件：main.go（分发 + usage + Adapter 的 blank import 注册点）
  ...
  ├─ cmd_report.go/cmd_journey.go/auditpaths.go  `vmr analyze` 子命令 + 两者共用的输入路径解析——见 Part 2
...
internal/tokenutil         零依赖、零分配的 token 估算：Analyze 按字符类型（英文字母/数字/英文符号/CJK/空白/其他）线性回归加权（Estimate/EstimateText/EstimateFromStats），server.facts、reqdetail、report、journey、replay 等所有需要"廉价估算一段文本大概多少 token"的调用点共用同一份系数，不各自实现一份
internal/rundir            默认目录解析公式（~/.vmr → 系统临时目录 → cwd），config 的 log_dir/image_cache_dir 缺省值共用
internal/config            YAML 加载、${ENV} 展开、校验、热加载 watch；quota.go：Provider.Quota 的 YAML 形状（QuotaConfig/LimitConfig）与校验，`metric` 仅接受 `requests`/`tokens`（见 §6.6）；pricing.go：顶层 `exchange_rate:` 与 `Provider.Pricing`（`currency`/`aliases`/`rates`）的 YAML 形状，`validate()` 阶段调 `internal/pricing` 做二层解析，费率行的四分量校验是"全给或全不给"的形状校验，与配额门禁无关（`metric: cost` 已删除）
...
internal/quota             额度感知路由的记账半区（见 §6.6）：quota.go（Counters/Registry，Charge/Used，按 provider 名字记账不含 key 哈希；Counters 只存 Fresh/CacheRead/CacheWrite/Out/Requests 五个原始分量，`model_multipliers` 是唯一一个"计费时算好、不在读取时重算"的例外——已删除的 `metric: cost` 曾是第二个例外，见 `docs/VirtualModelRouter_Design_v4_Quota.md` §9.2）、period.go（周期数学，(every,since) 推算窗口边界含月末截断）、score.go（Headroom/ScoreForLimit）、store.go（vmr-quota.json 原子落盘）；只依赖 core，config 与 router 都依赖它，它不依赖两者
internal/pricing           离线定价解析引擎（见 §6.6，路由热路径不消费）：Rate/Table（per-1M 四分量费率，nil 分量=未知，绝不是免费）、resolve.go（Resolve/EffectiveRate/Complete——账号覆盖 → 标准表 → 无费率的二层解析，first-match-wins 静态按模型区分（无时间维度——P0-A 移除了曾经的 date_*/hour_* 时间窗，见 docs/VirtualModelRouter_Design_v4_Quota.md 的相应章节），discount 递归作用于"下层解析出的费率"而非恒定的 Base）、embed.go（go:embed 内置的 standard_price_generated.yaml + standard_price_curated.yaml + standard_exchange_rate.yaml）、resolver.go（Resolver，`vmr analyze` 用的按 provider+model 记忆化解析）；只依赖 core，config 与 report 都依赖它，它不依赖两者
...
internal/report, internal/journey, internal/ctxgraph, internal/taskseg, internal/chatmsg
                            `vmr analyze` 的完整实现——见 Part 2（本节 §9.4 已指路），
...
| `GET /status` | `instance`（进程身份 + 运行环境 + `base_urls`（各协议客户端入口地址，从请求自身回显——Host 头 + 是否 TLS，非 `listen` 推导）+ `config` 子对象含新鲜度/重载/告警 + `concurrency`）+ `system`（goroutines/内存/磁盘余量）+ `traffic`（固定原子计数器：请求按协议/结果分类、全局 5 分量 token、`sticky.entries`）+ `models` 数组（每虚拟模型 × 协议一条：`capabilities` 取跨端点并集、`max_context_tokens` 取跨端点最大值，空/0 = 不限制；端点项含各自健康与 capabilities/context 覆盖；JSON 形状由 server 侧契约测试锁定）+ `quota` + `audit`/`image_cache` 磁盘占用 JSON（健康部分含 `probing` 字段--某个端点当前是否正被一次后台恢复探测占着单飞名额）；受 `api_keys` 鉴权保护 |
...
鉴权（可选 `api_keys`，字符串列表）同时接受 `Authorization: Bearer` 与 `x-api-key`，作用于 `/v1/*`。命中哪把 key 就用 `audit.KeyTag`（那把 key 自身的尾部）给这次请求打上 `client_key_tag`，供 `vmr analyze` 事后按调用方分组导出（见「审计日志」）——`api_keys` 是唯一的鉴权面，`authenticate()` 只有这一条代码路径。不配置 `api_keys` 时鉴权整体关闭，但 `authenticate()` 仍会对客户端自愿发来的 `Authorization`/`x-api-key` 值调用同一个 `audit.KeyTag`（无 16 字符下限——这个模式下它不是需要保护的密钥），让纯内网场景下客户端可以零 vmr 侧配置地自报身份标签；不发任何凭证的请求依旧是未打标签的记录。
...
ErrAuth         401 / 403（非内容类）；或 4xx body 命中 OAuth 标准错误码（authHint：
                invalid_grant / invalid_token / token has expired）→ 长冷却（10min 起），切换
...
ErrQuirk        端点专属协议约束拒绝（DeepSeek 思考模式要求 reasoning_content 回传、Google
                thought_signature 等）→ 切换，但不惩罚端点健康（零冷却；历史形态不合该端点的规则，
                非端点故障）
```
...
**已知边界**：个别厂商（如 MiniMax）会在 HTTP 200 响应内嵌合规标记（`input_sensitive`/`output_sensitive` 等字段）并可能返回空/替换内容。响应归一化器会嗅探这两个标记并记入审计 `norm`（`soft_block_detected`，见下文「响应侧归一化」）；**仅观测、不干预**：字节原样到达客户端，不触发 failover、不影响端点健康（详见 `KNOWN_ISSUES` 权衡论证）。事前关键词过滤仍是未来降低此类拦截触发概率的探索方向（见「路线图」）。
...
* **buffered**：整体缓冲，EOF 时单遍 regex 归一化。用于：非 SSE 响应（单 JSON，客户端本来就等完整体）；首个载荷事件含 `<think>` 或 content 以 `Thinking Process:` 开头的 SSE（思考期间客户端本就无正文可看，缓冲无体验损失）；未及判定即 EOF 的小流。**`<think>` 触发的缓冲在 closer 到达后恢复流式**（剥掉 think 块、已缓冲部分一次吐出、其余实时转发）——客户端只等了思考阶段；`Thinking Process:` 形态的结束标记在响应末尾才可识别，无法恢复，只能全缓冲。缓冲上限 8MB（按 1M-token 上下文窗口约 3-4MB 字节量 + 约 2 倍余量估算，与 `config.DefaultMaxRequestBodyMB` 同一套推导，而非拍脑袋的整数），超限放弃归一化降级为原样透传（=直连行为）。
...
| 中途截断的部分字节 flush | 上游流非 EOF 失败时，把可安全交付的已收字节（非 SSE 的部分 JSON，或已进入 passthrough 的 SSE 尾部）吐给客户端，随后 `router` abort 连接（见下方「上游流中途失败」） | `truncated_flush` |
| 中途截断时扣住 SSE 缓冲（**不 flush，仅记录**） | 上游流非 EOF 失败时，SSE 流仍处于 undecided/buffered（思考形态待剥离）——已收字节不吐出，避免泄漏未闭合 `<think>` | `truncated_withheld` |

`isSSE` 由上游响应的 `Content-Type` 判定（含 `text/event-stream`；缺失时回退到客户端的 `stream` 字段），而非盲信请求参数——上游若忽略 `stream` 返回 JSON，原样透传。

**上游流中途失败（非 EOF）**：连接重置、`stream_idle` 看门狗关闭等发生在响应头已发出之后时，`respnorm` 已积累但未吐出的字节不能静默丢弃——客户端已持有 `200 OK` + headers，一个干净的空 body 会被读成"成功的空回复"。处理：把**可安全交付**的已收字节 flush 给客户端后再上报错误（`truncated_flush`），`router` 随即 `panic(http.ErrAbortHandler)` 让 net/http 中止连接、不写终止 chunk，客户端 SDK 因此看到断掉的传输而非干净成功。"可安全交付"的边界：非 SSE 响应 flush 已缓冲的部分 JSON（直连也是这个结果）；SSE 流只在已进入 passthrough 时 flush 尾部，仍处于 undecided/buffered 的一律扣住不 flush（`truncated_withheld`）——SSE 进 buffered 只可能是思考形态被扣住待剥离，raw flush 会把未闭合的 `<think>` 泄漏给客户端，正是本层要防的反馈循环。audit 顶层 `outcome` 仍记 `ok`（HTTP 传输层口径），attempt 记 `truncated: <原因>`——两个账本口径不同，见「审计日志」六条约定第 1 条。
...
**只记原值、不下判断**：版本钉死（`gpt-4o` → `gpt-4o-2024-08-06`）、厂商前缀、套餐别名（`ark-code-latest` → `doubao-seed-code-251015`）每次都会合法地不一致，区分"别名"与"中转商偷换"只能靠聚合分布（稳定映射是别名），属于离线的 `vmr analyze`。代价是每个响应多一次正则扫描（`modelSeen` 闩住，不是每个 SSE chunk），纯读不改字节。
...
**计量**：`metric: requests`（计数 +1，零解析成本）与 `metric: tokens`（四分量求和，优先用上游返回的 usage 字段，拿不到时降级为按字节数估算并标记 `estimated_pct`；四分量的账号级权重 `token_weights` 在**读取时**套用，缺省全 1.0 等价于 P1 的等权求和）属 P1/P2.1。账号级 `model_multipliers`（按上游模型的整体倍率，在**计费时**套用）属 P2.1。多条 Limit 并存与按模型的 `models:` 子额度（含 `["*"]` 通配的"每模型独立池"与具名限定两种形态，`token_weights`/`model_multipliers` 随之下沉到单条 Limit）已随 P3 交付；`rolling` 滚动窗口仍未支持，配置里写了 `rolling: true` 会在加载期直接报错，不会静默忽略。曾经设想过的第三档 `metric: cost` 已随 Pricing 架构极简化重构（决策 6，见 `docs/VirtualModelRouter_Design_v4_Quota.md`）整体删除——不是降级，路由半区从此不再解析、不再消费任何价格数据；该档当初为什么被设计进来、后来又为什么判定它把控制面和计费面焊在了一起，完整论证见 `docs/VirtualModelRouter_Design_v4_Quota.md` 的决策记录。

**定价**：`internal/pricing`（叶子包，只依赖 `core`）持有内置标准价目表（`go:embed` 的 `standard_price_generated.yaml`，由 `tools/gen_standard_pricing` 从 LiteLLM 快照生成 + 手工维护的 `standard_price_curated.yaml` 补国产第一方厂商），叠加用户在 `config.yaml` 里每个 `providers[].pricing`（`currency`/`aliases`/`rates`，`rates` 含 `discount`/显式费率，first-match-wins 静态按模型区分——无时间维度，P0-A 移除了曾经的 `date_*`/`hour_*` 时间窗，见 `docs/VirtualModelRouter_Design_v4_Quota.md` 的相应章节）——纯粹的二层模型，没有第三层外部文件、没有顶层内联价目表。`providers[].pricing.currency` 只是**解析期标注**（这一行的 `rates` 用什么币种书写），经顶层 `exchange_rate`（通用的"1 美元 = X `<货币代码>`"映射表，未声明的货币先查内置默认表兜底）在 `validate()` 阶段一次性折算成 USD 存入内存，**从不是运行态量**——不进 `core.Endpoint`、不进审计日志、不进配额计数器。定价解析完全下沉到离线的 `vmr analyze`（`internal/pricing.Resolver`），路由热路径上不存在任何价格数据（完整设计见 `docs/VirtualModelRouter_Design_v4_Quota.md`）。`vmr analyze` 的展示币种（`-currency`）是纯渲染层的独立选项（`Resolver.WithDisplayFactor`），只重新标定最终显示的数字，从不改动解析逻辑。
...
- **失效**：TTL + 容量双闸，都搭在缓存目录访问上惰性触发（每目录每天至多一次，仿审计 housekeeping，不起 ticker）。TTL 按最近命中时间（mtime）（`ttl.image_cache`，缺省 7 天；命中时 `os.Chtimes` 刷 mtime，语义是"最近使用"）；容量上限 `imgprep.DefaultCacheCapBytes`（50MiB），超额按 mtime 淘汰最旧——TTL 窗口内一次性涌入大量互不相同的截图（长跑 agent）会在 TTL 到期前就把目录顶到几十 MB，容量闸是这种形态下唯一的上界。TTL 默认值比 `ttl.audit_retention`（默认 90 天）短得多是刻意的：审计有取证价值，删除是数据丢失风险；图片缓存纯粹是优化，没有"长期保留"的理由，更短的默认 TTL 与更主动的清理都是更安全的选择。
...
1. **成功尝试的响应 body 不存**：透传恒等，它与 `client.response.body` 字节相同，只在 client 层存一份；两者的字节差异**完整由 `norm` 列表解释**（`model_rewrite`/`think_strip`/`thinking_process_strip`/`done_appended`/`buffered`/`resumed_stream`/`opaque`/`overflow_raw_passthrough`/`truncated_flush`）——**纯观测、不对应字节改动的标记有 `soft_block_detected`、`crlf_framing_suspected`、`truncated_withheld`**：出现时 upstream body 与 client body 的关系见「响应侧归一化」。失败尝试的错误 body（≤128KB，`router.errBodyCap`）存在 attempt 内；超出上限时转发给客户端的字节仍是未改动的截断前缀（byte-faithful 对客户端始终成立），只有 attempt 内的审计副本会在末尾追加 `...(truncated at N bytes)` 标记（N = 上限本身，不是上游真实大小——`io.LimitReader` 故意不读过上限，真实大小未知）。成功尝试后流中断时 `error` 为 `"truncated: <原因>"`，audit 顶层 `outcome` 仍为 `ok`（status 与 error 并存即"当时 200 但中途断了"）——但客户端侧不再是干净结束：`respnorm` 先把可安全交付的已收字节 flush 给客户端（`truncated_flush`；SSE thinking 阶段扣住不 flush 记 `truncated_withheld`），`router` 随后 `panic(http.ErrAbortHandler)` 中止连接，客户端 SDK 因此看到断掉的传输（见「响应侧归一化」的"上游流中途失败"段）。
2. **body 编码；请求侧不截断，响应侧审计副本有 16MiB 上限**：合法 JSON 原样嵌入（可直接用 jq 查询，如 `.client.response.body.usage`）；非 JSON（如 SSE 流文本）为字符串。**请求侧不设记录上限**——不论原始请求体有多大，只要 vmr 接受了，审计里就是完整的那一份；入站体大小另有一个纯稳定性考虑的独立上限（`max_request_body_mb`，缺省 8MiB，超限 413），它只决定 vmr 愿不愿意接受这个请求，与审计完整性无关。**响应侧的 `client.response.body` 审计副本有一个 16MiB 上限**（`server.recorderBodyCap`，比 `respnorm` 8MiB 归一化缓冲留了更多余量——审计副本被截断丢的是 `vmr analyze` 需要的信息本身）：客户端始终收到完整、未截断的字节流，只有审计副本在超限时被截断并在末尾追加 `\n...(recording truncated at N bytes)` 标记。结合约定 1（成功尝试的响应 body 本就不存），一条 >16MiB 的成功响应在审计里既没有完整的 client body、也没有 attempt body。流式响应的 usage 通常在末尾 SSE 事件里，脚本需从字符串 body 中解析。
...
`internal/audit`：Record 类型 + Logger（互斥追加、按日期轮转，轮转时异步触发历史文件压缩/保留扫描，见后文；JSON 编码走 `sync.Pool` 在锁外并行，只有 `l.f.Write` 系统调用在互斥锁内）。server 层用包装 `ResponseWriter` 的录制器捕获 client 层响应（保留 `Flusher`，流式时延零影响，`bytes.Buffer` 增长到 `server.recorderBodyCap`（16MiB）为止——超出则审计副本截断并追加标记，客户端收到的字节流完全不受影响；`-audit=false` 时录制器只留 status + TTFT，不缓冲 body 也不脱敏请求头）；router 层在 failover 循环中逐次填充 attempts，包括 `protocol`/`provider`/`model`/`error_class` 四个结构化字段。Record 经 `Serve` 参数显式传递（nil = 关闭，零开销）。

### 9.4 统计分析工具 `vmr analyze` / `vmr analyze`

审计 JSONL 的离线消费方——聚合报表（`vmr analyze`）与 Agent 任务叙事重建（`vmr analyze`）——完整设计见姊妹文档
...
* **保留**：配置项 `ttl.audit_retention`（`CalendarDuration`，支持 `d`/`w`/`mo`/`y` 单位，大小写不敏感；`0`/未写 = 用默认 90 天）。不支持任何"永久保留"语义——`forever`/`permanent`/`never` 这类关键字在加载期直接拒绝；真正需要长期保留的部署显式写一个足够大的数值（如 `90000d` ≈ 246 年）。审计日志是 `vmr analyze` 成本核算的唯一数据源，删除是数据丢失风险，所以默认值选得偏保守（90 天而非更短），但"用户完全不管这个字段也不会丢数据"不再是设计前提——依赖旧版"0 = 永不删除"语义的部署升级后必须显式改写，否则 90 天后开始清理（`CHANGELOG.md` 的 Breaking Change 记录）。
...
* **`vmr analyze` 的配套**：`Build` 按扩展名分支，`.zst` 输入透明解压后再喂给同一套 JSONL 解析——历史压缩文件与当天明文文件可以混在同一次 glob 里（`vmr analyze 'vmr-audit-*.jsonl*'`），调用方不需要关心哪个是哪个。
...
api_keys:                     # 可选：vmr 自身鉴权（Bearer 或 x-api-key）；字符串数组，非具名映射；每把 ≥16 字符
  ...
  - ${VMR_KEY_OPENCLAW}       #   client_key_tag = audit.KeyTag(该 key)，供 vmr analyze 按调用方分组导出。
...
analytics:                    # 可选：/reports/ 静态托管（见端点表 `GET /reports/*` 行）；缺省整块不写 = 不挂载
  serve: false                # true = 对外服务 /reports/*；false = 裸 404（与未挂载无区别）。随热重载生效
  serve_dir: ./reports        # 托管根目录；相对路径按 vmr start 进程的工作目录解析。随热重载生效
...
**多 Provider 端点组与全局 FallbackEndpoints——都是配置期展开，不改变运行时候选列表的结构**：一条 entry 的 `providers`（始终是列表）让它代表好几个账号——`BuildSnapshot` 在既有展开循环外再套一层按 `eg.Providers` 的循环，每个 `core.Endpoint` 和手写独立 entry 完全一样（各自的 `HealthKey()`、Sticky 绑定、按 `Provider` 名读写的配额账本）。顶层 `fallback_endpoints:` 在处理完一个虚拟模型自己的 `endpoints` 后，按协议 key 直查并追加到每个已有对应协议路由的模型末尾（模型可用 `fallback: false` 整体退出）。解决「同优先级多供应商 / 跨虚拟模型重复兜底」两类配置膨胀。
...
| 降采样缓存按 mtime TTL + 50MiB 容量上限双闸，均惰性触发 | 只做 TTL / 只做容量 / 起独立 ticker | 单 TTL 不够：TTL 窗口内（缺省 7 天）涌入大量互不相同的图片会在到期前把目录顶高，容量上限（`imgprep.DefaultCacheCapBytes`，超额按 mtime 淘汰最旧）是这种形态下唯一的上界。两闸都搭在缓存目录访问上按天惰性触发，不为一个纯优化、可从空目录冷启动的派生目录养一条 ticker goroutine |
...
| `ttl.audit_retention` 的 `0`/未写 = 用默认 90 天，不支持任何"永久保留"关键字（`forever`/`permanent`/`never` 拒绝加载） | 缺省 `0` = 永不删除，需要具体天数才启用清理 | 早期设计让 `0` 兼表"用默认"和"永不删除"两种相反意图，是历史遗留的怪异零值语义；审计日志是 `vmr analyze` 成本核算的唯一数据源，删除是数据丢失风险，但"零值天然安全"这个前提本身站不住——默认值给得足够保守（90 天，压缩已经解决了大头的磁盘占用）比保留一个双关的零值更清楚。真正需要长期保留的部署显式写一个足够大的数值（如 `90000d`），这是用户的明确选择，不是字段的隐藏极性（Breaking Change，见 `CHANGELOG.md`） |
...
| `vmr replay --record` 写出的记录字段布局模仿真实流量的约定（`Client.Response` 存全量，`Attempts[0].Response.Body` 仅失败时存） | 无条件把响应体存两份 | 让 replay 产出的记录能被 `vmr analyze`/`jq`/再次 `vmr replay` 当作普通审计记录正确消费，不需要为"这是 replay 产出的"开一条特殊解析路径；`Client.Request.Body` 存的是回放前的原文（虚拟模型名），不是改写后发给上游的字节——同一约定，读侧不用区分来源 |
...
**工具选型：Vegeta**（`github.com/tsenart/vegeta`）。核心诉求是"配置文件驱动"（不为测个性能再学一门脚本语言）和"能应对 SSE 流式响应"；调研过 k6（功能最全但要学 JS + 自己编译 SSE 扩展）、oha/hey（命令行参数驱动，不支持按请求变化 body）、Artillery（引入 Node.js 依赖）。Vegeta 是 Go 原生、JSON-lines targets 文件、内置延迟百分位统计，与项目技术栈最贴合。**一个关键简化**：不需要压测工具懂 TTFB/流式细节——vmr 自己的审计日志已经把每条请求的 `ttft_ms`/`dur_ms` 记下来。于是分工很清楚：Vegeta 只负责"客户端视角的总延迟/吞吐/成功率"，"是不是因为全缓冲变慢了"这类更细的按场景归因，`loadtest/runner` 自己直接扫一遍这次运行产生的审计 JSONL 现算（`computeServerStats`，`runner/main.go`）；场景区分也顺势用虚拟模型名当标签，mock 上游按 model 名决定要模拟哪种响应形态。唯一需要自己写的是这个 mock 上游（`loadtest/mockupstream`）——没有任何通用压测工具会假装自己是一个 LLM provider，更不用说模拟已知的具体怪癖形态（MiniMax 的 thinking 泄漏文本、SSE 分块节奏）。**`loadtest/runner` 刻意不跑 `vmr analyze`、不 import `internal/report`**：压测测的是 vmr 的 HTTP 路由/转发层，它的结果不该系于另一个命令（`vmr analyze`）的渲染管线是否碰巧还能跑通——`internal/report` 的分析管线本身会独立演进（会话分组、成本估算等），压测不需要、也不该被它拖着走。这条边界由 `go list -deps ./loadtest/runner` 里不出现任何 `vmr/internal/*` 包来保证。

**场景矩阵**（`loadtest/config.yaml` 里每个场景一个虚拟模型，覆盖开销特征明显不同的代码路径，不做协议交叉/并发梯度扫描——一次性健全性检查不是要画一条完整性能曲线）：`baseline`（路由开销下限）、`stream_normal`（真流式透传）、`drip_stream`（真流式慢滴，~5s/请求，制造持续几十个在途流式连接以承载 InflightRegistry、流看门狗与并发审计写盘）、`thinking_leak`（已知最差路径——全程缓冲到 EOF）、`think_tag`（`<think>` 标签形态，先缓冲后恢复流式）、`big_response`（大体积非流式响应）、`big_image`/`multi_image`（图片降采样的完整 decode→scale→encode 链路，单图与多图）、`gif`（确认永不缩放的快速跳过路径依然便宜）、`long_history`（长对话历史的 JSON 探测扫描 + model splice + 审计全量写盘开销）、`failover`（health 状态机 + 冷却 + 故障切换循环开销）、`quota`（实测配额额度感知重排与真实 usage 计费路径）、`sticky`（跨请求会话指纹缓存与端点钉定）、`anthropic_baseline`/`anthropic_stream`（确认 Anthropic 协议适配器非流式与流式 SSE 路径无额外成本）、`responses_baseline`/`responses_stream`（确认 openai-responses 协议适配器非流式与流式 SSE 路径同样无额外成本）。`loadtest/runner`（`go run ./loadtest/runner`）把起 mock 上游、起 vmr、生成 targets、低强度预热、按 `light`/`moderate`/`heavy` 三档递增负载（10/50/150 req/s）依次跑 Vegeta、再现算按轮次/按场景/按端点的汇总并采样进程资源（RSS 与 CPU 时间），全部串成一条命令，产物落在项目原有的 `logs/loadtest/`（独立子目录，每次运行前清空）与 `reports/loadtest-report.md`（0600 权限），不与真实数据混放。客户端视角分为 `plain`（13 场景）、`stream`（`drip_stream`，5s 长流式）、`image`（3 场景：`big_image`/`multi_image`/`gif`）三个独立成本域分组统计百分位——避免图片处理与长流式的物理耗时将普通请求的 p95/p99 拉高失真。

**`big_image`/`multi_image` 用一批变体图片，且行交错写入**：`internal/imgprep` 的降采样磁盘缓存按内容哈希 + 目标像素上限做 key（见「请求图片自动降采样」§7.1），如果三档负载全程只用同一张固定图，除了全程第一次，其余请求都会命中缓存——真正的 decode→scale→encode 反而被缓存挡在外面，测不到这两个场景本该测的东西。`loadtest/gentargets` 因此给这两个场景各生成 `cacheBustVariants`（50）张内容不同、但尺寸/复杂度相同的变体；`gif` 虽然永不缩放，但按同一行数交错排列（`big_image[0], multi_image[0], gif[0], big_image[1]...`）——Vegeta 按目标文件行序循环发送，块状排列会导致短轮次（如 light 轮 20 请求）完全打不到后置场景，交错排列确保任意请求量下 3 场景均分流量。**`loadtest/config.yaml` 单独声明 `image_cache_dir`（`logs/loadtest` 下的子目录，随 `logs/loadtest` 一起在每次运行前清空）**：这批变体图片是确定性生成的（同样的 seed 每次产出同样的字节），如果和真实部署共用默认的 `~/.vmr/image_cache`，第二次跑测试时上一次生成的"新"变体早就被缓存预热过，测试会在不知不觉间悄悄退回"全程命中缓存"的老问题。

运行方式与如何读数字的完整操作说明见 [`loadtest/README.md`](../loadtest/README.md)，本节只记录设计判断与结论。

**实测结论（2026-09-12，17 场景 × 3 档负载 10/50/150 req/s，100% 成功率）**：

- **vmr 自己的路由/透传/归一化/协议适配开销维持亚毫秒级**——除图片降采样与慢滴流外，所有 plain 场景服务端 p50 均为 0ms（整数毫秒截断的真实 sub-ms 观测，不再向上偏置剔除），客户端视角 plain 组 p50 < 1ms、p95 4–5ms，即使在 150 req/s 重负载下也极度平稳。`anthropic_*` 与 `responses_*` 协议流式/非流式均无额外成本。
- **慢滴流式 `drip_stream` 成功建立并发长连接承载**——每个请求持续 5s，服务端首包 ttft 稳定在 ~100ms、dur 稳定在 5000ms；重负载轮成功承载持续并发，客户端成功率 100.0%，证实流式透传通道与 in-flight 跟踪在高并发下无死锁与漏流。
- **图片降采样真实反映计算开销与缓存收益**：`big_image` 冷态 p95 ~100ms、热态随变体复用降至 ~23ms；`multi_image` 冷态 ~60ms、热态 ~11ms；`gif` 维持 ~0-1ms 快速跳过。
- **配额与会话粘性**：`quota` 场景 241 次请求平稳计入账本，无错误无超扣；`sticky` 场景全量 241 次请求成功钉在首选端点上，第二端点零尝试，亲和链路验证完毕。
- **进程资源受控**：全过程采样 370+ 次，峰值 RSS ~147 MB，累计 CPU 耗时约 18 秒，未发生内存或协程失控泄漏。
...
目标：请求发往上游前做关键词过滤/替换（外部词库），降低触发厂商内容合规拦截的概率；对 2xx 内嵌的"软拦截"（如 MiniMax `input_sensitive`）是唯一的**事前**防线。响应侧现在能**事后观测**这类拦截（`soft_block_detected`）——但事前过滤仍是降低触发概率的唯一手段。
...
* **报表/叙事增强**：`vmr analyze` 自己的路线图见 Part 2。
...
判定"动它的收益低于扰动成本"的项。每项都不是 bug，改与不改行为一致；列在这里免得下次有人盯着它们犹豫时重新论证。分析半区的同类清单见 `KNOWN_ISSUES`。
...
| `vmr analyze` 不区分 `vmr replay --record` 产出的记录（`replay_of` 字段）与真实流量 | 指向包含两者的 glob 时，回放记录会被当普通请求计入统计 | 属于用户主动行为——`--record` 默认不写、写了也是独立文件，只有显式把 glob 指向它才会混入；混入本身有时是期望行为（比如想验证回放请求的 token 用量）。真出现"不小心混进日常统计"的抱怨再加过滤 |
...
**`--record`**：把这次回放的请求/响应也写一条 `audit.Record`，追加到用户指定的独立文件（不写入常规 `log_dir`，不会被 `vmr analyze` 的常规 glob 意外扫到）。字段布局刻意模仿真实流量的既有约定，而不是简单地把能填的都填满：`Client.Request.Body` 存回放前的原文（虚拟模型名，不是发给上游的改写后字节——那部分在 `Attempts[0].Request.Body`）；`Client.Response` 存完整响应；`Attempts[0].Response.Body` 只在失败（状态码 ≥400）时存，成功时省略——与 `router.tryOne` 的既有约定一致（透传恒等，省略是因为与 `Client.Response.Body` 字节相同）。这样 `--record` 产出的文件可以被 `vmr analyze`/`jq`/再次 `vmr replay` 当成普通审计记录正确消费，不需要为"这是回放产出的"单开一条解析路径；记录本身带 `replay_of` 字段（"来源文件:行号"）标注来源，供人工排查。`DurMS`/`Attempts[0].DurMS` 测的是完整响应体传输完成后的总耗时，不是拿到响应头就停表——与 server/router 对这两个字段"总耗时"的定义一致，否则一个响应头快、body 慢的请求会被记成"很快"。
