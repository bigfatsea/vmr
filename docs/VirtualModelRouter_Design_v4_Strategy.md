...
2. **第二层：新会话按配速分流**——只对没有 Sticky 命中的全新会话生效，在既有 `strategy.Sort`（如 `priority`）排序打平后的同一优先级梯队内部重排，不取代 `priority`。分配策略是无状态的贪心（按配速分数降序），不是平滑加权轮询——SWRR 需要额外的持久化累加器、且撒开流量本身会伤害 Prefix-Cache 局部性，与本项目"缓存命中率优先"的取舍方向相反。
