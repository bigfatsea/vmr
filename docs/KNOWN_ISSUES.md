...
- **不对 HTTP 2xx 软拦截（如 MiniMax `input_sensitive`）做运行时 Failover，坚守字节保真透传**：个别厂商（MiniMax）在触发内容合规拦截时返回 HTTP 200 OK 且内嵌 `"input_sensitive":true` / `"output_sensitive":true`，同时提供空回复或套话。VMR 裁决**不做** 2xx 响应拦截与 failover（彻底废除历史上的 `soft_block_failover` 开关与 64KB peek buffer），原因有三：① **流式不可逆**：95%+ 的 Agent 与生产请求走 SSE 流式，首包与 200 OK 状态行早已交付客户端，流式中途无法撤销请求再切换候选；② **契约与计费违背**：上游已记 200 并扣除 Token 配额，路由器私自拦截 200 丢弃并重试会造成非预期的二次计费与延迟翻倍；③ **消除单厂商侵入式负担（KISS/YAGNI）**：其他主流厂商（OpenAI/Claude/DeepSeek 等）合规均返回标准 4xx，天然由 `handleErrorResponse` 归为 `ErrContent` 自动切换；为单一厂商特定的非流式边缘行为在 `models`/`endpoints` 暴露配置项和运行时预读缓冲，给全体用户造成严重认知负担。**当前方案**：`respnorm` 仅在审计层观测并打上 `soft_block_detected` 记录供排查，响应字节原样透传。事前关键词过滤是降低此类触发的唯一正规路线（见设计文档路线图）。
...
- **`ctxgraph.Manifest.MsgIdx` 是承载性导出数据，不是死字段**：`ctxgraph` 不导出任何哈希函数，`MsgIdx` 是包外把 `Keys[i]` 对回 `chatmsg.Messages` 元素的**唯一通道**。两类消费方：`cmd/vmr` 的 `vmr diff` 用它把 manifest 哈希位置映射回真实消息角色（`tailLine` / `msgRoleAt`），是一等生产消费者；`internal/journey/structure_test.go` 用它验证"内容寻址坐标确实解析到所声称的内容"这条不变量。删掉它等于同时砍掉 `vmr diff` 与该不变量的包外验证，还要让全部用户白付一次全语料重解析。（本条曾写"没有生产消费者"——`vmr diff` 落地后已过时。）
...
- **`imgprep.HasImageMarker` 的宽松预检维持「宁误报不漏报」，不收窄**：`bytes.Contains(body, '"image'... )` 或 `input_image`）会让正文里 `"image_path"` 之类的代码文本误触整套降采样反序列化——但误报只多付一次 JSON 解析成本，解析后结构化 dispatch 找不到真实图片块即原样放行，无正确性后果（`TestDownscaleTextMentioningMarkerIsNotAnImage` 钉死「误报≠误判」）。收窄需枚举全部已知图片引用形态 token，未来新增形态（如本次修复的 Anthropic `tool_result` 子块 `type:"image"`）会重引漏报；漏报是硬路由 `HasImage` Condition 的正确性 bug，误报只是性能小税，故维持现状。
...
- **配了 quota 的部署里，`vmr-quota.json` 变化会使 L2 缓存失效**（2026-09-07，NEW-D 修复）：§2.5 与 `macro/finance.json` 的 `provider_quotas` 直接读 `<log_dir>/vmr-quota.json`（路由半区每次计费请求都会重写它），其内容因此并入 L2 输入哈希集——否则同批日志隔时重跑会在 L2 命中路径上给出过期的额度进度与已用量。副作用：路由半区正在活跃承接流量时，对同一批历史日志反复 `vmr analyze` 会频繁 L2 miss（配额计数器确实在动，§2.5 本就该重算）；这是刻意的保守失效，不是缺陷。不含 quota 限制的配置完全不受影响（`vmr-quota.json` 不进指纹）。`period_elapsed_pct` 这类纯 wall-clock 派生值仍是"as of 上次全量运行"——要 render-time 现算需把它移出切片、渲染侧按 `manifest.generated_at` 重算，属独立的低优改进。
...
#### 2.81 [已落地登记] 2026-09 配置形态简化（ttl/timeouts 归集、endpoints map 化、`disabled`、`model_defaults`）的后续决策
...
- **拆除触发条件**：无（登记防反复重提；后续若改任一取舍，先改本条再改代码）。
