...
- **`Digest` 构造在 report/journey 各有一份，不以共享包统一（2026-09，产物缓存落地时裁决）**：设计裁决 D8 要求全系统一个 Digest 构造，但两半区契约禁 report→journey import，唯一共享位置是新建叶子包——为两个 ~40 行纯函数建包不划算。落地形态：两份实现 + `cmd/vmr/digest_parity_test.go` 差分钉死（随机向量任意分割两侧同输出 + 手算向量钉 uvarint 前缀线格式），漂移即红。**不要以「D8 说只有一个」为由建共享包或让一侧 import 另一侧**；出现第三个消费者时再评估下沉。
