...
> **监控与故障恢复**：监控须对照任务清单跟踪各 Worker 的进度与状态，确认其真实推进而非空转。子 Agent 可能中途出错或卡死（典型信号：日志长时间无进展、反复重试同一错误）；主控依据现场状态判断：可恢复的注入补充指令续跑，不可恢复的终止进程、回退未提交改动后重新派发。
...
7. **长程任务计划先行、全程有账**：开工前主控须先制定完整执行计划（任务拆解与依赖排序），执行中的进度跟踪与状态记录均以此计划为基准——对照计划维护任务清单，逐项登记状态（待派发 / 进行中 / 已提交 / 已合并）、逐项销项，防止长程任务中疏漏或跑偏。
8. **子 Agent 故障按需处置**：子 Agent 可能中途出错或卡死。主控须依据日志与现场状态独立判断，按需恢复或重启——既不盲目重启，也不放任空转。
...
| **子 Agent 卡死或空转** | Worker 日志长时间无进展、反复重试同一错误，任务实际停滞 | 主控在监控阶段跟踪日志与进度，发现卡死按需处置：可恢复的注入补充指令续跑，不可恢复的终止进程、回退改动后重新派发 |
