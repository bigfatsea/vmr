...
| 编号 | 问题 | 级别 | 模块 | 性质 | 成本 | 风险 | 验收基准 | 最终核验状态（`2646b1d` 落地后） |
| :--- | :--- | :---: | :--- | :--- | :---: | :---: | :--- | :--- |
| **P0-A** | `metric: cost` 混合定价端点静默低估 | **P0** | `report` | 正确性 | 中 | 低 | 混合 fixture 差分测试 + 负向验证 | ✅ **已在 `2646b1d` 落地**（`WindowUnpricedPct` + `◇` 标记与脚注） |
| **P1-A** | 文件行数守卫仍是白名单（11 个大文件裸奔） | **P1** | `archtest` | 护栏 | 低 | 无 | 800 行临时文件必须让测试变红 | ✅ **已在 `2646b1d` 落地**（反转为默认 700 + 豁免表） |
| **P1-B** | 当前状态类文档在文档守卫扫描范围外 | **P1** | `archtest` / docs | 护栏 | 低 | 无 | 蓝图类文档纳入符号校验，或降级其定位 | 📌 **已闭环**（确立 `KNOWN_ISSUES` 为唯一权威清单，review 文档为历史档案） |
| **P2-A** | `detail.go` 1047/1150，四项正交职责 | **P2** | `report` | 内聚 | 中 | 低 | `detail.go` ≤ 700，子文件按实测登记预算 | 📋 **已转入 `KNOWN_ISSUES` §1.4 待定**（守卫纳管，超预算自会报警） |
| **P2-B** | `internal/audit` 三文件无行数预算 | **P2** | `audit` | 护栏 | 低 | 无 | 随 P1-A 自动闭环 | ✅ **已随 P1-A 自动闭环**（受全局默认 700 约束） |
| **P2-C** | `Row`/`ClientRow` 缺失 `CostEstimateEst` 字段 | **P2** | `report` | 完备性 | 低 | 低 | 字段补齐，多维报表透出估算占比 | 📋 **已转入 `KNOWN_ISSUES` §1.6 待定**（展示口径完备性演化） |
| **P3-A** | `i18n` 26 份工厂样板 | **P3** | `i18n` | 样板 | 中 | 低 | 文件数仍为 26，`i18n_e2e_test.go` 全绿 | 📌 **已转入 `KNOWN_ISSUES` §2.4 确定不修**（收益为负，仅省 ~50 行） |
| **P3-B** | `core` 包注释称"两个入口协议" | **P3** | `core` | 文档 | 极低 | 无 | 一行改动 | ✅ **已在 `2646b1d` 修复**（修正为三个入口协议） |
| **P3-C** | `diagnose`/`replay` 无文件预算 | **P3** | `archtest` | 护栏 | 低 | 无 | 随 P1-A 自动闭环 | ✅ **已随 P1-A 自动闭环**（受全局默认 700 约束） |
| **P3-D** | `session.go` / `cmd_story.go` / `compare.go` | 观察 | — | — | — | — | **不动，等预算报警** | 📋 **已转入 `KNOWN_ISSUES` §2 / 观察项**（守卫在位） |
| **P3-E** | `probe` 未登记在 `zeroInternalDepPackages` | **P3** | `archtest` | 护栏 | 极低 | 无 | 补充到守卫切片并通过 CI | 📌 **已转入 `KNOWN_ISSUES` §2.4 确定不修**（保留未来协议演进自由） |
| **P3-F** | `func_sizes_test.go` 同名 receiver 豁免精度 | **P3** | `archtest` | 护栏 | 低 | 无 | 豁免 key 改为包含 receiver 类型 | 📋 **已转入 `KNOWN_ISSUES` §1.7 待定**（当前无超标重名方法） |
