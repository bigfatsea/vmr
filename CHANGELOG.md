...
- A structurally corrupt `vmr-quota.json` (a `null` bucket, an unrecognized version stamp) is rejected wholesale instead of reaching a nil-dereference panic on the response path or being partially adopted
...
- A panic in the response-copy reader goroutine or the config watcher no longer kills the process or silently disables hot reload
- LLM findings whose step number falls outside the journey's real steps are dropped rather than clamped, and LLM-authored text is escaped for Markdown structure before it reaches either artifact
