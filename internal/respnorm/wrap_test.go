...
func TestWrap_ClientModelRewritesModelField(t *testing.T) {
	...
	out := readAll(t, Wrap(strings.NewReader(in), Options{ClientModel: "my-virtual-model", IsSSE: true, Protocol: "openai"}))
...
func TestWrap_UpstreamModelDrivesObservedModel(t *testing.T) {
	...
	ns := Wrap(strings.NewReader(in), Options{ClientModel: "agent", UpstreamModel: "requested-model", IsSSE: true, Protocol: "openai"})
	readAll(t, ns)
...
func TestWrap_IsSSEFalseSkipsDoneAppend(t *testing.T) {
	...
	out := readAll(t, Wrap(strings.NewReader(in), Options{ClientModel: "agent", IsSSE: false, Protocol: "openai"}))
...
func TestWrap_AnthropicProtocolNeverAppendsDone(t *testing.T) {
	...
	out := readAll(t, Wrap(strings.NewReader(in), Options{ClientModel: "claude", IsSSE: true, Protocol: "anthropic"}))
	if strings.Contains(out, "[DONE]") {
...
func TestWrap_OpaqueSkipsEveryTransform(t *testing.T) {
	...
	out := readAll(t, Wrap(strings.NewReader(in), Options{ClientModel: "agent", IsSSE: true, Protocol: "openai", Opaque: true}))
	if out != in {
