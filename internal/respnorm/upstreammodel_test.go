...
func TestUpstreamModelRecordedOnlyWhenItDiffers(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			rs := newStream(strings.NewReader(body), "coding", c.requested, false, "openai-completions", false)
			out := readAll(t, rs)
...
func TestUpstreamModelCapturedOncePerStream(t *testing.T) {
	...
	rs := newStream(src, "agent", "asked-for", true, "openai-completions", false)
	readAll(t, rs)
...
func TestUpstreamModelSkippedWhenRequestedNameUnknown(t *testing.T) {
	rs := newStream(strings.NewReader(`{"model":"anything"}`), "coding", "", false, "openai-completions", false)
	readAll(t, rs)
...
func TestUpstreamModelNotCapturedFromOpaqueBody(t *testing.T) {
	rs := newStream(strings.NewReader(`{"model":"real"}`), "coding", "asked", false, "openai-completions", true)
	readAll(t, rs)
	if got := rs.ObservedModel(); got != "" {
