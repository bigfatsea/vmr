...
func Detail(lang Lang) DetailText {
	...
	return DetailText{
		NormDescriptions: map[string]string{
			...
			"overflow_raw_passthrough":          "Response body exceeded the 32MB buffering cap; the normalizer gave up and passed the remaining bytes through as-is — subsequent steps like model rewrite/think stripping no longer run, equivalent to a direct connection",
