...
// llmRow holds journey_llm.go's literal templates, one row per Lang
// (Table's own doc comment). The big system prompts stay as named
// top-level consts below rather than moving into this row struct — they
// are already the target "one named blob per language" shape, and this
// package doc's own point (a wording change stays in one small file) would
// be undermined by burying them inside a composite literal. SectionTitle's
// "scope == """ branch is the only conditional here, identical both
// languages, written once in LLM below.
type llmRow struct {
	systemPrompt     string
	userPromptPrefix string
	userPromptSuffix string
	...
	noTextReply              string
	sectionTitleNoScopeFmt   string
	sectionTitleWithScopeFmt string
	sectionDisclaimerFmt     string
	cachedNote               string
...
var llmRows = Table[llmRow]{
	EN: {
		...
		sectionTitleNoScopeFmt:   "## LLM Interpretation (model: %s)\n\n",
		sectionTitleWithScopeFmt: "## LLM Interpretation (model: %s · %s)\n\n",
		sectionDisclaimerFmt:     "> The following was generated by `%s`. It is not the fact layer, may be wrong, and should be checked against the evidence table above.\n",
	...
	ZH: {
		...
		noTextReply:              "(无文本回复)",
		sectionTitleNoScopeFmt:   "## LLM 解读（模型：%s）\n\n",
		sectionTitleWithScopeFmt: "## LLM 解读（模型：%s · %s）\n\n",
		sectionDisclaimerFmt:     "> 以下内容由 `%s` 生成的解读，不是事实层，可能有误，请对照上面的证据表核实。\n",
...
func LLM(lang Lang) LLMText {
	...
	return LLMText{
		...
		UserPromptPrefix: r.userPromptPrefix,
		UserPromptSuffix: r.userPromptSuffix,

		SingleJourneySystemPrompt:     r.singleJourneySystemPrompt,
		SingleJourneyUserPromptPrefix: r.singleJourneyUserPromptPrefix,
		SingleJourneyUserPromptSuffix: r.singleJourneyUserPromptSuffix,

		DivergenceSystemPrompt:     r.divergenceSystemPrompt,
		DivergenceUserPromptPrefix: r.divergenceUserPromptPrefix,
		DivergenceUserPromptSuffix: r.divergenceUserPromptSuffix,

		ToolMisinterpretationSystemPrompt: r.toolMisinterpretationSystemPrompt,
		ToolMisinterpretationUserPrefix:   r.toolMisinterpretationUserPrefix,
		ToolMisinterpretationUserSuffix:   r.toolMisinterpretationUserSuffix,

		SemanticOscillationSystemPrompt: r.semanticOscillationSystemPrompt,
		SemanticOscillationUserPrefix:   r.semanticOscillationUserPrefix,
		SemanticOscillationUserSuffix:   r.semanticOscillationUserSuffix,

		GoalDriftSystemPrompt: r.goalDriftSystemPrompt,
		GoalDriftUserPrefix:   r.goalDriftUserPrefix,
		GoalDriftUserSuffix:   r.goalDriftUserSuffix,

		CompactionConstraintSystemPrompt: r.compactionConstraintSystemPrompt,
		CompactionConstraintUserPrefix:   r.compactionConstraintUserPrefix,
		CompactionConstraintUserSuffix:   r.compactionConstraintUserSuffix,

		PlanAuditSystemPrompt: r.planAuditSystemPrompt,
		PlanAuditUserPrefix:   r.planAuditUserPrefix,
		PlanAuditUserSuffix:   r.planAuditUserSuffix,

		CompletionClaimSystemPrompt: r.completionClaimSystemPrompt,
		CompletionClaimUserPrefix:   r.completionClaimUserPrefix,
		CompletionClaimUserSuffix:   r.completionClaimUserSuffix,

		NoTextReply: r.noTextReply,
		SectionTitle: func(model, scope string) string {
			if scope == "" {
				return fmt.Sprintf(r.sectionTitleNoScopeFmt, model)
			}
			return fmt.Sprintf(r.sectionTitleWithScopeFmt, model, scope)
		},
		SectionDisclaimer: func(model string) string { return fmt.Sprintf(r.sectionDisclaimerFmt, model) },
		CachedNote:        r.cachedNote,

		ScopeOverall:    r.scopeOverall,
		ScopeDivergence: r.scopeDivergence,
	}
}
