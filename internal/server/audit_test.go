...
func TestAuditRecordsFailoverBothLayers(t *testing.T) {
	...
	r := recs[0]
	if r.Model != "vm" || r.Protocol != "openai-completions" || r.Outcome != "ok" || r.Stream {
		t.Errorf("meta: %+v", r)
	}
	...
	if a1.Endpoint != "openai-completions:p1:model-one" || a1.Error != "transient" || a1.ErrorClass != "transient" || a1.Response == nil || a1.Response.Status != 500 || a1.Response.Body == nil {
		t.Errorf("failed attempt: %+v", a1)
	}
	if a1.Protocol != "openai-completions" || a1.Provider != "p1" || a1.Model != "model-one" {
	...
	if a2.Endpoint != "openai-completions:p2:model-two" || a2.Error != "" || a2.ErrorClass != "" || a2.Response == nil || a2.Response.Status != 200 {
		t.Errorf("success attempt: %+v", a2)
	}
	if a2.Protocol != "openai-completions" || a2.Provider != "p2" || a2.Model != "model-two" {
