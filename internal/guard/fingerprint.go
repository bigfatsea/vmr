...
// credential value (stamped into GuardRecord.Hit.FP): a truncated SHA-256
...
//
// Deterministic, no salt — a first-principles re-review found the
// originally-specified salt-keyed HMAC's one claimed protection (an
// operator-held salt makes a leaked report's fingerprints "correlatable
// but not independently confirmable") was never real: every `vmr analyze`
// output (macro/guard.json,
...
// already stores request bodies in full plaintext (local
