...
// Unicode steganography classification. Pure
// classification only — counting which code points appear, never deleting
// or rewriting them. The online sanitizer (sanitize.go) is
// the thing that turns a non-zero A/B-tier count into a deletion decision;
// this package only ever produces the count, because an offline
...
// a sanitizer-stamped SanitizedRunes are the same shape read the same way
// downstream.
const (
	...
	RuneCatVarSel     = "varsel"      // C: ZWNJ/ZWJ, LRM/RLM, U+FE00-FE0F, U+E0100-E01EF — required by real scripts/emoji, never deleted
)
...
// traffic per the corpus baseline — allocates nothing). s must already
...
// (SSE/JSON message assembly does not belong in guard). Scanning
// raw undecoded JSON bytes directly would miss a code point written as a
// \uXXXX escape entirely — exactly the documented evasion — which is why
// this package never takes raw wire bytes as its input.
...
// ClassifyRune returns r's category per the three-tier classification, or
// "" when r
