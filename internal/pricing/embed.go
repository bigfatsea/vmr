...
//
//go:embed standard.generated.yaml
...
//
//go:embed standard.curated.yaml
var standardCuratedYAML []byte

// LoadStandard parses and merges the embedded standard.generated.yaml and
// standard.curated.yaml into one Table, curated winning per-key conflicts
// (see Merge's doc comment for why that's a whole-row, not per-component,
// override). Called once by internal/config at validate() time — the
// result is cheap to hold onto (a few hundred rows) and never needs to be
// reparsed per request.
func LoadStandard() (*Table, error) {
	generated, err := ParseTable(standardGeneratedYAML)
	if err != nil {
		return nil, fmt.Errorf("embedded standard.generated.yaml: %w", err)
	}
	curated, err := ParseTable(standardCuratedYAML)
	if err != nil {
		return nil, fmt.Errorf("embedded standard.curated.yaml: %w", err)
	}
