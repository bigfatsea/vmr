...
// TestPricing_ProviderLegacyKeys_RejectedWithRenameHint pins that the two
// pre-2026-09 key names are no longer accepted: strict-YAML decode (outer
// KnownFields) rejects them like any unknown field, and the load error
// carries a targeted rename hint instead of leaving the user to guess.
func TestPricing_ProviderLegacyKeys_RejectedWithRenameHint(t *testing.T) {
	cases := []struct {
		name  string
		block string
	}{
	...
	for _, tc := range cases {
		_, err := Parse([]byte(pricingCfg("", tc.block, "gpt-4o")))
		if err == nil {
			t.Errorf("%s: legacy pricing key must be rejected, got a clean parse", tc.name)
			continue
		}
		if !strings.Contains(err.Error(), tc.name) {
			t.Errorf("%s: error should name the offending key, got %v", tc.name, err)
		}
		if !strings.Contains(err.Error(), "renamed map → aliases, overrides → rates") {
			t.Errorf("%s: error should carry the rename hint, got %v", tc.name, err)
		}
	}
}

// TestPricing_ProviderUnknownField_Rejected pins that a misspelled key in
// a providers[].pricing block is a load-time error — with the legacy shim
// gone, the block decodes under the same outer KnownFields strictness as
// the rest of the config, and the generic unknown-field message names the
// offending key.
...
`, "gpt-4o")
	_, err := Parse([]byte(yaml))
	if err == nil || !strings.Contains(err.Error(), "alises") {
