// Ver 2026-09-12 12:00, by dev
package report

import (
	"testing"

	"vmr/internal/chatmsg"
)

func TestClientEndpointCollectorGroupsAndSorts(t *testing.T) {
	c := newClientEndpointCollector()
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:p1:m1", usageInOK: true, usageOutOK: true,
		usage: chatmsg.Usage{In: 100, Out: 10}})
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:p2:m2", usageInOK: true, usageOutOK: true,
		usage: chatmsg.Usage{In: 500, Out: 50}})
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:p1:m1", usageInOK: true, usageOutOK: true,
		usage: chatmsg.Usage{In: 100, Out: 10}})
	c.add(&recRow{clientKey: "agent-b", endpoint: "openai-completions:p1:m1", usageInOK: true, usageOutOK: true,
		usage: chatmsg.Usage{In: 50, Out: 5}})
	rows := c.result()
	if len(rows) != 3 {
		t.Fatalf("rows = %d, want 3 (2 for agent-a, 1 for agent-b)", len(rows))
	}
	// agent-a sorts before agent-b (client-major); within agent-a, p2:m2 (600
	// in) sorts before p1:m1 (200 in, aggregated across the two adds).
	if rows[0].ClientKey != "agent-a" || rows[0].Endpoint != "openai-completions:p2:m2" {
		t.Errorf("rows[0] = %+v, want agent-a/openai-completions:p2:m2 first (higher tokens)", rows[0])
	}
	if rows[1].ClientKey != "agent-a" || rows[1].Endpoint != "openai-completions:p1:m1" {
		t.Errorf("rows[1] = %+v", rows[1])
	}
	if rows[1].Requests != 2 || rows[1].TokensIn != 200 {
		t.Errorf("rows[1] aggregation = %+v, want Requests=2 TokensIn=200", rows[1])
	}
	if rows[2].ClientKey != "agent-b" {
		t.Errorf("rows[2] = %+v, want agent-b", rows[2])
	}
}

// The key must be the full endpoint label, not the model name — the whole
// reason this feature exists: config.mba.yaml can put the same model under
// two different provider accounts, and only the endpoint label preserves
// that distinction.
func TestClientEndpointCollectorKeysByFullEndpointNotModel(t *testing.T) {
	c := newClientEndpointCollector()
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:volcengine:deepseek-v4-flash", usageInOK: true, usageOutOK: true, usage: chatmsg.Usage{In: 10}})
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:volcengine2:deepseek-v4-flash", usageInOK: true, usageOutOK: true, usage: chatmsg.Usage{In: 20}})
	rows := c.result()
	if len(rows) != 2 {
		t.Fatalf("rows = %d, want 2 (same model, two distinct accounts must not merge)", len(rows))
	}
}

func TestClientEndpointCollectorSkipsEmptyKeys(t *testing.T) {
	c := newClientEndpointCollector()
	c.add(&recRow{clientKey: "", endpoint: "openai-completions:p1:m1", usageInOK: true, usageOutOK: true, usage: chatmsg.Usage{In: 10}})
	c.add(&recRow{clientKey: "agent-a", endpoint: "", usageInOK: true, usageOutOK: true, usage: chatmsg.Usage{In: 10}})
	if rows := c.result(); len(rows) != 0 {
		t.Errorf("rows = %+v, want empty (both records missing a grouping key)", rows)
	}
}

// A record with no usable usage still counts as a request against that
// (client, endpoint) pair — it just contributes no tokens.
func TestClientEndpointCollectorUsageNotOKCountsRequestOnly(t *testing.T) {
	c := newClientEndpointCollector()
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:p1:m1", usageInOK: false, usageOutOK: false})
	rows := c.result()
	if len(rows) != 1 || rows[0].Requests != 1 || rows[0].TokensIn != 0 {
		t.Errorf("rows = %+v, want 1 request, 0 tokens", rows)
	}
}

func TestClientEndpointCollectorDeterministicTieBreak(t *testing.T) {
	c := newClientEndpointCollector()
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:zeta:m", usageInOK: true, usageOutOK: true, usage: chatmsg.Usage{In: 100}})
	c.add(&recRow{clientKey: "agent-a", endpoint: "openai-completions:alpha:m", usageInOK: true, usageOutOK: true, usage: chatmsg.Usage{In: 100}})
	rows := c.result()
	if len(rows) != 2 || rows[0].Endpoint != "openai-completions:alpha:m" || rows[1].Endpoint != "openai-completions:zeta:m" {
		t.Errorf("order = %v, want alpha before zeta on a token tie", rows)
	}
}
