...
// A one-shot conformance check between this package's own session grouping
// (AnalyzeSessions/group) and ctxgraph's independently-computed lineage
// grouping (Scan), on the same corpus. This was R8's only mitigation before
// group() was changed to source its grouping directly from ctxgraph's Scan
// output ("Phase A permanently forks from report, Phase B never happens") —
// every difference had to land on one of two explicit outcomes, expectedSame
// or a whitelisted knownImprovement (the anchor-gluing case: two lineages
// glued together across a hidden Contract/Fork break).
//
// Now that group() sources its grouping FROM this same Scan output, every
// assertion below is now a same-source consistency check rather than a
// comparison between two independent implementations — but it still earns
...
// first) and as the one place the anchor-gluing fix is verified end-to-end
// through the production AnalyzeSessions entry point, including the new
...
// actual production entry points — AnalyzeSessions and Scan. Since group()
// now sources ReqInfo.SessKey directly from the correlated ctxgraph.Manifest,
// this is closer to a wiring check (did the (path,line) correlation in
// group() actually run) than an independent cross-implementation comparison.
// Still the foundation every other assertion here depends
...
// f6AnchorGluedFixture reproduces the real s231 pattern the design doc
...
// report started consuming ctxgraph's edit classification, report had no
// Contract-detection mechanism and glued both halves into one session purely
// because the anchor survived; ctxgraph's edit classifier (Classify) always
// split it into two lineages, and report was changed to consume that same
// split instead of its own former per-SessKey bucketing — see
...
// to be the one whitelisted knownImprovement outcome and is now
...
func TestConformance_F6AnchorGluedLineageSplitMatchesCtxgraph(t *testing.T) {
	...
	if len(a.Sessions) != 2 {
		t.Fatalf("report sessions = %d, want 2 (report must now split on the same Contract edge ctxgraph does)", len(a.Sessions))
	}
	...
	if bf == nil {
		t.Fatal("second session's lineage should carry a BrokeFrom (the anchor-glued case's structural signature)")
	}
...
// invariant of the anchor-gluing case above, checked across every fixture
// this file knows about: a ctxgraph lineage must never span more than one
// distinct report session. That would mean ctxgraph MERGED content report
// kept separate — the opposite failure mode from anchor-gluing, and not on either
