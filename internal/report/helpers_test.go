...
func TestExtractUsage(t *testing.T) {
	cases := []struct {
		...
		{"openai json", map[string]any{"usage": map[string]any{"prompt_tokens": 10.0, "completion_tokens": 20.0}}, "openai-completions", 10, 20, 0, 0, true},
		{"anthropic json", map[string]any{"usage": map[string]any{"input_tokens": 5.0, "output_tokens": 7.0}}, "anthropic-messages", 5, 7, 0, 0, true},
		{"no usage", map[string]any{"choices": []any{}}, "openai-completions", 0, 0, 0, 0, false},
		{"anthropic sse", "event: message_start\ndata: {\"type\":\"message_start\",\"message\":{\"usage\":{\"input_tokens\":41,\"output_tokens\":1}}}\n\nevent: message_delta\ndata: {\"type\":\"message_delta\",\"usage\":{\"output_tokens\":9}}\n\ndata: [DONE]\n", "anthropic-messages", 41, 9, 0, 0, true},
		{"openai sse with usage chunk", "data: {\"choices\":[{\"delta\":{\"content\":\"x\"}}]}\n\ndata: {\"choices\":[],\"usage\":{\"prompt_tokens\":6,\"completion_tokens\":3}}\n\ndata: [DONE]\n", "openai-completions", 6, 3, 0, 0, true},
		{"sse without usage", "data: {\"choices\":[{\"delta\":{\"content\":\"x\"}}]}\n\ndata: [DONE]\n", "openai-completions", 0, 0, 0, 0, false},
		...
		}}, "openai-completions", 110, 20, 90, 0, true},
		// OpenAI Responses: input_tokens_details.cached_tokens is a subset already inside input_tokens.
		{"openai responses cache json", map[string]any{"usage": map[string]any{
			"input_tokens": 110.0, "output_tokens": 20.0,
			"input_tokens_details": map[string]any{"cached_tokens": 90.0},
		}}, "openai-responses", 110, 20, 90, 0, true},
		...
		}}, "openai-completions", 110, 20, 80, 0, true},
	}
	for _, c := range cases {
		tt := c
