...
func buildReplayEndpoint(cfg *config.Config, opts Options, rv *recordView) (ad adapter.Adapter, protocol string, providerCfg config.Provider, ep *core.Endpoint, err error) {
	...
	ep = &core.Endpoint{
		...
		Model:       model,
		// The provider's role_map must reach BuildRequest the same way it
		// does on live traffic: BuildSnapshot is what copies RoleMap onto a
		// live Endpoint, but ep is hand-assembled here and never goes
		// through it — without this line a configured role_map (e.g.
