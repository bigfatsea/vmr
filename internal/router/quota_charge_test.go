...
func TestChargeQuota_Tokens_SniffedUsage_SSE(t *testing.T) {
	...
	sse := "data: {\"choices\":[{\"delta\":{\"content\":\"hi\"}}]}\n\n" +
		...
		"data: [DONE]\n\n"
	rs := newRespStream(strings.NewReader(sse), "m", "m", true, "openai", false)
	if _, err := io.Copy(io.Discard, rs); err != nil {
		t.Fatalf("drain: %v", err)
	}
...
`
	rs := newRespStream(strings.NewReader(sse), "m", "m", true, "anthropic", false)
	if _, err := io.Copy(io.Discard, rs); err != nil {
		t.Fatalf("drain: %v", err)
	}
	u, ok := rs.Usage()
...
func TestChargeQuota_Tokens_TruncatedMidStream_Degrades(t *testing.T) {
	...
	rs := newRespStream(tr, "m", "m", false, "openai", false)
...
func TestChargeQuota_Tokens_OpaqueAlwaysDegrades(t *testing.T) {
	...
	rs := newRespStream(strings.NewReader(body), "m", "m", false, "openai", true)
	if _, err := io.Copy(io.Discard, rs); err != nil {
		t.Fatalf("drain: %v", err)
	}
	if _, ok := rs.Usage(); ok {
