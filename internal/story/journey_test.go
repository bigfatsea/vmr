...
func TestBuildAll_MatchesIndividualBuild(t *testing.T) {
	...
	for i, l := range g.Lineages {
		...
		if gotMD, wantMD := RenderMarkdown(got[i], ComputeMetrics(got[i]), ComputeFindings(got[i], i18n.EN), i18n.EN, false), RenderMarkdown(want, ComputeMetrics(want), ComputeFindings(want, i18n.EN), i18n.EN, false); gotMD != wantMD {
